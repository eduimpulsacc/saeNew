	<?	session_start(); 
		...
		echo $_SESSION['cmb_ins'];
				$sql="select sum(estadistica.cant_conex) as total2 from estadistica where rdb not in (select rdb from estadistica where rdb = ".$_cmb_ins." ) UNION select sum(estadistica.cant_conex) as total1 from estadistica where rdb = ".$_cmb_ins;
				echo $sql;
