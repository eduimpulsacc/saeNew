	<?	session_start(); 
		...
		$sql_corp = " where estadistica.rdb in (select rdb from corp_instit where num_corp = '".$_CORP."') ";
		$sql="SELECT SUM(suma),rdb FROM ( SELECT SUM(estadistica.cant_conex) as suma,estadistica.perfil,estadistica.rdb,estadistica.id_usuario FROM estadistica $sql_corp GROUP BY estadistica.cant_conex,estadistica.perfil,estadistica.rdb,estadistica.id_usuario) AS LA GROUP BY la.rdb ORDER BY sum DESC";
		$sql_total="SELECT SUM(suma) FROM ( SELECT SUM(estadistica.cant_conex) as suma FROM estadistica  $sql_corp  GROUP BY estadistica.cant_conex) AS LA ORDER BY sum DESC";
		echo $sql;
