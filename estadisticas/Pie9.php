	<?	session_start(); 
		...
		echo $_SESSION['cmb_ins'];
				$sql="SELECT SUM(suma),rdb,perfil FROM ( SELECT SUM(estadistica.cant_conex) as suma,estadistica.perfil,estadistica.rdb,estadistica.id_usuario FROM estadistica GROUP BY estadistica.cant_conex,estadistica.perfil,estadistica.rdb,estadistica.id_usuario) AS LA GROUP BY la.rdb,la.perfil ORDER BY sum DESC LIMIT 8";
				echo $sql;
				$sql_total="SELECT SUM(suma) FROM (SELECT SUM(estadistica.cant_conex) as suma FROM estadistica GROUP BY estadistica.cant_conex) as total";
				...
				$labels= array();
				   ...
				   // etiquetas que le pasaremos a Open Flash Chart
					 while ($arr_adms = @pg_fetch_array($resultado)) {
						...
						$val=$arr_adms[0];
						$institucion=substr($arr_institucion['nombre_instit']," ",7);
						$labels[] =$institucion;
