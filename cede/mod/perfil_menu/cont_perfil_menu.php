...
if($funcion==1){ 
	...
	if($cmbPERFIL!=0){
		 $sql = "SELECT * FROM cede.perfil_menu WHERE rdb=".$institucion." AND id_perfil=".$cmbPERFIL;
...
<? $sql = "SELECT id_menu,nombre,bool_i,bool_m,bool_e,bool_v FROM cede.menu WHERE nivel=1 ORDER BY orden ASC";
$rs_menu = @pg_Exec($obj_Perfil_Menu->Conec->conectar(),$sql);
for($i=0;$i<@pg_numrows($rs_menu);$i++){
$fila_menu = @pg_fetch_array($rs_menu,$i);
if($caso==2){
$sql ="SELECT id_menu FROM cede.perfil_menu WHERE rdb=".$institucion." AND id_perfil=".$cmbPERFIL." AND id_menu=".$fila_menu['id_menu'];
...
?>
<tr>
<td class="textosimple" width="10%">&nbsp;<input name="ck_menu<?=$i;?>"  id="ck_menu<?=$i;?>" type="checkbox" value="<?=$fila_menu['id_menu'];?>" <?=$activo_menu;?>>&nbsp;<?=$fila_menu['nombre'];?></td>
<td width="90%"><table width="100%" border="1" cellspacing="0" cellpadding="0">
<tr>
...
<tr>
<td width="25%"  class="textosimple"><table width="100%" border="1" cellspacing="0" cellpadding="0">
<? $sql = "SELECT id_categoria,nombre_categoria,bool_i,bool_m,bool_e,bool_v FROM cede.menu_categoria WHERE id_menu=".$fila_menu['id_menu']." AND nivel=1 ORDER BY orden_categoria ASC";
...
$activo_cat_v="&nbsp;";
if($caso==2){
$sql ="SELECT bool_i,bool_m,bool_e,bool_v FROM cede.perfil_menu WHERE rdb=".$institucion." AND id_perfil=".$cmbPERFIL." AND id_menu=".$fila_menu['id_menu']." AND id_categoria=".$fila_cat['id_categoria'];
...
?>
<tr>
<?  $sql ="SELECT id_item,nombre_item,bool_i,bool_m,bool_e,bool_v FROM cede.menu_categ_item WHERE id_menu=".$fila_menu['id_menu']." AND id_categoria=".$fila_cat['id_categoria']." AND nivel=1 ORDER BY orden_item ASC";
...
$activo_item_v="&nbsp;";
if($caso==2){
$sql ="SELECT bool_i,bool_m,bool_e,bool_v FROM cede.perfil_menu WHERE rdb=".$institucion." AND id_perfil=".$cmbPERFIL." AND id_menu=".$fila_menu['id_menu']." AND id_categoria=".$fila_cat['id_categoria']." AND id_item=".$fila_item['id_item'];
...
<tr>
<td  width="25%" class="textosimple"><? if($fila_menu['bool_i']==1){ ?>
<input name="ck_ingreso[<?=$i;?>]" type="checkbox" value="1" <?=$activo_item_i;?> >
&nbsp;I&nbsp;
<? } ?></td>
<td  width="25%" class="textosimple"><? if($fila_menu['bool_m']==1){ ?>
<input name="ck_modifica[<?=$i;?>]" type="checkbox" value="1" <?=$activo_item_m;?> >
&nbsp;M&nbsp;
<? } ?></td>
<td  width="25%" class="textosimple"><? if($fila_menu['bool_e']==1){ ?>
<input name="ck_elimina[<?=$i;?>]" type="checkbox" value="1" <?=$activo_item_e;?>>
&nbsp;E&nbsp;
<? } ?></td>
<td  width="25%" class="textosimple"><? if($fila_menu['bool_v']==1){ ?>
