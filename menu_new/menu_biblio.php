...
<div id="wrapper"><img src="<?=$d;?>menu_new/images/redondo.jpg" width="220" height="22">
	<ul class="menu">
   	<? 
	if($_PERFIL==0 || $_PERFIL==64){
		$sql ="SELECT id_menu,nombre,url FROM biblio.menu ORDER BY orden ASC";
	}else{
		$sql ="SELECT id_menu,nombre,url FROM biblio.menu WHERE id_menu in (SELECT id_menu FROM biblio.perfil_menu WHERE id_perfil=".$_PERFIL." AND rdb=".$_INSTIT.") ORDER BY orden ASC";
	}
...
<? 	
		if($_PERFIL==0 || $_PERFIL==64){
			$sql ="SELECT id_categoria,nombre,url FROM biblio.menu_categoria WHERE id_menu=".$fila_menu['id_menu']." ORDER BY orden ASC";																																																																																																																																																																																																																																																																																																																																																																	
		}else{
			$sql ="SELECT id_categoria,nombre,url FROM biblio.menu_categoria WHERE id_menu=".$fila_menu['id_menu']." AND id_categoria in (SELECT id_categoria FROM biblio.perfil_menu WHERE id_perfil=".$_PERFIL." AND rdb=".$_INSTIT.") ORDER BY orden ASC";																																																																																																																																																																																																																																																																																																																																																																	
		}
