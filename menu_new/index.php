...
<div id="wrapper"><img src="<?=$d;?>menu_new/images/redondo.jpg" width="220" height="22">
	<ul class="menu">
   	<? 
	if($_PERFIL==0){
		$sql ="SELECT id_menu,nombre,url FROM planificacion.menu ORDER BY orden ASC";
	}else{
		$sql ="SELECT id_menu,nombre,url FROM planificacion.menu WHERE id_menu in (SELECT id_menu FROM planificacion.perfil_menu WHERE id_perfil=".$_PERFIL." AND rdb=".$_INSTIT.") ORDER BY orden ASC";
	}
...
<? 	
		if($_PERFIL==0){
			$sql ="SELECT id_categoria,nombre,url FROM planificacion.menu_categoria WHERE id_menu=".$fila_menu['id_menu']." ORDER BY orden ASC";																																																																																																																																																																																																																																																																																																																																																																	
		}else{
			$sql ="SELECT id_categoria,nombre,url FROM planificacion.menu_categoria WHERE id_menu=".$fila_menu['id_menu']." AND id_categoria in (SELECT id_categoria FROM planificacion.perfil_menu WHERE id_perfil=".$_PERFIL." AND rdb=".$_INSTIT.") ORDER BY orden ASC";																																																																																																																																																																																																																																																																																																																																																																	
		}
...
<?			
             ...
             <?php  if(($_PERFIL==0 || $_INSTIT==14762) && $fila_menu['id_menu']==1){?>
       <li><a href="../../planificacion/plani_anual/plani_anual.php">Planificaci&oacute;n mensual</a>
        <?php }?>
			</ul></li>
<?		}else{?>
		</li>
