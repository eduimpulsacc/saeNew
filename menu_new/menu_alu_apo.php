...
<div id="wrapper"><img src="<?=$d;?>menu_new/images/redondo.jpg" width="220" height="22">
	<ul class="menu">
   	<? 
	if($_PERFIL==0){
		$sql ="SELECT id_menu,menu FROM menu_alu_apo ORDER BY orden ASC";
	}else{
		...
		$sql ="SELECT id_menu,menu,url FROM menu_alu_apo ORDER BY orden ASC";
	}
	...
	for($i=0;$i<@pg_numrows($rs_menu);$i++){
		$fila_menu = @pg_fetch_array($rs_menu,$i);
			...
			?>
		<li><a href="<? echo $url;?>"><?=$fila_menu['menu'];?></a>
<? 	
		if($_PERFIL==0){
			$sql ="SELECT id_categoria,nombre,url FROM menu_cat_alu_apo WHERE id_menu=".$fila_menu['id_menu']." ORDER BY orden ASC";																																																																																																																																																																																																																																																																																																																																																																	
		}else{
			$sql ="SELECT id_categoria,nombre,url FROM menu_cat_alu_apo WHERE id_menu=".$fila_menu['id_menu']." ORDER BY orden ASC";																																																																																																																																																																																																																																																																																																																																																																	
