...
<script>
	...
	function cargar_intitu(num_corp){
		...
		//alert(parametros);
		$.ajax({
			url:'mod/reportes/reporteExcel/cont_motor.php',
		    data:parametros,
		    type:'POST',
...
function carga_pauta(nacional){
	
	//var bloque = $('#cmbCARGO').val();
	var parametros ="funcion=pauta&nacional="+nacional;
	//alert(parametros);
	$.ajax({
		url:'mod/reportes/reporteExcel/cont_motor.php',
		data:parametros,
	...
	function valida(form){
		...
		if($('#cmbPAUTA').val()==0){
		alert("Escoja Una Pauta de Evaluación");
		return false;
	 }	
	
		form.action = 'mod/reportes/reporteExcel/printReporteExcel.php';
		form.submit(true);		
