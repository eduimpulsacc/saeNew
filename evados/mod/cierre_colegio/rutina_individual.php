...
for($j=0;$j<pg_numrows($rs_plantilla);$j++){
	...
	for($i=0;$i<pg_numrows($rs_evaluado);$i++){
		...
		echo "\t".$sql="SELECT sum(valor_final), COUNT(*) as cantidad FROM evados.eva_cierre_dimension_final WHERE id_cierre=$cierre and id_plantilla=".$fila['id_plantilla']." AND rut_Evaluado=".$fila['rut_evaluado']." AND id_periodo=".$fila['id_periodo']." ";
