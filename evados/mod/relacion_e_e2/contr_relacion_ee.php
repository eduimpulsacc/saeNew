	  ...
	  if($funcion==6){ // busca evaluadores por bloque
		  ...
		  <thead>
			...
			<tbody>';
			  ...
			  $fila = pg_fetch_array($result,$e);
						...
						//$check1 = "<a href='#' onclick='buscar_relacion(".$fila['rut_evaluador'].",".$fila['id_cargo'].")' ><img src='img/PNG-48/Search.png' width='22' height='22' border='0' /></a>";

                        $check2 = "<a href='#' onclick='insertar_relacion(".$fila['rut_evaluador'].",".$fila['id_cargo'].")' ><img src='img/PNG-48/Add.png' width='22' height='22' border='0' /></a>";
		...
		$Curso_pal = CursoPalabra($fila['id_curso'], 1,$obj_Relacion_ee->Conec->conectar());
			 ...
			 $table.='<td  class="textosimple">'.$Curso_pal.'</td>';
					  ...
					  $rut_del_evaluador = '<input type="hidden" id="rut_evaluador" name="rut_evaluador" value="'.$fila['rut_evaluador'].'">';
					  
				$table .='<td>'. $rut_del_evaluador.$check2.'</td>';	
						$table .='</tr>';	
		...
		$result = $obj_Relacion_ee->buscarevaluados($id_cargo,$_ANO,$_INSTIT);
		  ...
		  <thead>
			<tr>
			  ...
			  <th width="30" class="textonegrita" >Edit</th>
			...
			<tbody>';
					...
					//cargar_evaluador('.$e.','.$fila['rut_evaluado'].','.$fila['id_cargo'].'),	
					
					$table .= '<tr>
					<td class="textosimple"><input type="hidden" class="revaluado" name=r_evaluado[] id="revaluado'.$e.'" value="'.$fila['rut_evaluado'].'">'.$fila['rut_evaluado'].'</td>
					<td class="textosimple">'.$fila['ape_pat']." ".$fila['ape_mat']." ".$fila['nombre_emp'].'&nbsp;</td>';
					$table.='<td class="textosimple"><input name=c_evaluado[] id="cargar_evaluador'.$e.'" type="checkbox" value="'.$fila['rut_evaluado'].'" class="cevaluado" onchange="cargar_evaluador2()" />&nbsp;</td>
					</tr>';
					   
						 }// fin for
		   
						$table .= "<tbody></table>";
						//echo $rut_del_evaluado = '<input type="text" id="rut_evaluado" name="rut_evaluado" value="'.$fila['rut_evaluado'].'">';
						echo $table;
...
$check1 = "<a href='#' onclick='eliminar_relacion(".$fila['rut_evaluador'].",".$fila['id_cargo'].",".$fila['rut_evaluado'].",".$fila['cargo_evaluado'].")' ><img src='img/PNG-48/Delete.png' width='22' height='22' border='0' /></a>";
	 ...
	 $tabla .= '<h3 align="center" >Reporte de Relacionados Evaluador Evaluados</h3>';
		...
		$result = $obj_Relacion_ee->buscarevaluadorestodos($id_bloque,$cargo_evaluador,$idano,$periodo,$rut_evaluador);
