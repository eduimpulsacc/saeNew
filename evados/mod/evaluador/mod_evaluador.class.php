...
class Evaluador{
	...
	public function listadoEvaluados($rdb,$cargo,$ano,$periodo){  // guarda en la base datos 
		...
		$sql = "SELECT 	empleado.rut_emp as rut ,empleado.dig_rut as dv,UPPER(CAST(ape_pat || ' ' || ape_mat  || ' ' || empleado.nombre_emp as varchar)) as nombre, eva.porcentaje,eva.id_cargo FROM empleado INNER JOIN trabaja ON empleado.rut_emp=trabaja.rut_emp INNER JOIN cargos ON trabaja.cargo=cargos.id_cargo And cargos.id_cargo = '".$cargo."' LEFT JOIN evados.eva_evaluador eva ON empleado.rut_emp=eva.rut_evaluador  AND eva.id_periodo='".$periodo."' and id_ano='".$ano."' and  eva.id_cargo = '".$cargo."' WHERE trabaja.rdb='".$rdb."' ORDER BY nombre ASC; "; 
	...
	if($rut=="todos"){
	  ...
	  if(pg_num_rows($result)!=0){
			...
			for($i=0;$i<pg_num_rows($result);$i++){
				...
				$sql_2 = "SELECT * FROM evados.eva_evaluador WHERE 
				...
				//if(pg_num_rows($rs_1)==0){
					...
					$insert .="INSERT INTO evados.eva_evaluador (id_ano,rut_evaluador,porcentaje,id_cargo,id_periodo) 
	 ...
	 if($this->crear_perfil($rut,$rdb)){ 
		...
		WHERE id_ano = ".$ano." AND  rut_evaluador =".$rut." and id_periodo=".$id_periodo." AND  porcentaje = 1  AND id_cargo=".$cargo."; ";
		...
		$sql = "SELECT * FROM evados.eva_evaluador 
		WHERE id_ano = ".$ano." AND  rut_evaluador =".$rut." and id_periodo=".$id_periodo." AND id_cargo=".$cargo."; ";
		$rs = pg_exec($this->Conec->conectar(),$sql) or die ("fallo:".$sql);	
	...
	public 	function CopiarEvaluadores($a,$b,$periodo){
		...
		if(pg_num_rows($result)  > 0){
			...
			if(pg_num_rows($rs_ano)  > 0){
				...
				FROM evados.eva_evaluador WHERE id_ano = (SELECT ae.ano_anterior FROM evados.eva_ano_escolar ae 
		...
		public function consulta_cargo($rut,$ano,$periodo){
			$sql="SELECT c.nombre_cargo
					FROM evados.eva_evaluador ee 
					INNER JOIN cargos c ON ee.id_cargo=c.id_cargo
					WHERE ee.id_ano=".$ano." and ee.id_periodo=".$periodo." and ee.rut_evaluador=".$rut;
			 $result = pg_Exec( $this->Conec->conectar(),$sql ) or die( "Error 12" );
