...
if($cmb_tipou==1){
	...
	if($cmb_usuario==0){
		$rs_listado = $ob_reporte->listaDevolucionEmpleadoTodos($conn,$_INSTIT,$_ANO,CambioFE($txtFECHADEV));
		
	}
	//si no viene empleado seleccionado
	else{
		$rs_listado = $ob_reporte->listaDevolucion($conn,$cmb_usuario,$_ANO,CambioFE($txtFECHADEV));
...
if($cmb_tipou==2){
	...
	if($cmb_usuario==0){
		$rs_listado = $ob_reporte->listaDevolucionApoderadoTodos($conn,$_INSTIT,$_ANO,$cmb_curso,CambioFE($txtFECHADEV));
	}
...
if($cmb_tipou==3){
	...
	if($cmb_usuario==0){
		$rs_listado = $ob_reporte->listaDevolucionAlumnoTodos($conn,$_INSTIT,$_ANO,$cmb_curso,CambioFE($txtFECHADEV));
	}
...
<table width="100%" border="0">
  <tr>
    <td colspan="3" class="textonegrita" align="center">REPORTE DEVOLUCIONES DE LIBROS <?php echo $tp ?></td>
  </tr>
  <tr>
  ...
  <tr class="cuadro01">
    ...
    <?php if($cmb_usuario==0){?>
    <td width="40%" align="center">USUARIO</td>
   <?php  }?>
    <td width="34%" align="center">T&Iacute;TULO</td>
    <td width="22%" align="center">FECHA ENTREGA</td>
    </tr>
  <?php if(pg_numrows($rs_listado)>0){
	  for($o=0;$o<pg_numrows($rs_listado);$o++){  
	   ...
	   case 2:
	  	$estado="DEVUELTO";
		$rea=$rea+1;
		$fdev = CambioFD($fila_res['fecha_entrega']);
	   break;
	   case 3:
	   	$estado="ATRASADO";
		$anu=$anu+1;
  ...
  <?php }else{?>
  <tr class="detalleoff">
    <td colspan="5" align="center">SIN INFORMACI&Oacute;N DE DEVOLUCIONES</td>
    </tr>
    <?php }?>
    </table>
