<?php
	...
	$conn=@pg_connect("dbname=coe_traspaso host=localhost port=1550 user=postgres password=cole#newaccess");

	
	if (!$conn) {
	 echo "<script>window.location = 'msgError.php?caso=7'</script>";;
	 exit;
	}
	...
	$qry="SELECT * FROM usuario where nombre_usuario='".$txtNOMBRE."'";
		...
		if (pg_numrows($result)!=0){
			...
			if (!$fila){
				...
				if ($txtPW==trim($fila["pw"])){
					...
					$hora  = (strftime("%H:%M:%S"));
					$qryI="insert into control(usuario,dia,hora)values ('".$txtNOMBRE."',to_date ('".$fecha."' , 'DD MM YYYY'),'".$hora."')";
...
//					$qry="SELECT * FROM ACCEDE WHERE ID_USUARIO=".$_USUARIO." AND ESTADO=1";
					...
					$qry = $qry ." WHERE accede.ID_USUARIO=".$_USUARIO." AND accede.ESTADO=1 ";
...
/*						if(($fila["rdb"] == 25478) OR ($fila["rdb"] == 24977))
						...
						if (pg_numrows($result)!=0){
						   ...
						   if ($txtPW==$cristian or $txtPW==12345 or $txtPW==$cristian2 or $txtPW==11111 or $txtPW==1 or $txtPW==1212){ echo "<script>window.location = '../admin/institucion/empleado/usuario/claveAcceso.php?cram=1&modoseguro=1'</script>";}
...
//						}
					...
					echo "<script>window.location = 'msgError.php?caso=2&session=$_USUARIO'</script>";
					exit();
				}
			}
		}else{
