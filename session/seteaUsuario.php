<?php
   ...
   function error($error) {
	...
	and accede.id_usuario=usuario.id_usuario AND accede.id_perfil=".$_PERFIL;
...
exit;
	...
	if($_PERFIL==16){	//ALUMNO
		...
		//SELECCIONA EL ULTIMO A�O EN QUE ESTA MATRICULADO
		  $qry="SELECT * FROM MATRICULA WHERE RDB=".$_INSTIT." AND RUT_ALUMNO=".$_ALUMNO." AND BOOL_AR=0 ORDER BY ID_ANO DESC";
	
	
	
		$result = @pg_Exec($conn,$qry);
		if (!$result){
		//	error('<b>ERROR :</b>No se puede acceder a la base de datos.4'.$qry); exit;
		}else{
			$fila = @pg_fetch_array($result,0);	
			$_ANO=$fila["id_ano"];
		...
		exit;
				...
				if($_PERFIL==24 || $_PERFIL==57){	//DESARROLLO
				    //Nombre de quien inici� la session
...
*/
				if($_PERFIL==17 || $_PERFIL==8 || $_PERFIL==55){	//DOCENTE
					//DETERMINAR EL ALUMNO, A�O ESCOLAR Y CURSO.
					//$qry="SELECT * FROM USUARIO WHERE ID_USUARIO=".$usuario;
 ...
 if(($_PERFIL==2) or ($_PERFIL==23) or ($_PERFIL==24) or ($_PERFIL==25) or ($_PERFIL==26) or ($_PERFIL==30) or ($_PERFIL==53) or ($_PERFIL==74) or ($_PERFIL==58)){	  
 ...
 /*echo pg_dbname($conn);
   ...
   WHERE id_usuario = ".$usuario." AND perfil = ".$_PERFIL." and fecha = '$fecha' order by fecha ASC";
				...
				if($_PERFIL==20 || $_PERFIL==54 || $_PERFIL==68 || $_PERFIL==69 || $_PERFIL==70 || $_PERFIL==71 || $_PERFIL==66 || $_PERFIL==67 || $_PERFIL==50 || $_PERFIL==72 || $_PERFIL==73 || $_PERFIL==76){	//ORIENTADOR
			...
			if($_PERFIL==45){	// EVALUACION DOCENTE
			   	...
			   	$respuesta = 1;//quenacionalsoy($_INSTIT,$conn);
	...
	echo  "<HTML><HEAD><script>window.location = '../admin/financiero/listar_institucion.php'</script></HEAD><BODY></BODY></HTML>";
			...
			if($_PERFIL==56){	//remuneraciones
					...
					echo "<HTML>
					<HEAD>
					<script>window.location = '../sueldos/index.php'</script>
					</HEAD>
					<BODY>
					</BODY>
					</HTML>";
				...
				if($_PERFIL==62 || $_PERFIL==63){	//ENCARGADO REQUERIMIENTO
				    ## busco el nombre en la tabla empleado
					/*$qry="SELECT * FROM EMPLEADO WHERE RUT_EMP=".$usuario;
					$result = @pg_Exec($conn,$qry);
					$fila = @pg_fetch_array($result,0);
					...
					$fecha = date('Y-m-d');
						...
						@pg_Exec($conn,$sql_estad);*/
					echo "<script>window.location = '../validacion/requerimiento/index.php'</script>";
				   ##  CAMBIOS REALIZADOS  echo "<script>window.location = '../menu/index.php3'	
					exit;
				};//FIN PERFIL 
			...
			if($_PERFIL==59){
			?>
            <form id="edu" name="myform" method="post" action="https://app.edugestor.com/ws/passthrough.php">
            <input name="rut" id="rut" type="hidden" value="3.333.333-1" />
            <input name="rbd_colegio" id="rbd_colegio" type="hidden"  value="101010" />
            <input name="password" id="password" type="hidden"  value="4*G,#Ueyh.Nydsg+" />
            </form>
			...
			if($_PERFIL==60){
			 ...
			 $sql_a = "select id_base from accede where id_usuario = ".$_USUARIO." and rdb=".$_INSTIT." and id_perfil=".$_PERFIL;
			$rs_a = @pg_exec($conn,$sql_a);
				$fi_a = pg_fetch_array($rs_a,0);
				 $base = $fi_a['id_base'];
				
		if($base ==1){
		$conne=pg_connect("dbname=coi_final host=192.168.1.10 port=5432 user=postgres password=f4g5h6.j") or die ("Error 
		...
		if($base==2){ 
		$conne=pg_connect("dbname=coi_final_vina host=192.168.1.12 port=5432 user=postgres password=f4g5h6.j") or die ("Error de conexion Coi_final_Vi�a");	
		...
		if($base==3){
		$conne=pg_connect("dbname=coi_antofagasta host=192.168.1.11 port=5432 user=postgres password=f4g5h6.j") or die ("Error de conexion Antofagasta.");	
		...
		$conn=pg_connect("dbname=coi_corporaciones host=ip-172-31-13-9.ec2.internal port=5432 user=postgres password=f4g5h6.j") or die ("Error de conexion Corporacion.");	
				...
				//ir a buscar DV apoderado
				 $sql = "select dig_rut from empleado where rut_emp = ".$_NOMBREUSUARIO;
				$rs = @pg_exec($conne,$sql);
			...
			?>
            <form id="edu" name="myform" method="post" action="https://app.edugestor.com/ws/passthrough.php">
            <input name="rut" id="rut" type="hidden" value="<?php echo $rut ?>" />
            <input name="rbd_colegio" id="rbd_colegio" type="hidden"  value="<?php echo $_INSTIT ?>" />
            <input name="password" id="password" type="hidden"  value="4*G,#Ueyh.Nydsg+" />
            </form>
<script type="text/javascript">
			function submitform()
		...
		if($conne==4){ 
		/*echo "<script>window.location='http://www.colegiointeractivo.com'</script>";*/
		$conne=pg_connect("dbname=coi_corporaciones host=192.168.1.12 port=5432 user=postgres password=f4g5h6.j") or die ("Error de conexion Corporacion.");	
		}
				
				
				//ir a buscar DV alumno
				$sql = "select dig_rut from alumno where rut_alumno = ".$_NOMBREUSUARIO;
				$rs = pg_exec($conne,$sql);
				$fi = @pg_fetch_array($rs,0);	
			...
			?>
            <form id="edu" name="myform" method="post" action="https://app.edugestor.com/ws/passthrough.php">
              <input name="rut2" id="rut2" type="hidden" value="<?php echo $rut ?>" />
              <input name="rbd_colegio" id="rbd_colegio" type="hidden"  value="<?php echo $_INSTIT ?>" />
            <input name="password" id="password" type="hidden"  value="4*G,#Ueyh.Nydsg+" />
</form>
<script type="text/javascript">
		...
		if($_PERFIL==64){	// ADMIN BIBLIOTECA
		...
		if($base ==1){
		$conne=pg_connect("dbname=coi_final host=ip-172-31-0-119.ec2.internal port=5432 user=postgres password=f4g5h6.j") or die ("Error 
		de conexion Coi_final");	
		
		}
		
		if($base==2){ 
		$conne=pg_connect("dbname=coi_final_vina host=ip-172-31-11-223.ec2.internal port=5432 user=postgres password=f4g5h6.j") or die ("Error de conexion Coi_final_Vi�a");	
		...
		if($conne==4){ 
			$conne=pg_connect("dbname=coi_corporaciones host=ip-172-31-11-223.ec2.internal port=5432 user=postgres password=f4g5h6.j") or die ("Error de conexion Corporacion.");	
		}
		
		$sql_am = "select * from ano_escolar aes where aes.id_institucion = ".$_INSTIT." and aes.situacion = 1";
		$rs_am = pg_exec($conne,$sql_am);
		$f_am = pg_fetch_array($rs_am,0);
	...
	/*echo  "<HTML><HEAD><script>window.location = '../biblioteca/resumen/resumen.php'</script></HEAD><BODY></BODY></HTML>";*/
	echo  "<HTML><HEAD><script>window.location = '../biblioteca/dashboard/dashboard.php'</script></HEAD><BODY></BODY></HTML>";
	
			   
			};//FIN PERFIL 
			
			
		if($_PERFIL==75){	//POSTVENTA
			echo "<script>window.location = '../postventa/index.php'</script>";
			exit;
		};//FIN PERFIL 
