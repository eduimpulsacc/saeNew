		    ...
		    if ($perfil != 0){    
				...
				if (!$resulte) {
					...
					if (pg_numrows($resulte)!=0){
						...
						$_USUARIOENSESION=trim($fila200['nombre_emp'])." ".trim($fila200['ape_pat']);
				 ...
				 $qryap="SELECT * FROM APODERADO WHERE RUT_APO = ".$fila100['nombre_usuario']." ";
				 
				
				
				$resultap =@pg_Exec($conn,$qryap);
				
				if (!$resultap) {
					//error('<B> ERROR :</b>Error al acceder a la BD. (3)</B>');
				}else{
					if (pg_numrows($resultap)!=0){
						$fila300 = @pg_fetch_array($resultap,0);	
						if (!$fila300){
							error('<B> ERROR :</b>Error al acceder a la BD. (4)</B>');
							//exit();
						}
						...
						//echo trim($fila2['nombre_emp']." ".$fila2['ape_pat']." ".$fila2['ape_mat']);
						$_USUARIOENSESION=$fila300['nombre_apo'];
						$_USUARIOENSESION.=$fila300['ape_pat'];
						session_register('_USUARIOENSESION');	
						$usuarioensesion = $_USUARIOENSESION;		
...
?>	
<link rel="stylesheet" type="text/css" href="<?=$d;?>cortes/style_top.css"/>
...
<script type="text/javascript" src="<?=$d;?>cortes/functions.js"></script>
...
<!--  <a href="http://www.simce.cl" target="_blank"><img src="<? //echo $d; ?>cabecera/Banner.jpg" width="328" height="136" border="0" /></a>	   --><div class="slide">
  <figure>    <img class="img4" alt="alt img1" src="<?=$d;?>cortes/foto_top_new66.png" /> 
	          <img class="img2" alt="alt img2" src="<?=$d;?>cortes/foto_top_new33.png" /> 
	          <img class="img3" alt="alt img3" src="<?=$d;?>cortes/foto_top_new77.png" />
              <img class="img1" alt="alt img4" src="<?=$d;?>cortes/foto_top_new11.png" />   
  </figure>
	</div>
<!--<img src="<?=$d;?>Telefono_actualizado_SAE_2.png" width="272"  height="87" />--></td>
						  </tr>
					 </table>		  
				...
				<?	}	?>
				  ...
				  if (($_PERFIL == 17) OR ($_PERFIL == 2) OR ($_PERFIL == 19)OR ($_PERFIL == 33) OR ($_PERFIL == 35) OR ($_PERFIL == 22) OR ($_PERFIL == 22)OR ($_PERFIL == 22)OR ($_PERFIL == 32)OR ($_PERFIL == 31)OR ($_PERFIL == 30)OR ($_PERFIL == 29)OR ($_PERFIL == 28)OR ($_PERFIL == 27)OR ($_PERFIL == 26)OR ($_PERFIL == 25)OR ($_PERFIL == 21)OR ($_PERFIL == 9)OR ($_PERFIL == 8)OR ($_PERFIL == 6)OR ($_PERFIL == 22)OR ($_PERFIL == 14) OR ($_PERFIL == 20) OR ($_PERFIL == 21) OR ($_PERFIL == 1) OR ($_PERFIL == 6) OR ($_PERFIL == 31) OR ($_PERFIL == 25) OR ($_PERFIL == 26) OR ($_PERFIL == 28)  OR ($_PERFIL == 27)OR ($_PERFIL == 53) OR ($_PERFIL == 44) OR ($_PERFIL == 41) OR ($_PERFIL == 47) OR ($_PERFIL == 49) OR ($_PERFIL == 24)OR ($_PERFIL == 54) OR ($_PERFIL == 51)OR ($_PERFIL == 52)OR ($_PERFIL == 76) OR ($_PERFIL == 55) OR ($_PERFIL == 66) OR ($_PERFIL == 68) OR ($_PERFIL == 72)){ 
					    ?>					
	...
	<span class="textosesion">
		<a href="<? echo $d; ?>admin/institucion/empleado/empleado.php3?usr=1">Mis Datos Personales</a></span> - <span class="textosesion">
						 ...
						 <a href="<? echo $d; ?>cabecera/Manual_Administrador_SAE.pdf">Manual SAE</a>
						 </span> 
                         <br/>
                         <span class="textosesion">
...
<td>
    ...
    echo '<div id="System_Help"></div>';
		...
		?>
                       ...
                       <tr>
                         <td height="22" class="textosesion">
                         <?php if($_PERFIL==16){ 
							
							   $qralu="SELECT * FROM ALUMNO WHERE RUT_ALUMNO = ".$_NOMBREUSUARIO." ";
			
				$resultalu =@pg_Exec($conn,$qralu);
				
				if (!$resultalu) {
					//error('<B> ERROR :</b>Error al acceder a la BD. (3)</B>');
				}else{
					if (pg_numrows($resultalu)!=0){
						$fila500 = @pg_fetch_array($resultalu,0);	
						if (!$fila500){
							error('<B> ERROR :</b>Error al acceder a la BD. (4)</B>');
							//exit();
						}
						...
						//echo trim($fila2['nombre_emp']." ".$fila2['ape_pat']." ".$fila2['ape_mat']);
						$_USUARIOENSESION=$fila500['nombre_alu'];
						$_USUARIOENSESION.=$fila500['ape_pat'];
						session_register('_USUARIOENSESION');	
						$usuarioensesion = $_USUARIOENSESION;		
                             ...
                             <?php if($_PERFIL==15){ 
							   ...
							   $qralu="SELECT * FROM APODERADO WHERE RUT_APO = ".$_NOMBREUSUARIO." ";
				...
				if (!$resultalu) {
					...
					if (pg_numrows($resultalu)!=0){
						...
						//echo trim($fila2['nombre_emp']." ".$fila2['ape_pat']." ".$fila2['ape_mat']);
						$_USUARIOENSESION=$fila500['nombre_apo'];
						$_USUARIOENSESION.=$fila500['ape_pat'];
						session_register('_USUARIOENSESION');	
						$usuarioensesion = $_USUARIOENSESION;		
                         ...
                         Iniciado por: <?=$usuarioensesion ?></td>
                       </tr>
                     </table>
				<? } ?>					 
					  <?php if($_PERFIL==0){?>
