...
echo "num-->".$num_2;
    if ($num_2 > 0){
		...
		echo "<br>".$sql_trab = "insert into trabaja (rdb, rut_emp, cargo) values ('".trim($rdb)."','".trim($rut)."','5')";
		 ...
		 */
		  ...
		  echo "<br>".$sql_ins = "insert into empleado (rut_emp, dig_rut, nombre_emp, ape_pat, ape_mat,sexo,region,ciudad,comuna,email,calle,telefono ) values ('".trim($rut)."','$dig','$nombre','$ape_pat','$ape_mat','$sexo','$region','$ciudad','$comuna','$email','$calle','$fono')";
		 ...
		 echo  $sql_ins2 = "insert into trabaja (rdb, rut_emp, cargo) values ('".trim($rdb)."','".trim($rut)."','5')"; 
...
echo "<br><br>ok...actualizacion ESCUELA SAINT MARY SCHOOL.... ".$rdb;  	           
?> 
