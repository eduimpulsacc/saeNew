...
//$sql ="SELECT rut_alumno, dig_rut, nombre_alu, ape_pat, ape_mat FROM alumno";
 $sql =" SELECT rut_apo, dig_rut, nombre_apo, ape_pat, ape_mat FROM apoderado";
...
for($i=0;$i<@pg_numrows($rs_final);$i++){
	$fila = @pg_fetch_array($rs_final,$i);
	 ...
	 $sql ="SELECT count(*) as contador FROM dato_usuario WHERE rut_usuario=".$fila['rut_apo'];
	$rs_existe = @pg_exec($conn,$sql);
	$contador = pg_result($rs_existe,0);
	
	if($contador==0){	
		$sql ="INSERT INTO dato_usuario (rut_usuario, dig_rut, nombre, ape_pat, ape_mat) VALUES (".$fila['rut_apo'].",'".$fila['dig_rut']."','".$fila['nombre_apo']."','".$fila['ape_pat']."','".$fila['ape_mat']."')";
