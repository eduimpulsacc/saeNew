...
<table width="650" border="0" align="center">
  <tr>
    <td class="cuadro02">PLANIFICACION MENSUAL</td>
  ...
  <tr>
    <td colspan="4"><input type="hidden" name="id_mensual" id="id_mensual" value="<?php echo $fila_unidad['id_mensual'] ?>" /><input type="hidden" name="id_anual" id="id_anual" value="<?php echo $fila_unidad['id_anual'] ?>" /></td>
  </tr>
  <tr>
    <td colspan="4" align="right"> <div id="nvo">
    <input type="button" name="nuevaUnidad" id="nuevaUnidad" value="Nueva Clase" onclick="creaClase()" class="botonXX" />&nbsp;&nbsp;&nbsp; <input type="button" name="VOLVER" id="VOLVER" value="VOLVER" onclick="location.href='../plani_anual/plani_anual.php'" class="botonXX" />
    </div></td>
  </tr>
  <tr>
  ...
  <tr>
		...
		<tr>
		  <td class="cuadro01"><?php echo CambioFD($fila_unidad['fecha_inicio']) ?><input name="ffi" id="ffi" type="hidden" value="<?php echo CambioFD($fila_unidad['fecha_inicio']) ?>" /></td>
		  <td class="cuadro01"><?php echo CambioFD($fila_unidad['fecha_termino']) ?><input name="fft" id="fft" type="hidden" value="<?php echo CambioFD($fila_unidad['fecha_termino']) ?>"/></td>
...
<table width="650" border="0" align="center" cellspacing="3">
 <tr class="cuadro02">
   <td colspan="4" align="center">    INFORMACION CLASE </td>
  </tr>
		<tr>
		  <td colspan="4" >&nbsp;</td>
  </tr>
...
<table width="650" border="0" align="center" cellspacing="3">
 <tr>
   <td class="cuadro02">N&uacute;mero clase</td>
   <td class="cuadro02">Horas asignadas</td>
   <td class="cuadro02">Fecha Inicio</td>
   <td class="cuadro02">Fecha T&eacute;rmino</td>
 </tr>
 <tr>
   <td class="cuadro01"><input type="text" name="numclase" id="numclase"  class="solo-numero"/></td>
   <td class="cuadro01"><input type="text" name="cantoras" id="cantoras"  class="solo-numero" /></td>
   <td class="cuadro01"><input type="text" name="f_inicio" id="f_inicio" onchange="vfi(this.value)" /></td>
   <td class="cuadro01"><input type="text" name="f_termino" id="f_termino" onchange="vft(this.value)" /></td>
 </tr>
 <tr>
   <td colspan="4" >&nbsp;</td>
 </tr>
 <tr>
   <td colspan="4" class="cuadro02">Actividades</td>
 </tr>
 <tr>
   <td colspan="4" class="cuadro01"><textarea name="txt_actividades" cols="80" rows="5" id="txt_actividades"></textarea></td>
 </tr>
</table><br />
<table width="650" border="0" align="center">
 <tr>
...
if($funcion==4){
	//show($_POST);
	$rs_objetivo=$ob_clase->traeObjUnidad($conn,$tipo,$unidad);
	?>
...
//var_dump($_POST);	
	...
	$id_semanal=$id_mensual;
	$fini = CambioFE($f_inicio);
	$fter = CambioFE($f_termino);
	
$rs_guarda = $ob_clase->guardaClase($conn,$id_anual,$id_mensual,$id_semanal,$txt_actividades,$numclase,$cantoras,$fini,$fter);

if($rs_guarda){
	 
$rs_ultimo = $ob_clase->ultimaClase($conn,$iunidad);
$clase = pg_result($rs_ultimo,3);
...
<table width="650" border="1" align="center" style="border-collapse:collapse">
  <tr>
    <td colspan="13" align="center" class="cuadro02">Listado </td>
  </tr>
  <tr>
   
    <td class="cuadro02">Fecha Inicio</td>
    <td class="cuadro02">Fecha T&eacute;rmino</td>
    <td class="cuadro02">Clase N&ordm;</td>
  ...
  <tr>
    ...
    <td class="cuadro01"><?php echo $fila['numclase'] ?></td>
    ...
    <td class="cuadro01"><input type="button" name="rl" id="rl" value="<?php echo ($fila['realizada']==1)?"NRL":"RL" ?>" title="Marcar Clase como <?php echo ($fila['realizada']==1)?"NO realizada":"Realizada" ?>" <?php echo ($fila['estado']!=2)?"disabled":"" ?> onclick="claserl(<?php echo $fila['id_clase'] ?>,<?php echo ($fila['realizada']==1)?0:1 ?>)" class="<?php echo ($fila['estado']!=2)?"botonXXI":"botonXX" ?>" /></td>
    <?php }?>
...
<table width="650" border="0" align="center">
 ...
 <tr>
		  ...
		  <td width="152" class="cuadro02">NUMERO CLASE</td>
		...
		<tr>
		  ...
		  <td class="cuadro01"><?php echo CambioFD($fila['fecha_termino']) ?></td>
		  <td class="cuadro01"><?php echo $fila['numclase'] ?></td>
		  <td class="cuadro01"><?php echo $fila['cantoras'] ?></td>
	  </tr>
		<tr>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
...
<table width="650" border="0" align="center" cellspacing="3">
  ...
  <tr>
   <td class="cuadro02">Actividades</td>
 </tr>
 <tr>
   <td class="cuadro01"><?php echo nl2br($fila['actividades']) ?></td>
 </tr>
...
if($funcion==8){
	...
	$rs_tipo=$ob_clase->tejeUnidad($conn,$fila['id_anual']);
...
<tr>
 ...
 <tr>
   <td class="cuadro01"><input type="text" name="numclase" id="numclase" value="<?php echo $fila['numclase'] ?>" onchange="valclase(this.value)" /></td>
   <td class="cuadro01"><input type="text" name="cantoras" id="cantoras" value="<?php echo $fila['cantoras'] ?>" /></td>
   <td class="cuadro01"><input type="text" name="f_inicio" id="f_inicio" value="<?php echo CambioFD($fila['fecha_inicio']) ?>" onchange="vfi(this.value)"  /></td>
   <td class="cuadro01"><input type="text" name="f_termino" id="f_termino" value="<?php echo CambioFD($fila['fecha_termino']) ?>" onchange="vft(this.value)" /></td>
 </tr>
 <tr>
   <td colspan="4" >&nbsp;</td>
 </tr>
 <tr>
   <td class="cuadro02" colspan="4">Actividades</td>
 </tr>
 <tr>
   <td class="cuadro01" colspan="4"><textarea name="txt_actividades" cols="80" rows="5" id="txt_actividades"><?php echo $fila['actividades'] ?></textarea></td>
 </tr>
</table><br />
...
<input name="tipo" id="tipo1" type="radio" value="1" onclick="cargatipoedi(1,<?php echo $fila['id_unidad']?>); " />
    ...
    <input type="hidden" name="cargahab" id="cargahab"  value="<?php echo $cab ?>" />-->
	 ...
	 for($ti=0;$ti<pg_numrows($rs_tipo);$ti++){
      ...
      <input name="tipo" id="tipo<?php echo $t ?>" type="radio" value="<?php echo $fila_tipo['id_objetivo']; ?>" onclick="cargatipo(<?php echo $fila_tipo['id_objetivo']; ?>,<?php echo $fila['id_anual']?>);" <?php echo ($ti==0)?"checked":"" ?> /><?php echo $fila_tipo['nombre']; ?>
      <input type="hidden" name="cargaind[]" id="cargaind<?php echo $fila_tipo['id_objetivo']; ?>" value="<?php echo $cad_ind ?>" />
...
<script>
cargatipo(<?php echo  $crg?>,<?php echo $fila['id_anual']?>);
...
if($funcion==10){
 ...
 $rs_guarda = $ob_clase->modificaClase($conn,$numclase,$cantoras,CambioFE($f_inicio),CambioFE($f_termino),utf8_decode($txt_actividades),$id_clase);

if($rs_guarda){
	$ob_clase->delIndicador($conn,$id_clase); 
...
$rs_guarda = $ob_clase->guardaClase($conn,$destino,$curso,$ramo_destino,$doc_destino,$txt_nombre,$cmb_tipo,$f_inicio,$f_termino,$txt_evaluacion,$txt_inicio,$txt_desarrollo,$txt_cierre,$txt_actitudes,$cant_clases,$cant_horas,$fini,$fter);
