...
inner join planificacion.plani_anual_new_obj ob on ob.id_obj_hab = oh.id_obj
 where oh.tipo=$tipo and ob.id_semana=$idUnidad order by oh.id_obj";
 $result = pg_exec($conn,$sql);
	return $result;	
}	

public function guardaClase($conn,$id_anual,$id_mensual,$id_semanal,$actividades,$numclase,$cantoras,$fecha_inicio,$fecha_termino){
	
	// $sql="insert into planificacion.plani_semanal(id_unidad, id_curso, id_ramo, rut_emp, nombre, tipo, fecha_inicio, fecha_termino, evaluacion, inicio, desarrollo, cierre, actitudes,cant_clase,cant_hora) values($iunidad,$icurso,$iramo,$cmbDocente,'".utf8_decode($txt_nombre)."',$cmb_tipo,'$f_inicio','$f_termino','".utf8_decode($txt_evaluacion)."','".utf8_decode($txt_inicio)."','".utf8_decode($txt_desarrollo)."','".utf8_decode($txt_cierre)."','".utf8_decode($txt_actitudes)."',$cant_clases,$cant_horas)";
	 $sql="insert into planificacion.plani_semanal (id_anual,id_mensual,id_semana,actividades,estado,realizada,numclase,cantoras,fecha_inicio,fecha_termino) values($id_anual,$id_mensual,$id_semanal,'$actividades',1,0,$numclase,$cantoras,'$fecha_inicio','$fecha_termino')";
...
public function ultimaClase($conn,$unidad){
  $sql="select * from planificacion.plani_semanal where id_anual=$unidad order by id_clase desc limit 1";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function guardaObjetivo($conn,$clase,$id_obj){
 $sql="insert into planificacion.plani_semanal_obj values($clase,$id_obj);";
$result = pg_exec($conn,$sql);
return $result;
...
public function traeClases($conn,$unidad){
  $sql="select * from planificacion.plani_semanal where id_anual=$unidad and visible=1 order by id_clase";
$result = pg_exec($conn,$sql);
return $result;
}

public function traeClaseUno($conn,$clase){
  $sql="select * from planificacion.plani_semanal where id_clase=$clase";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function traeObjclase($conn,$tipo,$clase){
       $sql="select oh.* from planificacion.obj_hab oh inner join planificacion.plani_semanal_obj ob  on ob.id_obj = oh.id_obj
...
public function modificaClase($conn,$numclase,$cantoras,$f_inicio,$f_termino,$actividades,$id_clase){

  $sql="update planificacion.plani_semanal set fecha_inicio ='$f_inicio',fecha_termino='$f_termino',numclase=$numclase,cantoras=$cantoras,actividades='$actividades' where id_clase=$id_clase";
...
public function traeHistorialcambios($conn,$clase){
 $sql="select * from planificacion.plani_semanal_observacion where id_plani_semana=$clase order by fecha,id_obervacion asc";
$result = pg_exec($conn,$sql);
	return $result;
}

public function guardaHistorialcambios($conn,$clase,$fecha,$observacion){
  $sql="insert into planificacion.plani_semanal_observacion (id_plani_semana,fecha,observacion) values($clase,'$fecha','".utf8_decode($observacion)."')";
$result = pg_exec($conn,$sql);
	return $result;
}

public function cambiaEstadoClase($conn,$clase,$estado){
   $sql="update planificacion.plani_semanal set estado=$estado where id_clase=$clase";
$result = pg_exec($conn,$sql);
	return $result;
}

public function cambiaRealizada($conn,$clase,$estado){
    $sql="update  planificacion.plani_semanal set realizada=$estado where id_clase=$clase";
$result = pg_exec($conn,$sql);
	return $result;
}
...
left join planificacion.plani_anual_new_obj ob on ob.id_obj_hab = oh.id_obj 
where ob.id_semana=$unidad order by tob.id_objetivo
";
$result = pg_exec($conn,$sql);
return $result;	
	}
...
from planificacion.tipo_objetivo tob 
inner join planificacion.obj_hab oh on oh.tipo =tob.id_objetivo 
left join planificacion.plani_semanal_obj ob on ob.id_obj = oh.id_obj 
where ob.id_clase=$clase order by tob.id_objetivo";	
$result = pg_exec($conn,$sql);
return $result;
	}
...
public function delObjClase($conn,$clase){
    $sql="delete from planificacion.plani_semanal_obj where id_clase=$clase";
$result = pg_exec($conn,$sql);
return $result;	
}
public function delIndicador($conn,$clase){
    $sql="delete from planificacion.plani_semanal_ind where id_clase=$clase";
$result = pg_exec($conn,$sql);
return $result;	
}
public function delClase($conn,$clase){
  $sql="delete from planificacion.plani_semanal where id_clase=$clase ";
  //$sql="update planificacion.plani_semanal set visible=0 where id_clase=$clase";
$result = pg_exec($conn,$sql);
return $result;	
}
...
inner join planificacion.plani_semanal_obj ob on ob.id_obj = oh.id_obj
...
public function buscaIndicadorSel($conn,$id_obj,$uano){
 ...
 on inde.id_indicador = indic.id_indicador where inde.id_obj =$id_obj
  and inde.id_semana=$uano";
$result = pg_exec($conn,$sql);
return $result;	
}

public function guardaIndicadorSel($conn,$clase,$iobj,$indicador){
 $sql="insert into planificacion.plani_semanal_ind values($clase,$iobj,$indicador)";
$result = pg_exec($conn,$sql);
return $result;	
}
...
inner join planificacion.plani_semanal_obj oh on oh.id_obj = inde.id_obj
