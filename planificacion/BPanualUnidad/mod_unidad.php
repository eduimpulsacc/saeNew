...
public function traeCursosN($conn,$ano){
	 $sql="SELECT distinct(grado_curso) as id_curso,ensenanza,grado_curso ||'-'||  nombre_tipo AS curso, ensenanza FROM curso INNER JOIN tipo_ensenanza ON curso.ensenanza=tipo_ensenanza.cod_tipo WHERE id_ano=".$ano."  ORDER BY ensenanza,curso ASC";
		$result = pg_exec($conn,$sql);
...
public function traeRamoN($conn,$curso,$ramo=''){
$sql ="select distinct(s.cod_subsector),s.nombre
 ...
 $qry.=" order by ramo s.cod_subsector.id_orden";	
...
public function guardaUnidad($conn,$rdb,$ano,$sel_curso,$sel_ramo,$docdicta,$txt_fechaini,$txt_fechater,$txt_semana,$texto,$txt_nombre,$tipu,$mes,$txt_horas){
	...
	$txt_semana=($txt_semana=="")?0:$txt_semana;
	  ...
	  $sql="insert into planificacion.bp_unidad_anio(rdb,id_ano,id_curso,id_ramo,rut_emp,fecha_inicio,fecha_termino,cant_semanas,texto,nombre,tipo,num_mes,nro_horas,estado) values ($rdb,$ano,$sel_curso,$sel_ramo,$docdicta,'$txt_fechaini','$txt_fechater',$txt_semana,'$texto','$txt_nombre',$tipu,$mes,$txt_horas,1)";
	$result = pg_exec($conn,$sql);
...
public function ultimaUnidad($conn,$rdb){
  	 $sql="select id_unidad from planificacion.bp_unidad_anio where rdb=$rdb order by id_unidad desc limit 1";
	$result = pg_exec($conn,$sql);
...
public function guardaObjetivo($conn,$id_unidad,$id_obj){
    $sql="insert into planificacion.bp_unidad_anio_obj values($id_unidad,$id_obj);";
	$result = pg_exec($conn,$sql);
	return $result;
...
public function traeObjUnidad($conn,$tipo,$idUnidad){
      $sql="select oh.* from planificacion.obj_hab oh inner join planificacion.bp_unidad_anio_obj ob  on ob.id_obj = oh.id_obj where oh.tipo=$tipo and ob.id_unidad=$idUnidad order by oh.codigo";
 $result = pg_exec($conn,$sql);
	return $result;	
}	
...
public function actualizaUnidad($conn,$idUnidad,$txt_fechaini,$txt_fechater,$txt_semana,$txt_horas,$texto,$txt_nombre){
    ...
    $sql= "update planificacion.bp_unidad_anio set fecha_inicio='$txt_fechaini',fecha_termino='$txt_fechater',cant_semanas=$txt_semana,nro_horas=$txt_horas,texto='$texto',nombre='$txt_nombre',estado=3 where id_unidad=$idUnidad";
...
$sql="delete from planificacion.bp_unidad_anio_obj where id_unidad=$idUnidad";
$result = pg_exec($conn,$sql);
	return $result;	
...
public function traeMarcado($conn,$idUnidad,$id_obj){
    $sql="select id_obj as marcado from planificacion.bp_unidad_anio_obj where id_unidad=$idUnidad and id_obj=$id_obj";
 $result = pg_exec($conn,$sql);
	return $result;	
}

public function existe($conn,$fecha_inicio,$fecha_termino,$id_curso,$id_ramo){
    $sql="select * from planificacion.bp_unidad_anio where id_curso=$id_curso and id_ramo=$id_ramo and fecha_inicio>='$fecha_inicio' AND fecha_termino<='$fecha_termino' ";
 $result = pg_exec($conn,$sql);
	return $result;	
}
...
public function existeT2($conn,$id_curso,$id_ramo,$tipo){
    $sql="select * from planificacion.bp_unidad_anio where id_curso=$id_curso and id_ramo=$id_ramo and tipo=$tipo ";
 $result = pg_exec($conn,$sql);
	return $result;	
}
...
public function guardaUnidadT2($conn,$rdb,$ano,$curso,$id_ramo,$dicta,$txt_fechaini,$txt_fechater,$txt_nombre,$tipu,$grd,$ens,$cod_ramo){
	
	
	 // echo "<br>". 
	     $sql="insert into planificacion.bp_unidad_anio (rdb,id_ano,id_curso,id_ramo,rut_emp,fecha_inicio,fecha_termino,nombre,tipo,estado,grado_curso,ensenanza,cod_ramo) values ($rdb,$ano,$curso,$id_ramo,$dicta,'$txt_fechaini','$txt_fechater','$txt_nombre',$tipu,1,$grd,$ens,$cod_ramo)";
	   $result = pg_exec($conn,$sql);
...
public function ramosDctN($conn,$ano,$ens,$grd){
  $qry ="select distinct(s.cod_subsector),s.nombre
	from subsector s
	inner join ramo r on r.cod_subsector = s.cod_subsector
	inner join curso c on c.id_curso = r.id_curso
	where c.id_ano=$ano and c.ensenanza=$ens and c.grado_curso=$grd
	order by s.cod_subsector;";
$result = pg_exec($conn,$qry);
...
public function traeHistorialcambiosUAnual($conn,$unidad){
   $sql="select * from planificacion.bp_unidad_anio_observacion where id_unidad=$unidad order by fecha,id_observacion asc";
$result = pg_exec($conn,$sql);
	return $result;
}

public function guardaHistorialcambiosUAnual($conn,$unidad,$fecha,$observacion){
    $sql="insert into planificacion.bp_unidad_anio_observacion (id_unidad,fecha,observacion) values($unidad,'$fecha','".utf8_decode($observacion)."')";
$result = pg_exec($conn,$sql);
	return $result;
}

public function cambiaEstadoClaseUAnual($conn,$unidad,$estado){
    $sql="update planificacion.bp_unidad_anio set estado=$estado where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
	return $result;
}
...
public function cambiaRealizada($conn,$unidad,$estado){
   $sql="update planificacion.bp_unidad_anio set ejecutada=$estado where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
	return $result;
}

public function traearchivo($conn,$unidad){
    $sql="select * from planificacion.bp_unidad_anio_archivo where id_unidad=$unidad and visible=1";
$result = pg_exec($conn,$sql);
return $result;	
}

public function pisaArchivos($conn,$unidad){
    $sql="update planificacion.bp_unidad_anio_archivo set visible=0 where id_unidad=$unidad and visible=1";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function archivoUanual($conn,$unidad){
   $sql="select * from planificacion.bp_unidad_anio_archivo where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function delUnidad($conn,$unidad){
   $sql="delete from planificacion.bp_unidad_anio where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function guardaIndicador($conn,$unidad,$iobj,$indicador){
   $sql="insert into planificacion.bp_unidad_anio_indicador values($unidad,$iobj,$indicador)";
$result = pg_exec($conn,$sql);
return $result;	
}
...
$sql="delete from planificacion.bp_unidad_anio_indicador where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}
...
inner join planificacion.bp_unidad_anio_obj ob on ob.id_obj = oh.id_obj
