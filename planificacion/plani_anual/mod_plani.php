...
public function traeSupervisa($conn,$curso){
   $qry = "select
d.rut_emp,
d.id_curso,
e.nombre_emp||' '|| e.ape_pat||' '|| e.ape_mat as nombre
...
public function ultimaUnidad($conn,$rdb){
 	 $sql="select id_planificacion from planificacion.plani_anual_new where rdb=$rdb order by id_planificacion desc limit 1";
	$result = pg_exec($conn,$sql);
...
public function guardaObjetivo($conn,$id_unidad,$id_obj){
   $sql="insert into planificacion.plani_anual_new_obj values($id_unidad,$id_obj);";
	$result = pg_exec($conn,$sql);
	return $result;
...
public function listaUnidad($conn,$id_ano,$rdb,$curso,$ramo,$docente){
	 $sql="select planificacion.plani_anual_new.* from planificacion.plani_anual_new inner join curso 
	on planificacion.plani_anual_new.id_curso = curso.id_curso
	where rdb=$rdb";
	
	
	if($id_ano>0){
	$sql.=" and planificacion.plani_anual_new.id_ano=$id_ano";
	}
	
	if($curso>0){
	$sql.=" and planificacion.plani_anual_new.id_curso=$curso";
	}
	...
	$sql.=" order by curso.ensenanza,curso.grado_curso,curso.letra_curso,planificacion.plani_anual_new.id_planificacion ";
	//echo $sql;
	$result = pg_exec($conn,$sql);
...
public function traeUnidad($conn,$idUnidad){
 //$sql="select * from planificacion.plani_anual_new where id_planificacion=$idUnidad";
 $sql="select ai.*,am.*,ad.*
from planificacion.plani_anual_new ai
inner join planificacion.plani_new_mensual am
on am.id_anual = ai.id_planificacion
inner join planificacion.plani_anual_new_det ad
on ad.id_pmensual = am.id_mensual
where ai.id_planificacion =$idUnidad";
echo $sql;
	$result = pg_exec($conn,$sql);
...
inner join planificacion.plani_anual_new_obj ob on ob.id_obj_hab = oh.id_obj
 where oh.tipo=$tipo and ob.id_semana=$idUnidad order by oh.codigo";
 $result = pg_exec($conn,$sql);
	return $result;	
}	

public function actualizaUnidad($conn,$idUnidad,$fecha_inicio,$fecha_termino,$tipo,$ambito){
	//$txt_semana=($txt_semana=="")?0:$txt_semana;
   ...
   $sql="update planificacion.plani_anual_new set fecha_inicio='$fecha_inicio',fecha_termino='$fecha_termino',tipo='$tipo',ambito='$ambito' where id_planificacion=$idUnidad";
...
public function traeMarcado($conn,$idUnidad,$id_obj){
   $sql="select id_obj_hab as marcado from planificacion.plani_anual_new_obj where id_semana=$idUnidad and id_obj_hab=$id_obj";
 $result = pg_exec($conn,$sql);
	return $result;	
}

public function existe($conn,$fecha_inicio,$fecha_termino,$id_curso,$id_ramo){
     $sql="select * from planificacion.plani_anual_new where id_curso=$id_curso and id_ramo=$id_ramo and fecha_inicio>='$fecha_inicio' AND fecha_termino<='$fecha_termino' ";
 $result = pg_exec($conn,$sql);
	return $result;	
}
...
public function traeHistorialcambiosUAnual($conn,$unidad){
   $sql="select * from planificacion.plani_anual_new_observacion where id_planificacion=$unidad order by fecha,id_observacion asc";
$result = pg_exec($conn,$sql);
	return $result;
}

public function guardaHistorialcambiosUAnual($conn,$unidad,$fecha,$observacion){
    $sql="insert into planificacion.plani_anual_new_observacion (id_planificacion,fecha,observacion) values($unidad,'$fecha','".utf8_decode($observacion)."')";
$result = pg_exec($conn,$sql);
	return $result;
}
...
public function traearchivo($conn,$unidad){
     $sql="select * from planificacion.plani_anual_new_archivo where id_planificacion=$unidad and visible=1";
$result = pg_exec($conn,$sql);
return $result;	
}

public function pisaArchivos($conn,$unidad){
    $sql="update planificacion.plani_anual_new_archivo set visible=0 where id_planificacion=$unidad and visible=1";
$result = pg_exec($conn,$sql);
return $result;	
}

public function guardaArchivos($conn,$unidad,$ruta){
    $sql="insert into planificacion.plani_anual_new_archivo(id_planificacion,ruta,visible) values ($unidad,'$ruta',1)";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function tejeUnidad($conn,$unidad){
	 $sql="
select DISTINCT(tob.id_objetivo),tob.nombre from planificacion.tipo_objetivo tob 
inner join planificacion.obj_hab oh on oh.tipo =tob.id_objetivo
 left join planificacion.plani_anual_new_obj ob on ob.id_obj_hab = oh.id_obj where ob.id_semana=$unidad order by tob.id_objetivo
";
$result = pg_exec($conn,$sql);
return $result;	
	}
...
public function archivoUnidadU($conn,$unidad){
   $sql="select * from planificacion.plani_anual_new_archivo where id_planificacion=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}

public function delObsUnidadU($conn,$unidad){
   $sql="delete from planificacion.plani_anual_new_observacion where id_planificacion=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function delObjUnidadU($conn,$unidad){
   $sql="delete from planificacion.plani_anual_new_obj where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}

public function delUnidadU($conn,$unidad){
   $sql="delete from planificacion.plani_anual_new where id_planificacion=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}
...
inner join planificacion.plani_anual_new_indicador inde
...
public function guardaIndicador($conn,$unidad,$iobj,$indicador){
   $sql="insert into planificacion.plani_anual_new_indicador values($unidad,$iobj,$indicador)";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function tipoEjesBloqueAnio($conn,$unidad,$tipo){
	      $sql="select DISTINCT(tej.id_eje),tej.texto from planificacion.ejes tej 
inner join planificacion.obj_hab oh on oh.id_eje = tej.id_eje 
inner join planificacion.plani_anual_new_obj ob on ob.id_obj_hab = oh.id_obj 
where ob.id_semana=$unidad and tej.tipo=$tipo order by tej.texto";
$result =pg_exec($conn,$sql);
...
public function traeObjeUnidadAnio($conn,$eje,$tipo,$unidad){
  $sql="select oh.* from planificacion.obj_hab oh 
inner join planificacion.plani_anual_new_obj ob on ob.id_obj_hab = oh.id_obj 
where oh.tipo=$tipo and id_eje =$eje and ob.id_semana=$unidad order by oh.id_obj";
 $result = pg_exec($conn,$sql);
	return $result;	
}	
...
on inde.id_indicador = indic.id_indicador 
inner join planificacion.obj_hab oh on oh.id_obj = inde.id_obj
 where inde.id_semana = $unidad and oh.tipo=$tipo";
$result = pg_exec($conn,$sql);
	return $result;
}
...
on inde.id_indicador = indic.id_indicador 
inner join planificacion.obj_hab oh on oh.id_obj = inde.id_obj 
where inde.id_semana = $unidad and oh.id_obj=$obj ";	
$result = pg_exec($conn,$sql);
	return $result;
}
...
public function guardaUnidadAno($conn,$rdb,$ano,$sel_curso,$sel_ramo,$docdicta,$fecha_inicio,$fecha_termino,$txt_nombre,$tipo,$ambito){
	   $sql="insert into planificacion.plani_anual_new(rdb,id_ano,id_curso,id_ramo,rut_emp,fecha_inicio,fecha_termino,nombre,tipo,ambito) values($rdb,$ano,$sel_curso,$sel_ramo,$docdicta,'$fecha_inicio','$fecha_termino','$txt_nombre','$tipo','$ambito')";
	$result = pg_exec($conn,$sql);
		
		return $result;	
	}

public function numUnidad($conn){
$sql="select * from planificacion.num_unidad order by id_unidad";
$result = pg_exec($conn,$sql);
...
public function insPlaniMensual($conn,$uanual,$cant_semanas,$cant_horas,$didactico,$evaluacion,$nombre){
   $sql="insert into planificacion.plani_new_mensual (id_anual,cant_semanas,cant_horas,didactico,evaluacion,nombre) values($uanual,$cant_semanas,$cant_horas,'$didactico','$evaluacion','$nombre')";
$result = pg_exec($conn,$sql);
return $result;
...
public function ultimaUnidadMenusal($conn,$rdb){
 	 $sql="select id_mensual from planificacion.plani_new_mensual where id_anual in ( select id_planificacion from planificacion.plani_anual_new where rdb=$rdb order by id_planificacion desc limit 1)";
	$result = pg_exec($conn,$sql);
...
public function ingAnualDetalle($conn,$id_anual,$id_mensual,$num_semana,$unidad,$fecha_inicio,$fecha_termino,$num_mes,$n_unidad){
   $sql="insert into planificacion.plani_anual_new_det (id_panual,id_pmensual,fecha_inicio,fecha_termino,id_semana,num_mes,num_unidad) values($id_anual,$id_mensual,'$fecha_inicio','$fecha_termino',$num_semana,$num_mes,$n_unidad)";
$result = pg_exec($conn,$sql);
...
public function ultimaUnidadMensual($conn,$rdb){
 	 $sql="select id_anual_det from planificacion.plani_anual_new_det where id_panual in ( select id_planificacion from planificacion.plani_anual_new where rdb=$rdb order by id_planificacion desc limit 1)";
	$result = pg_exec($conn,$sql);
...
public function traeAnualDet($conn,$unidad){
$sql="select * from planificacion.plani_anual_new_det where id_panual=$unidad";	
$result = pg_exec($conn,$sql);
...
public function upPMensiual($conn,$id_mensual,$cant_semanas,$cant_horas,$didactico,$evaluacion){
 $sql="update planificacion.plani_new_mensual set cant_semanas =$cant_semanas ,cant_horas=$cant_horas,didactico='$didactico',evaluacion='$evaluacion' where id_mensual=$id_mensual";
if(!$result)
		{
			return false;	
		}else{
...
public function upPlanidetalle($conn,$id_det,$fecha_inicio,$fecha_termino,$id_semana,$id_mes,$n_unidad){
	 $sql="update planificacion.plani_anual_new_det set where fecha_inicio='$fecha_inicio',fecha_termino='$fecha_termino',id_semana=$id_semana,num_mes=$id_mes,num_unidad=$n_unidad where id_anual_det=$id_det";
	if(!$result)
		{
			return false;	
		}else{
...
public function eliminaObjAn($conn,$idUnidad){

$sql="delete from planificacion.plani_anual_new_obj where id_semana=$idUnidad";
$result = pg_exec($conn,$sql);
	return $result;	

}

public function eliminaIndAn($conn,$idUnidad){

$sql="delete from planificacion.plani_anual_new_indicador where id_semana=$idUnidad";
$result = pg_exec($conn,$sql);
	return $result;	

}

public function traeClasesSemana($conn,$unidad){
   $sql="select * from planificacion.plani_semanal inner join  where id_mensual=$unidad order by id_clase";
$result = pg_exec($conn,$sql);
return $result;
}
