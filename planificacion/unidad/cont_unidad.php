...
//show($_SESSION);
	...
	//	alert(<?php echo $curso ?>)
     $("#currr").val(<?php echo $curso ?>);
});
...
?>
		<option value="<?=$fila['id_ramo'];?>" <?php echo ($_RAMO==$fila['id_ramo'])?"selected":"" ?>   ><?=$fila['nombre'];?></option>
...
if($funcion==3){
	...
	$( document ).ready(function() {
	//	alert(<?php echo $ramo ?>)
     $("#rara").val(<?php echo $ramo ?>);
	 $("#sel_ramo option[value='"+<?php echo $ramo ?>+"']").prop('selected', true);
});
...
<table width="700" border="0" align="center">
 ...
 <input name="ens2<?php echo $fila['id_unidad']?>" type="hidden" id="ens2<?php echo $fila['id_unidad']?>" value="<?php echo pg_result($rs_curso,3) ?>"/>
    ...
    <td align="center"><input name="ed" type="button" id="ed" onclick="editaUnidad(<?php echo $fila['id_unidad']?>)" value="E" class="botonXX" /></td>
    
    <td align="center"><input name="es" type="button" value="ES" title="Cambiar Estado" onclick="cambiaEstado(<?php echo $fila['id_unidad'] ?>)" class="botonXX" /></td>
   
      <?php if($_PERFIL==0 || $_PERFIL==14 || $_PERFIL==17 || $_PERFIL==25){?>
    <td align="center"><input type="button" name="c" id="c" value="C" title="asignar clases" onclick="creaclases(<?php echo $fila['id_unidad']?>)" class="botonXX" /></td>
   <?php }?>
    <td align="center"><input type="button" name="r" id="r" value="R" title="Replicar Unidad" onclick="replicaUnidad(<?php echo $fila['id_unidad']?>)" class="botonXX" /></td>
     ...
     <td align="center" class="tablaredonda"><input type="button" name="no" id="no" value="N" title="Asociar a notas" <?php echo ($fila['estado']!=2)?"disabled":"" ?> onclick="asocianota(<?php echo $fila['id_unidad'] ?>)" class="<?php echo ($fila['estado']!=2)?"botonXXI":"botonXX" ?>" /></td>
    <td align="center" class="tablaredonda"><a href="carhgafile/index.php?rr=<?php echo $_INSTIT ?>&icls=<?php echo $fila['id_unidad'] ?>" target="_blank" onClick="window.open(this.href, this.target, 'width=600,height=300'); return false;"><input type="button" name="ac" id="ac" value="A" title="Subir Archivo Evaluación" class="botonXX" ></a></td>
    <?php if($_PERFIL==0 ){?>
    <td  class="tablaredonda"><input type="button" name="button2" id="button2" value="X" class="botonBX" / onclick="borraUnidad(<?php echo $fila['id_unidad'] ?>)" ></td>
   <?php }?>
  </tr>
  <?php }?>
</table>
...
if($funcion==5){
	...
	$f_curso = pg_fetch_array($rs_curso,0);
	$rs_tipo=$ob_unidad->tejeRamo($conn,$cod_ramo,$f_curso['ensenanza'],$f_curso['grado_curso']);
...
?>
    ...
    <tr>
     ...
     <? if(pg_numrows($rs_unidad)>0){
		...
		$rs_eje=$ob_unidad->tejeUnidad2($conn,$idUnidad);
...
if($funcion==11){
	...
	$rs_ramo=$ob_unidad->traeRamo($conn,$fila_unidad['id_curso'],$fila_unidad['id_ramo']);
	//$rs_tipo=$ob_unidad->tejeUnidad2($conn,$idUnidad);
...
$rs_objetivo= $ob_unidad->ejeDeRamo($conn,$tipo,$rdb,$cod_ramo);
//echo pg_numrows($rs_objetivo);
 for($o=0;$o<pg_numrows($rs_objetivo);$o++){
		 $fila_obj = pg_fetch_array($rs_objetivo,$o);
		 ?>
		  <tr>
		    <td class="cuadro02"><?php echo $fila_obj['texto'] ?></td>
		  <?
		//$rs_eje =$ob_unidad->traeObj($conn,$fila_obj['id_eje'],$rdb,$ense,$grado);
		$rs_eje =$ob_unidad->traeObjRamo($conn,$fila_obj['id_eje'],$rdb,$ense,$grado,$tipo);//($conn,$fila_obj['id_eje'],$rdb,$ense,$grado);
		 ...
		 for($j=0;$j<pg_numrows($rs_eje);$j++){
			...
			$cad_ind="";
			$rs_inditpo = $ob_unidad->traeIndiRamo($conn,$fila['id_obj']);
			if(pg_numrows($rs_inditpo)>0){
...
<input name="ccr" type="hidden" id="ccr" value="<?php echo $_SESSION['IDR'] ?>"  />
...
?>
		<option value="<?=$fila_r['id_ramo'];?>" <?php echo ($fila['id_ramo']==$_RAMO)?"selected":"" ?> ><?=$fila_r['nombre'];?></option>
                            ...
                            <table width="100%" border="0">
                              <tr>
                                <td align="right"><span id="nvo">
                                 <?php if($_PERFIL==0){?><input type="button" name="nuevaUnidad" id="nuevaUnidad" class="botonXX" value="Banco de Planificaciones" onclick="creaUnidadBanco()" /> <?php }?>
...
//ver si tengo clases asociadas
$rs_cls= $ob_unidad->traeClases($conn,$id_unidad);
if(pg_numrows($rs_cls)>0){
...
$ruta_u ="acv/";
//busco si tengo archivos asociados
		$rs_listau = $ob_unidad->archivoUnidadT($conn,$id_unidad);
		if(pg_numrows($rs_lista)>0){
			for($b=0;$b<pg_numrows($rs_listau);$b++){
			$filau = pg_fetch_array($rs_listau,$b);
				@unlink($ruta.$filau['ruta']) ;
			}
		}


$ob_unidad->delObsUnidad($conn,$id_unidad);
$ob_unidad->delArchivoUnidad($conn,$id_unidad);
$ob_unidad->delObjUnidad($conn,$id_unidad);
$ob_unidad->delNotaUnidad($conn,$id_unidad);
...
$rs_indi = $ob_unidad->buscaIndicadorSelRamo($conn,$obj,$ciu);
?><br />
<br />
...
<?php for($i=0;$i<pg_numrows($rs_indi);$i++){
  ...
  <tr id="filaind<?php echo  $fila['id_indicador']?>">
    <td onclick="ppi(<?php echo $fila['id_indicador'] ?>,<?php echo $obj ?>,<?php echo $fila['tipo'] ?>)"><?php echo nl2br($fila['texto']) ?>&nbsp;
...
if($funcion==28){
	...
	//$rs_uni =  $ob_unidad->listaUnidadBanco($conn,$_ANO,$_INSTIT,$curso,$ramo,'','');
	$rs_uni = $ob_unidad->listaUnidadN($conn,$_ANO,$_INSTIT,$ense,$grado,$cod_ramo)
	?>
    <select name="c_unidadp" id="c_unidadp">
    <option value="0">Seleccione Unidad</option>
    <?php for($i=0;$i<pg_numrows($rs_uni);$i++){
		$rs = pg_fetch_array($rs_uni,$i);
		?>
    <option value="<?php echo $rs['id_unidad'] ?>"><?php echo $rs['nombre'] ?></option>
    <?php }?>
    </select>
    <?
}
if($funcion==29){
	...
	$rs_objetivo =$ob_unidad->traeEjeObjetivo($conn,$rdb,$cod_ramo);
	$rs_habilidad =$ob_unidad->traeEjeHabilidad($conn,$rdb,$cod_ramo);
	$rs_unidad =$ob_unidad->traeUnidadBP($conn,$idUnidad);
	$fila_unidad = pg_fetch_array($rs_unidad,0);
	
	/*// echo pg_numrows($rs_obj);
	$rs_hab=$ob_unidad->traeObjUnidad($conn,1,$idUnidad);*/
	$rs_ramo=$ob_unidad->traeRamo($conn,$fila_unidad['id_curso'],$fila_unidad['id_ramo']);
	$rs_tipo=$ob_unidad->tejeUnidadBP($conn,$idUnidad);
	 $cad_ind="";
...
$(document).ready(function(){
  ...
  $("#nvo").html('<input type="button" name="actUnidad" id="actaUnidad" value="Guardar Datos" onclick="GuardaUnidad()" class="botonXX"/> <input type="button" name="cUnidad" id="cUnidad" value="Cancelar" onclick="cancela()" class="botonXX" />');
 ...
 <tr>
    ...
    <input type="hidden" name="cargahab" id="cargahab"  value="<?php echo $cab ?>" />-->
	 ...
	 for($ti=0;$ti<pg_numrows($rs_tipo);$ti++){
		  ...
		  ?>
           <input type="hidden" name="cargatipo[]" id="cargatipo<?php echo $fila_tipo['id_objetivo']; ?>" value="<?php echo $cob ?>" /> 
      <input name="tipo" id="tipo<?php echo $t ?>" type="radio" value="<?php echo $fila_tipo['id_objetivo']; ?>" onclick="cargatipoBP(<?php echo $fila_tipo['id_objetivo']; ?>,<?php echo $idUnidad; ?>);" <?php echo ($ti==0)?"checked":"" ?> /><?php echo $fila_tipo['nombre']; ?>
      <input type="hidden" name="cargaind[]" id="cargaind<?php echo $fila_tipo['id_objetivo']; ?>" value="<?php echo $cad_ind ?>" />  
    <?php }?>
    </td>
  </tr>
...
<script>
cargatipoBP(<?php echo $crg ?>,<?php echo $idUnidad ?>);
</script>
<?	
}
if($funcion==30){
...
<table width="650" align="center" border="1" style="border-collapse:collapse">
	<?
	//if($tipo==0){ejeDeUnidad ejeDeUnidad($conn,$unidad,$tipo,$rdb,$cod_ramo)
$rs_objetivo =$ob_unidad->ejeDeUnidadPB($conn,$unn,$tipo,$rdb,$cod_ramo);
//echo pg_numrows($rs_objetivo);
 for($o=0;$o<pg_numrows($rs_objetivo);$o++){
		 $fila_obj = pg_fetch_array($rs_objetivo,$o);
		 ?>
		...
		// $rs_eje =$ob_unidad->traeObj($conn,$fila_obj['id_eje'],$rdb,$ense,$grado);
		  ...
		  $rs_eje =$ob_unidad->traeObjUnidadBP($conn,$tipo,$unn);
		 for($j=0;$j<pg_numrows($rs_eje);$j++){
			$fila = $fila_obj = pg_fetch_array($rs_eje,$j);
			
			$cad_ind="";
			$rs_inditpo = $ob_unidad->traeIndiUnidadOBP($conn,$unn,$fila['id_obj']);
			if(pg_numrows($rs_inditpo)>0){
  ...
  <tr><td align="justify" id="fila<?php echo  $fila['id_obj']?>" onclick="ppBP(<?php echo  $fila['id_obj']?>);sumatipo(<?php echo  $fila['tipo']?>); buscasel(<?php echo  $fila['tipo']?>);" class="i textosimple"><?php echo nl2br($fila['codigo']."-".$fila['texto']) ?><input name="obj_destino[]" type="checkbox" class="oo<?php echo  $fila['tipo']?>" id="destino<?=$fila['id_obj']?>" style="visibility:hidden" value="<?php echo  $fila['id_obj']?>" /> <input type="hidden" id="lindv<?php echo  $fila['id_obj']?>" name="lindv<?php echo  $fila['id_obj']?>" class="lindv<?php echo  $fila['tipo']?>" value="<?php echo $cad_ind ?>"/></td></tr>
...
$rs_indi = $ob_unidad->buscaIndicadorSelBP($conn,$obj,$ciu);
?><br />
<br />
