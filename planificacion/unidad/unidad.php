...
$_IDR = $rr;
$_SESSSION['_ANO'] = $_GET['aa'];
$_CURSO = $cc;
$_MENU = 4;
$_CATEGORIA = 61;
$_PERFIL = 17;

$_DOCRAMO=$_NOMBREUSUARIO;
$_SUBRAMO=$ss;
...
//$rs_ramo = $ob_unidad->traeRamo($conn,$_CURSO);
//$rs_ramo = $ob_unidad->traeRamoUno($conn,$_IDR);
...
$(document).ready(function(){
		...
		$("#docdicta").val(<?php echo $_DOCRAMO ?>);
		$("#cod_ramo").val(<?php echo $_SUBRAMO ?>);
		dicta(<?php echo $_RAMO ?>);
	
	   // $("#sel_curso").val("<?php echo $_CURSO ?>").change();
	 
		
		//  $('#sel_curso option[value="<?php echo $_CURSO ?>"]').prop('selected', true);
		
		//setSelectValue('sel_curso',<?php echo $_CURSO ?>);
		
			
		<?php if(!isset($_GET['orr'])){?>
		
		//$("#sel_ramo").val(<?php echo $_RAMO ?>);
			traeUnidades();
		
		<?php }else{?>
		$("#rara").val(<?php echo $_GET['rr'] ?>);
		
		dicta($("#rara").val());
		$("#docdicta").val(<?php echo $_DOCRAMO ?>);
		$("#cod_ramo").val(<?php echo $_SUBRAMO ?>);
		editaUnidad(<?php echo $_GET['uu'] ?>);
  ...
  document.getElementById("sel_ramo").options.selectedIndex = index;
}
...
function cancela(){
 $("#nvo").html('<?php if($_PERFIL==0){?><input type="button" name="nuevaUnidad" id="nuevaUnidad" class="botonXX" value="Banco de Planificaciones" onclick="creaUnidadBanco()" /> <?php }?><input type="button" name="nuevaUnidad" id="nuevaUnidad" value="Nueva Unidad" onclick="creaUnidad()" class="botonXX" /> <input type="button" name="busca" id="busca" class="botonXX" value="Buscar" onclick="traeUnidades()" />');
  traeUnidades();
}
...
function editaUnidadG(idUnidad){
var funcion =11;
var rdb = <?php echo $_INSTIT ?>;
var cod_ramo = <?php echo intval($ss) ?>;
var grdo = <?php echo intval($gg) ?>;
var ens = <?php echo intval($eee); ?>;
var parametros = "&funcion="+funcion+"&idUnidad="+idUnidad+"&cod_ramo="+cod_ramo+"&rdb="+rdb+"&grdo="+grdo+"&ens="+ens;
$("#nvo").html('<input type="button" name="actUnidad" id="actaUnidad" value="Actualizar Datos" onclick="GuardaUnidadAct()" class="botonXX"/> <input type="button" name="cUnidad" id="cUnidad" value="Cancelar" onclick="cancela()" class="botonXX" />');
...
function vuelveAnual(cic){
	location.href="../anualUnidad/unidad.php";
...
function creaUnidadBanco(){
var funcion=28;
var curso =$("#sel_curso").val();
var ramo=$("#sel_ramo").val();
var cod_ramo=$("#cod_ramo").val();
var parametros="funcion="+funcion+"&curso="+curso+"&ramo="+ramo+"&cod_ramo="+cod_ramo;	
$.ajax({
	  url:'cont_unidad.php',
	  data:parametros,
	  type:'POST',
				...
				},buttons: [
    {
        ...
        click: function() { 
		//armalin(obj);
		var idplan = $("#c_unidadp").val();
		if(idplan>0){
		armaplan(idplan);
		$(this).dialog("close");
		}else{
		alert("Elija planificaci\xf3n")	
		}
...
function armaplan(idplan){
var funcion =29;
var id_ramo= $('#sel_ramo').val();
var rdb = <?php echo $_INSTIT ?>;
var cod_ramo = $("#cod_ramo").val();
//var grdo = 0;$("#grdo2"+idUnidad).val();
//var ens = 0;$("#ens2"+idUnidad).val();
var parametros = "funcion="+funcion+"&idUnidad="+idplan+"&cod_ramo="+cod_ramo+"&rdb="+rdb;
//alert(parametros);
$("#nvo").html();
...
function cargatipoBP(tipo,unidad){
...
function indicaBP(obj){
//alert(obj);
var funcion=31;
var ciu = $("#idUnidad").val();
var ttp = $('input:radio[name=tipo]:checked').val()
var parametros="funcion="+funcion+"&obj="+obj+"&ciu="+ciu;	
$.ajax({
	  url:'cont_unidad.php',
...
<br>
                      ...
                      <form id="frm" name="frm">
                        ...
                        <input name="cir" type="hidden" id="cir" value="<?php echo $_SESSION['IDR'] ?>"  />
                        ...
                        <label for="currr"></label>
                        <input type="hidden" name="currr" id="currr" value="<?php echo $_CURSO ?>" />
                        
                        <input type="hidden" name="rara" id="rara" value="<?php echo $_RAMO ?>" />
                        </div>
...
<table width="650" border="0" align="center">
  <tr>
                     ...
                     <!--       <td class="textonegrita">UNIDAD</td>
                          ...
                          <tr>
                            <td>&nbsp;</td>
                            <td><input type="hidden" name="cod_ramo" id="cod_ramo" value="<?php echo $ccr ?>" /></td>
                          </tr>
                          <tr>
                            <td colspan="2" align="left">
                            <table width="100%" border="0">
                              <tr>
                                <td align="right"><div id="nvo"> <?php if($_PERFIL==0){?><input type="button" name="nuevaUnidad" id="nuevaUnidad" class="botonXX" value="Banco de Planificaciones" onclick="creaUnidadBanco()" /> <?php }?>
...
<div id="nota" title="Asociar unidad a una nota" align="center"> </div>
 ...
 <div id="cb" title="Seleccionar Banco de Planificaciones"></div>
</body>
