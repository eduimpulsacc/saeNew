...
inner join planificacion.unidad_anio_obj 
on planificacion.unidad_anio_obj.id_obj = planificacion.obj_hab.id_obj
WHERE id_eje=$id_eje and planificacion.obj_hab.tipo=$tipo and   planificacion.unidad_anio_obj.id_unidad=$unidad
...
public function guardaUnidad($conn,$rdb,$ano,$sel_curso,$sel_ramo,$docdicta,$txt_fechaini,$txt_fechater,$cant_clases,$txt_horas,$texto,$txt_nombre,$ciu){
	   ...
	   $sql="insert into planificacion.unidad(rdb,id_ano,id_curso,id_ramo,rut_emp,fecha_inicio,fecha_termino,cantidad_clases,nro_horas,texto,nombre,unidad_anual,estado) values ($rdb,$ano,$sel_curso,$sel_ramo,$docdicta,'$txt_fechaini','$txt_fechater',$cant_clases,$txt_horas,'".utf8_decode($texto)."','".utf8_decode($txt_nombre)."',".intval($ciu).",1)";
	$result = pg_exec($conn,$sql);
...
public function ultimaUnidad($conn,$rdb){
 	$sql="select id_unidad from planificacion.unidad where rdb=$rdb order by id_unidad desc limit 1";
	$result = pg_exec($conn,$sql);
...
public function guardaObjetivo($conn,$id_unidad,$id_obj){
	  $sql="insert into planificacion.unidad_obj values($id_unidad,$id_obj);";
	$result = pg_exec($conn,$sql);
	
}	
	
public function listaUnidad($conn,$id_ano,$rdb,$curso,$ramo,$docente,$id_unidad){
	$sql="select planificacion.unidad.* from planificacion.unidad inner join curso 
	on planificacion.unidad.id_curso = curso.id_curso
	where rdb=$rdb";
	
	if($id_ano>0){
	$sql.=" and planificacion.unidad.id_ano=$id_ano";
	}
	
	if($curso>0){
	$sql.=" and planificacion.unidad.id_curso=$curso";
	}
	...
	$sql.=" order by curso.ensenanza,curso.grado_curso,curso.letra_curso,planificacion.unidad.id_unidad ";
	//echo $sql;
	$result = pg_exec($conn,$sql);
...
public function actualizaUnidad($conn,$idUnidad,$txt_fechaini,$txt_fechater,$cant_clases,$txt_horas,$texto,$txt_nombre){
 $sql= "update planificacion.unidad set fecha_inicio='$txt_fechaini',fecha_termino='$txt_fechater',cantidad_clases=$cant_clases,nro_horas=$txt_horas,texto='".utf8_decode($texto)."',nombre='".utf8_decode($txt_nombre)."',estado =3 where id_unidad=$idUnidad";
...
$sql="delete from planificacion.unidad_obj where id_unidad=$idUnidad";
$result = pg_exec($conn,$sql);
	return $result;	
...
public function traeMarcado($conn,$idUnidad,$id_obj){
    $sql="select id_obj as marcado from planificacion.unidad_obj where id_unidad=$idUnidad and id_obj=$id_obj";
 $result = pg_exec($conn,$sql);
	return $result;	
}
...
public function listaUnidadAnio($conn,$curso,$ramo){
 $sql="select * from planificacion.unidad_anio where id_curso=$curso and id_ramo=$ramo";
//echo  $sql;
	$result = pg_exec($conn,$sql);
...
public function traearchivo($conn,$unidad){
     $sql="select * from planificacion.unidad_archivo where id_unidad=$unidad and visible=1";
$result = pg_exec($conn,$sql);
return $result;	
}
...
return $result;	
	...
	public function tejeUnidad2($conn,$unidad){
	   $sql="select DISTINCT(tob.id_objetivo),tob.nombre from planificacion.tipo_objetivo tob
inner join planificacion.obj_hab oh on oh.tipo =tob.id_objetivo
...
inner join planificacion.unidad_anio_obj ao on ao.id_obj = oh.id_obj
...
$sql="insert into planificacion.indeva_unidad values($unidad,$indicador)";
$result = pg_exec($conn,$sql);
return $result;	
}

public function guardaIndicadorSel($conn,$unidad,$iobj,$indicador){
$sql="insert into planificacion.indeva_unidad values($unidad,$iobj,$indicador)";
$result = pg_exec($conn,$sql);
return $result;	
}

public function borra($conn,$unidad){
$sql="delete from planificacion.indeva_unidad where id_unidad=$unidad";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function listaUnidadBanco($conn,$id_ano,$rdb,$curso,$ramo,$docente,$id_unidad){
	$sql="select planificacion.bp_unidad_anio.* from planificacion.bp_unidad_anio inner join curso 
	on planificacion.bp_unidad_anio.id_curso = curso.id_curso
	where rdb=$rdb";
	
	if($id_ano>0){
	$sql.=" and planificacion.bp_unidad_anio.id_ano=$id_ano";
	}
	
	if($curso>0){
	$sql.=" and planificacion.bp_unidad_anio.id_curso=$curso";
	}
	...
	$sql.=" order by curso.ensenanza,curso.grado_curso,curso.letra_curso,planificacion.bp_unidad_anio.id_unidad ";
...
public function traeUnidadBP($conn,$idUnidad){
...
public function tejeUnidadBP($conn,$unidad){
...
return $result;	
	...
	public function ejeDeUnidadPB($conn,$unidad,$tipo,$rdb,$cod_ramo){
	$sql="SELECT DISTINCT(pe.id_eje),pe.texto
 FROM planificacion.ejes pe 
 inner join planificacion.obj_hab oh on pe.id_eje = oh.id_eje
  inner join planificacion.bp_unidad_anio_obj ao on ao.id_obj = oh.id_obj 
  WHERE ao.id_unidad=$unidad and pe.tipo=$tipo
  and cod_subsector=$cod_ramo and pe.rdb in(0,$rdb) order by pe.texto";
$result = pg_exec($conn,$sql);
return $result;		
	}
	
public function traeObjUnidadBP($conn,$tipo,$idUnidad){
    $sql="select oh.* from planificacion.obj_hab oh inner join planificacion.bp_unidad_anio_obj ob  on ob.id_obj = oh.id_obj
where oh.tipo=$tipo and ob.id_unidad=$idUnidad order by oh.id_obj";
 $result = pg_exec($conn,$sql);
	return $result;	
}

public function traeIndiUnidadOBP($conn,$unidad,$tipo){
   $sql="select * from planificacion.indicador_evaluacion indic
inner join planificacion.indeva_unidad inde
on inde.id_indicador = indic.id_indicador
inner join planificacion.obj_hab oh on oh.id_obj = inde.id_obj
...
public function buscaIndicadorSelBP($conn,$id_obj,$uano){
  $sql="select indic.* from planificacion.indicador_evaluacion indic
 inner join planificacion.bp_unidad_anio_indicador  
 inde on inde.id_indicador = indic.id_indicador
  where inde.id_obj =$id_obj and inde.id_unidad=$uano";
$result = pg_exec($conn,$sql);
return $result;	
}
...
public function tejeRamo($conn,$subsector,$ense,$grado){
     $sql="select DISTINCT(tob.id_objetivo),tob.nombre from planificacion.tipo_objetivo tob
...
public function ejeDeRamo($conn,$tipo,$rdb,$cod_ramo){
	   $sql="SELECT DISTINCT(pe.id_eje),pe.texto 
FROM planificacion.ejes pe 
inner join planificacion.obj_hab oh on pe.id_eje = oh.id_eje
WHERE pe.tipo=$tipo and cod_subsector= $cod_ramo
and pe.rdb in(0,$rdb) order by pe.texto";
$result = pg_exec($conn,$sql);
return $result;		
	}

public function traeObjRamo($conn,$id_eje,$rdb,$ense,$grado,$tipo){
    $sql="SELECT * FROM planificacion.obj_hab WHERE id_eje=$id_eje and tipo=$tipo and tipo_ense=$ense and grado=$grado and rdb in(0,$rdb) order by codigo";
		$result = pg_exec($conn,$sql);
		//echo $sql."<br>";
		return $result;	
}	
...
public function traeIndiRamo($conn,$tipo){
    $sql="select * from planificacion.indicador_evaluacion inde
 inner join planificacion.obj_hab oh on oh.id_obj = inde.id_obj
  where oh.id_obj=$tipo";	
$result = pg_exec($conn,$sql);
	return $result;
}
public function buscaIndicadorSelRamo($conn,$id_obj){
 $sql="select indic.* from planificacion.indicador_evaluacion indic
 where indic.id_obj =$id_obj ";
$result = pg_exec($conn,$sql);
return $result;	
}


public function listaUnidadN($conn,$id_ano,$rdb,$ense,$grado,$ramo){
	$sql="select planificacion.bp_unidad_anio.* from planificacion.bp_unidad_anio 
where rdb=$rdb 
and id_ano=$id_ano 
and grado_curso=$grado 
and ensenanza=$ense 
and cod_ramo=$ramo 
order by 
grado_curso,
