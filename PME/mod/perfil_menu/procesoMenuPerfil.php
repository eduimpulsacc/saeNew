...
$obj_Perfil_vs_Menu = new Crea_Perfil_Menu($_IPDB,$_ID_BASE);

$funcion = $_POST['funcion'];

	if($caso==2){
		$sql = "DELETE FROM pme.perfil_menu WHERE rdb=".$institucion." AND id_perfil=".$cmbPERFIL;
	...
	for($i=0;$i<$contador_menu;$i++){
		...
		if(isset($menu)){
			 ...
			 $sql ="SELECT * FROM pme.perfil_menu WHERE id_perfil=".$cmbPERFIL." AND rdb=".$institucion." AND id_menu=".$menu;
			...
			if(@pg_numrows($result)==0){
				if(trim($contador_cat)>0){
					for($j=0;$j<$contador_cat;$j++){
						...
						if(isset($contador_item)){
							for($x=0;$x<$contador_item;$x++){
								...
								if(isset($item)){
								 $sql = "INSERT INTO pme.perfil_menu (id_perfil,rdb,id_menu,id_categoria,id_item,bool_i,bool_m,bool_e,bool_v) VALUES (".$cmbPERFIL.",".$institucion.",".$menu.",".$categoria.",".$item.",".$ingreso.",".$modifica.",".$elimina.",".$ver.")";
									$rs_menu = @pg_Exec($obj_Perfil_vs_Menu->Conec->conectar(),$sql)or die("Fallo Insert 1".$sql);
								}
							}
						}else{
							if(isset($categoria)){
								$sql = "INSERT INTO pme.perfil_menu (id_perfil,rdb,id_menu,id_categoria,id_item,bool_i,bool_m,bool_e,bool_v) VALUES (".$cmbPERFIL.",".$institucion.",".$menu.",".$categoria.",0,".$ingreso.",".$modifica.",".$elimina.",".$ver.")";
					...
					$sql = "INSERT INTO pme.perfil_menu (id_perfil,rdb,id_menu,id_categoria,id_item,bool_i,bool_m,bool_e,bool_v) VALUES (".$cmbPERFIL.",".$institucion.",".$menu.",0,0,".$ingreso.",".$modifica.",".$elimina.",".$ver.")";
