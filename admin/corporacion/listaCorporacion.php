...
if ( $_PERFIL != 26 ){
	...
	$qry_ins="select corp_instit.rdb, institucion.nombre_instit from corp_instit, institucion where 
	corp_instit.num_corp = $_CORPORACION and corp_instit.rdb = institucion.rdb";
    $result_ins=@pg_Exec($conn,$qry_ins);

}else{

	/*$conn=pg_connect("dbname=coi_final_vina host=200.29.21.124 port=5432 user=postgres 
