...
function cargarinicio(x){ 
 ...
 cargaContenido(id); 
	...
	function valida(form){
	 ...
	 if(document.form.cmbNIVEL.value==0){
		 alert("Seleccione Nivel");
		 return false;
		 }	 	 
		 
	 if(document.form.cmbSUBSECTOR_1.value==0){
		 alert("Seleccione Subsector");
		 return false;
		 }	 
		 
		 
		 form.action = 'printAproReproSubNivel.php';
				form.submit(true);
	 }		
        ...
        <tr>
		  ...
		  <table width="61%" border="0" align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td class="cuadro01"><? 	$sql ="	SELECT id_nivel, nombre FROM NIVELES ";
					$rs_nivel = @pg_exec($conn,$sql);
				?>
                <select name="cmbNIVEL" id="cmbNIVEL" onChange='cargaContenido(this.id)'>
                  <option value="0" selected="selected">seleccione...</option>
                  <? 	for($x=0;$x<@pg_numrows($rs_nivel);$x++){
						$fila_nivel = @pg_fetch_array($rs_nivel,$x);
				?>
                  <option value="<?=$fila_nivel['id_nivel'];?>" <? if($cmbNIVEL==$fila_nivel['id_nivel']){?> selected="selected"<? }?>>
                  <?=$fila_nivel['nombre'];?>
                  </option>
                  <? } ?>
                </select></td>
            ...
            <tr>
              ...
              <td class="cuadro01">			 
			 <select disabled="disabled" name="cmbSUBSECTOR_1" id="cmbSUBSECTOR_1">									    
				<option value="0">Selecciona opci&oacute;n...</option>
				</select>
			 </td><td class="cuadro01">&nbsp;</td>
            </tr>
            <tr>
              <td  class="cuadro01">&nbsp;</td>
