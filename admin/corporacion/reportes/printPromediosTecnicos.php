...
<table width="750" height="843" border="0" align="center" cellpadding="0" cellspacing="0" >
  <tr>
    <td height="113" valign="top">
      ...
      <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" >
        <tr>
          <td class="titulo1"><div align="center">PROMEDIOS ESTABLECIMIENTOS TECNICOS POR NIVEL </div>
                    A&Ntilde;O <?=$ano;?></td>
        </tr>
      </table>
    <br />
	<?  				$qry_niv = "select id_nivel, nombre from niveles where id_nivel=".$cmbNIVEL;
						...
						$qry2.="tipo_ense_inst.cod_tipo = 410  group by institucion.rdb, institucion.nombre_instit";
    ...
    <table width="650" border="1" align="center" cellpadding="2" cellspacing="0" class="Estilo7">
								  ...
								  $sql_curso1 = "select  sum(CAST(promedio AS integer)) as suma, count(promedio) as
									 cantidad from notas$cmbANO where id_ramo in (select id_ramo from ramo
									 where id_curso in (select id_curso from curso where id_ano in (select										
									 id_ano from ano_escolar where id_institucion = ".$rdb_[$i]." and id_ano
									 = ".$id_ano_[$i].") and ensenanza = '410' or ensenanza ='510' or ensenanza ='610' and grado_curso = '1'))   and
									 id_periodo = ".$id_periodo_[$i]." and promedio > '0'";
								 ...
								 $sql_curso2 = "select  sum(CAST(promedio AS integer)) as suma, count(promedio) as cantidad
									 from notas$cmbANO where id_ramo in (select id_ramo from ramo where id_curso
									 in (select id_curso from curso where id_ano in (select id_ano from
									 ano_escolar where id_institucion = ".$rdb_[$i]." and id_ano 
									= ".$id_ano_[$i].") and ensenanza IN(410,510,610) and grado_curso = '2')) and 
									id_periodo = ".$id_periodo_[$i]." and promedio > '0'";
									...
									$sql_curso3.=" ensenanza IN(410,510,610) and grado_curso = '3'))   and id_periodo = ";
									$sql_curso3.="".$id_periodo_[$i]." and promedio > '0'";
									...
									$sql_curso4 = "select  sum(CAST(promedio AS integer)) as suma, count(promedio) as cantidad from
									 notas$cmbANO where id_ramo in (select id_ramo from ramo where id_curso in 
									(select id_curso from curso where id_ano in (select id_ano from ano_escolar where
									 id_institucion = ".$rdb_[$i]." and id_ano = ".$id_ano_[$i].") and ensenanza IN(410,510,610) or grado_curso = '4')) and id_periodo = ".$id_periodo_[$i]." and promedio > '0'";
