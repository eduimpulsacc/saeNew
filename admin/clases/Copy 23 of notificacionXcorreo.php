...
function correo_notificacion_anotacion($rut,$ano,$conn,$usuario,$guardo,$id_ramo) {
		 ...
		 ncc.rbd, ncc.tipo_ensenanza, ncc.cargo, ncc.notifica_notas, 
		 ncc.nro_notas, ncc.nota_deficiente, ncc.notifica_anotaciones, ncc.nro_anotaciones, 
		 ncc.notifica_asistencia, ncc.dias_asistencia, ncc.periodo_notificacion, 
		 ...
		 WHERE ma.rut_alumno = $rut AND ma.id_ano = $ano AND ncc.notifica_anotaciones = 1";
	   ...
	   // entra solo si esta activa la opcion de enviar por anotaciones 
			 ...
			 FROM (select count(aa1.id_anotacion) as num_anotaciones_negativas  from anotacion aa1
			 ...
			 UNION
			 select count(aa1.id_anotacion) as num_anotaciones_negativas  from anotacion aa1
			 where 
			 aa1.rut_alumno = $rut 
       ...
       if( $fsql['nro_anotaciones'] <= $totalnegativas ){   // si se cumple la condici�n	   
...
ORDER BY  tipo_conducta,fecha desc";
			...
			$asunto="Notificacion Alumnos Cumple Condici�n de Anotaci�nes";
...
WHERE ma.id_curso = $idcurso AND ncc.notifica_asistencia = 1
AND ma.id_ano = $idano LIMIT 1";
	   
	   $rsql_parametros = pg_Exec($conn,$qsql_parametros);
	   $n0 = @pg_numrows($rsql_parametros);
	   $fsql_parametros = pg_fetch_array($rsql_parametros,0);
       
	   $idconfiguracion = $fsql_parametros['id_notifica_correo_configuracion'];
	   $nro_a�o = $fsql_parametros['nro_ano'];
	   $rbd = $fsql_parametros['rbd'];
...
$f_datos = pg_fetch_array($r_curso,0);
 ...
 $mensaje = '<br>
            ...
            <table border="1">
			<tr align="left" >
			<th colspan="3" >Listado de Alumnos que cumplen '.$dias_asistencia.' o m�s dias inasistencia</th>
			</tr>
			<tr align="left" >
			<th>Rut</th>
			<th>Nombre</th>
...
$asunto="Notificacion Alumnos Cumple Condici�n de inasistencias";
...
function envio_correo_notificacion($asunto,$mensaje,$idconfiguracion,$conn,$rbd,$idano,$idcurso,$usuario,$guardo,$id_ramo) { //INICIO
		...
		$destinatario = substr($destinatario,0,-1);
			...
			$cabeceras .= "To: Patricio Cardenas <eee@colegiointeractivo.com> \r\n";
			$cabeceras .= "From: Patricio Cardenas <pcardenas@colegiointeractivo.com> \r\n";
