...
(function($) {
		...
		*/
		...
		this.getWeekIndex = function(dayName){
			if(dayName.toUpperCase() == "DOM" || dayName.toUpperCase() == "DOMINGO"){
				...
				return 2;
			}else if(dayName.toUpperCase() == "MIE" || dayName.toUpperCase() == "MI&Eacute;RCOLES"){
				return 3;
			}else if(dayName.toUpperCase() == "JUE" || dayName.toUpperCase() == "JUEVES"){
				return 4;
			}else if(dayName.toUpperCase() == "VIE" || dayName.toUpperCase() == "VIERNES"){
				return 5;
			}else if(dayName.toUpperCase() == "SAB" || dayName.toUpperCase() == "S&Aacute;BADO"){
				return 6;
			}else{
				return -1
			}			
	...
	// static properties
	Calendar.dayNames = new Array("Lun","Mar","Mie","Jue","Vie","S&aacute;b","Dom");
	/**
	...
	Calendar.showMoreAgendaModal = function(cal,date,agendaItems){
		...
		var xd = date.toLocaleString();
		var res = xd.split(" ");
		modal.attr("title","Programaci&oacute;n para el d&iacute;a: " + res[0]);
		modal.dialog({
			autoOpen: false,
			height: 350,
			width: 400,
		...
		for(var i=0; i<agendaItems.length; i++){
			...
			var agein = agendaStartDt.toLocaleString();
			var resin = agein.split(" ");
			
			
			//alert(agendaStartDt.toLocaleString());
			agendaEndDt = agi.getEndDate();
			var agefn = agendaStartDt.toLocaleString();
			...
			if(agi.isAllDay()){
				...
				/*var xd = agendaStartDt.toLocaleString();
				var res = xd.split(" ")*/
				
				agendaDiv.append("Desde las " + DateUtil.getAgendaDisplayTime(agendaStartDt) + " del " + resin[0] + 
					" Hasta las " + DateUtil.getAgendaDisplayTime(agendaEndDt) + " del " + resfn[0]  + "<br>");
			}
			// add click event
		...
		this.addAgendaItem = function(calId,title,startDate,endDate,allDay,data,displayProp){
			if(calId != null && title != null && startDate != null && endDate != null && allDay != null){
				...
				if(DateUtil.secondsDifferenceDirection(startDate,endDate) < 0){
					alert("No puede crear un evento que termine antes que empiece");
					return;
				}
