...
$conn=pg_connect("dbname=coi_final host=10.132.10.36 port=5432 user=postgres password=cole#newaccess");
 ...
 $sql = "SELECT ncc.rbd,ncc.tipo_ensenanza,
               ncc.id_notifica_correo_configuracion,
               ncc.notifica_anotaciones,
               ncc.nro_anotaciones	
	  ...
	  if(pg_numrows($resul)>0){
        ...
        echo $tipo_ensenanza = $fila['tipo_ensenanza'];
	    ...
	    $myArr=array(  "RUT","Nombre_Alumno","Curso", "Ensenanza","Docente","Detalle","Fecha"  );
	 ...
	 $qsql = "SELECT ma.rut_alumno FROM matricula as ma 
	                   INNER JOIN curso ON curso.id_curso = ma.id_curso AND curso.ensenanza = $tipo_ensenanza
						WHERE ma.rdb = $rbd  and ma.bool_ar = 0 and ma.id_ano = 1223";
						
	   $rsql = pg_Exec($conn,$qsql) or die ( pg_last_error($conn)."INICIO 2");
	   
	   $sql = "SELECT * FROM ano_escolar WHERE id_ano = 1223";
...
*/
        ...
        echo "<br/>(Inicio Envio de Correo)<br/>";
						...
						$mail->Body="Hola,<br>este correo ha sido enviado desde PHP usando <strong>PHPMailer</strong>.";
						$exito = $mail->Send();
						$mail->IsMail();  
					    $intentos=1; 
