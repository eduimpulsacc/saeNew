...
FROM ramo inner join subsector on ramo.cod_subsector=subsector.cod_subsector WHERE id_curso in (SELECT id_curso FROM curso WHERE id_ano=".$this->ano. " AND ensenanza=".$this->ensenanza.")";
	...
	///////////////////////////////////////////////////////////////////////
	function Datos_inst_NFP($conn){
		$sql= "SELECT institucion.nombre_instit, institucion.insignia, institucion.calle, institucion.nro, region.nom_reg, provincia.nom_pro, comuna.nom_com, institucion.telefono ";
		$sql.="FROM ((institucion INNER JOIN region ON institucion.region = region.cod_reg) INNER JOIN comuna ON (institucion.ciudad = comuna.cor_pro) AND (institucion.comuna = comuna.cor_com) AND (institucion.region = comuna.cod_reg)) INNER JOIN provincia ON (institucion.ciudad = provincia.cor_pro) AND (institucion.region = provincia.cod_reg) ";
		$sql.="WHERE (((institucion.rdb)=".$this->institucion.")); ";
		$result =pg_exec($conn,$sql) or die ("Fallo sql:".$sql);
		 $fila=pg_fetch_array($result,0);
		 $this->nombre_institu = ucwords(strtolower($fila_['nombre_instit']));
		 $this->direccion = ucwords(strtolower($fila['calle'] . " " . $fila['nro'] . " - " . $fila_['nom_com']));
		 $this->telefono = $fila['telefono'];
		 $this->region = ucwords(strtolower($fila['nom_reg']));
		 $this->provincia = ucwords(strtolower($fila['nom_pro']));
		 $this->comuna = ucwords(strtolower($fila['nom_com']));
		 return;
	}	
	function Datos_curso_NFP($conn){
		...
		$sql_curso = $sql_curso . "WHERE (((curso.id_curso)=".$this->curso."));";
		$result_curso = @pg_Exec($conn, $sql_curso);
			$fila_curso = @pg_fetch_array($result_curso ,0);
			$this->decreto_eval = $fila_curso['nombre_decreto_eval'];
			$this->planes = $fila_curso['nombre_decreto'];
			$this->truncado_per = $fila_curso['truncado_per'];
		return;
	}
	function Datos_profejefe_NFP($conn){
		$sql_profe = "SELECT empleado.ape_pat, empleado.ape_mat, empleado.nombre_emp ";
		...
		$sql_profe = $sql_profe . "WHERE (((curso.id_curso)=".$this->curso.")); ";
		$result_profe = @pg_Exec($conn, $sql_profe);
		$fila_profe = @pg_fetch_array($result_profe ,0);
			$this->profesor = ucwords(strtoupper($fila_profe['nombre_emp'] . " " . $fila_profe['ape_pat'] . " " . $fila_profe['ape_mat']));
	
		return;
	}
	function Datos_Alumnos_NFP($conn){
		...
		$sql_alu = $sql_alu . "WHERE (((matricula.id_curso)=".$this->curso.")) AND matricula.bool_ar = '0' ";
		$sql_alu = $sql_alu . "ORDER BY alumno.ape_pat, alumno.ape_mat, alumno.nombre_alu; ";
		$resp = pg_exec($conn,$sql_alu);
		return $resp;
	}
	function Datos_empleado_NFP($conn){
		$sql = "SELECT nombre_emp, ape_pat FROM empleado a INNER JOIN dicta b ON a.rut_emp=b.rut_emp WHERE id_ramo=".$this->id_ramo."";
		...
		$this->Docente = $Apellido_emp.".".$Nombre_emp;
		
		return;
	}
	function Promedio_NFP($conn){
		$sql_pl = "select promedio from notas".$this->nro_ano." where id_ramo = '".$this->id_ramo."' and id_periodo = '".$this->periodo."'";
			if($this->rut_alumno != 0){
				$sql_pl= $sql_pl . " AND rut_alumno = '".$this->rut_alumno."'";
			}
		$resp = pg_Exec($conn,$sql_pl);
		
		return $resp;
	}
	function Id_Ramos_NFP($conn){
		$sql_ramo = "SELECT ramo.id_ramo FROM ramo  WHERE (((ramo.id_curso)=".$this->curso.")) AND ramo.id_ramo in (SELECT id_ramo FROM ";
		$sql_ramo = $sql_ramo." notas".$this->nro_ano." WHERE id_periodo = '".$this->periodo."') AND ramo.cod_subsector < 50000 ORDER BY ramo.id_orden ";
		$resp = pg_exec($conn,$sql_ramo);
		
		return $resp;
	}	
	function Datos_ramos_NFP($conn){
		...
		$sql_sub = $sql_sub . "WHERE (((ramo.id_curso)=".$this->curso.")) and ramo.id_ramo in (select id_ramo from notas".$this->nro_ano." where id_periodo = '".$this->periodo."') and ramo.cod_subsector < 50000 ORDER BY ramo.id_orden; ";
		
		$resp=pg_exec($conn,$sql_sub);
		
		return $resp;
	}
	function Notax($conn){
		$sql_nota = "SELECT nota".$this->xx." as nota FROM notas".$this->nro_ano." WHERE id_ramo = ".$this->id_ramo." AND id_periodo = '".$this->periodo."'";
		$resp = pg_exec($conn,$sql_nota);
		
		return $resp;	
	}
	function Estadistica_NFP($conn){
		$sql_inf = "SELECT count(*) as promedio FROM notas".$this->nro_ano." WHERE id_ramo = ".$this->id_ramo." AND id_periodo = '".$this->periodo."' AND promedio between 0 and 39";
		$result_inf = @pg_exec($conn,$sql_inf);
		$prom_inf = pg_result($result_inf,0);
		
	
		$sql_media = "SELECT count(*) FROM notas".$this->nro_ano." WHERE id_ramo = ".$this->id_ramo." AND id_periodo = '".$this->periodo."' AND promedio between 40	and 59";
		$result_media = @pg_exec($conn,$sql_media);
		$prom_media = @pg_result($result_media,0);
	
		$sql_mayor = "SELECT count(*) FROM notas".$this->nro_ano." WHERE id_ramo = ".$this->id_ramo." AND id_periodo = '".$this->periodo."' AND promedio between 60 and 70";
		$result_mayor = @pg_exec($conn,$sql_mayor);
		$prom_mayor = @pg_result($result_mayor,0);
		
		$this->prom_inf 	 = $prom_inf;
		$this->prom_media	 = $prom_media;
		$this->prom_mayor	 = $prom_mayor;
	
		return;
	
	}
	function curos_NFP($conn){
		$sql_curso= "SELECT curso.id_curso, curso.grado_curso, curso.letra_curso, tipo_ensenanza.nombre_tipo, curso.ensenanza, curso.cod_decreto ";
		$sql_curso = $sql_curso . "FROM curso INNER JOIN tipo_ensenanza ON curso.ensenanza = tipo_ensenanza.cod_tipo ";
		$sql_curso = $sql_curso . "WHERE (((curso.id_ano)=".$this->ano.")) ";
