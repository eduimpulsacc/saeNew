...
class Reporte { 
	...
	function Periodo($conn){
	   ...
	   /* if ( $this->periodo != "" )   {*/
		...
		$sql="SELECT fecha_inicio,fecha_termino,dias_habiles, nombre_periodo,id_periodo FROM periodo 
	...
	/*	if ( $this->$periodoporano != "" )  {
		$sql="SELECT fecha_inicio,fecha_termino,dias_habiles,nombre_periodo,id_periodo FROM periodo 
		WHERE per.id_ano=".$this->$periodoporano; 
		$Rs_Periodoporano = @pg_exec($conn,$sql);
		$this->result = $Rs_Periodoporano;
  ...
  function RamoAlumno($conn){
  	  $sql = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval, ramo.bool_ip,subsector.nombre_ingles, ramo.cod_subsector, ramo.bool_ip ";
  ...
  function AlumnosSubsector($conn){
	...
	$sql.=" WHERE id_ramo=".$this->ramo.") ORDER BY nro_lista ASC";
	$result = @pg_exec($conn,$sql);
	return $result;
  }
  function Notas($conn){
  ...
  function PromedioRamoCurso($conn){
  	$sql ="SELECT sum(cast (promedio as INTEGER)) as suma, count(*) as contador FROM notas$this->nro_ano WHERE id_ramo=".$this->ramo." AND promedio>0";
	if($this->periodo!=""){
		$sql.="AND id_periodo=".$this->periodo."";
	}
	$result = @pg_exec($conn,$sql);
...
/*	function Matriculados($conn){
	...
	function DocenteProyecto($conn){
		...
		$sql.=" (c.rdb=a.rdb AND c.rut_emp=a.rut_emp AND c.rut_emp=b.rut_emp) INNER JOIN empleado_estudios d ON a.rut_emp=d.rut_empleado AND ";
		$sql.=" b.rut_emp=d.rut_empleado AND c.rut_emp=d.rut_empleado where a.rdb=".$this->rdb." AND c.id_ano=".$this->ano." AND id_proy=".$this->id_pro."";
		$sql.=" AND d.tipo=1 ";
