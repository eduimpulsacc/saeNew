...
public function tablaGrupo($conn,$ano,$ensenanza,$nivel,$subsector){
		   $sql="select * from grupo_nota_curso  where id_ano=$ano and ensenanza=$ensenanza and nivel =$nivel and subsector = $subsector order by orden";
		$result=pg_exec($conn,$sql);
		return $result;
	}
...
public function veMarca($conn,$ano,$ensenanza,$nivel,$subsector,$posicion,$grupo=""){
	 if(strlen($grupo)>0){
	  ...
	  $sql="
select nota$posicion from grupo_nota_curso  where id_ano=$ano and ensenanza=$ensenanza and nivel =$nivel and subsector = $subsector $cad order by nota$posicion desc limit 1";
$result = pg_exec($conn,$sql);
...
public function guardaGrupo($conn,$id_ano,$porcentaje,$nota1,$nota2,$nota3,$nota4,$nota5,$nota6,$nota7,$nota8,$nota9,$nota10,$nota11,$nota12,$nota13,$nota14,$nota15,$nota16,$nota17,$nota18,$nota19,$nota20,$leccionario,$ensenanza,$nivel,$subsector,$orden){
		
	  $sql="insert into grupo_nota_curso(id_ano,porcentaje,nota1,nota2,nota3,nota4,nota5,nota6,nota7,nota8,nota9,nota10,nota11,nota12,nota13,nota14,nota15,nota16,nota17,nota18,nota19,nota20,nombre,ensenanza,nivel,subsector,orden) values($id_ano,$porcentaje,$nota1,$nota2,$nota3,$nota4,$nota5,$nota6,$nota7,$nota8,$nota9,$nota10,$nota11,$nota12,$nota13,$nota14,$nota15,$nota16,$nota17,$nota18,$nota19,$nota20,'$leccionario',$ensenanza,$nivel,$subsector,$orden)";
		$result=pg_exec($conn,$sql);
		return $result;
...
public function PegaGrupo($conn,$ano,$ensenanza,$nivel,$subsector,$curso,$id_ramo){
	 $sql="insert into grupo_nota(id_ano, id_curso,id_ramo, porcentaje,nota1,
...
from grupo_nota_curso where id_ano=$ano and ensenanza=$ensenanza and nivel =$nivel and subsector = $subsector order by orden";
$result=pg_exec($conn,$sql);
		return $result;
	}	
...
public function countGrupoforRamo($conn,$ramo){
		   $sql="select * from grupo_nota where id_ramo=$ramo order by id_grupo";
		$result=pg_exec($conn,$sql);
		return $result;
	}	
