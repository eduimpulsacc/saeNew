...
if($funcion==1){
    ...
    <option value="0">(TODOS LOS TIPO DE ENSE&Ntilde;ANZA)</option>
...
if($funcion==5){
	$rs_listado=$ob_grupo->tablaGrupo($conn,$ano,$ensenanza,$nivel,$asig,$periodo);
	?>
...
if($funcion==11){
	//echo "per->".$per = $periodo;

$rs_listado=$ob_grupo->tablaGrupo($conn,$_ANO,$ensenanza,$nivel,$subsector,$periodo);
...
if($periodo !=0 && $ensenanza!=0 && $nivel!=0 && $subsector!=0){
	
//reviso los cursos que no tienen activado los grupos para activarlos
$ramosingupo = $ob_grupo->ramoGR($conn,$_ANO,$ensenanza,$nivel,$subsector,0,$periodo);
...
$rs_pego = $ob_grupo->PegaGrupo($conn,$_ANO,$ensenanza,$nivel,$subsector,$curso,$id_ramos,$periodo);
...
if(pg_numrows($rs_grupoC)==0){
	$rs_pego = $ob_grupo->PegaGrupo($conn,$_ANO,$ensenanza,$nivel,$subsector,$curso,$id_ramo,$periodo);
	...
	//echo "porcentaje->".
		...
		$cuengrp=0;
			...
			for($nog=1;$nog<=19;$nog++){
				...
				if(strlen($nota)>0 && trim($nota) !='101'){
						 ...
						 $dato[$_ANO][$codEnse][$codNivel][]=$codSub;
						  //recorrer ramo = 
							 ...
							 for($rs=0;$rs<pg_numrows($ramosingupo);$rs++){
								...
								for($p=0;$p<pg_numrows($rs_periodo);$p++){
								$fila_periodo = pg_fetch_array($rs_periodo,$p);
								 echo "\nper->". $idP= $fila_periodo['id_periodo'];
...
//recorre grupos
			...
			for($nog=1;$nog<=19;$nog++){
				...
				$nota = pg_result($rsln,0);
				if(strlen($nota)>0 && trim($nota) !='101' && trim($nota) !=''){
...
if($funcion==12){
	...
	$rs_periodo= $ob_grupo->getPeriodo($conn,$ano);
	?>
    <select name="id_periodo" id="id_periodo" >
    <option value="0">(ANUAL)</option>
    <?php for($e=0;$e<pg_numrows($rs_periodo);$e++){
		$fila_nivel = pg_fetch_array($rs_periodo,$e);
		?>
    <option value="<?php echo $fila_nivel['id_periodo'] ?>"><?php echo $fila_nivel['nombre_periodo'] ?></option>
    <?php }?>
    </select>
    <?
