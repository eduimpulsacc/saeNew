...
public function PegaGrupoAll($conn,$ano,$curso,$id_ramo,$id_periodo,$ensenanza=0,$nivel=0,$subsector=0,$per=0){
	...
	$sql="insert into grupo_nota_ramo_porcentaje(id_ano, id_curso,id_ramo,id_periodo, porcentaje,nota1,
...
nota2, nota3, nota4,nota5,nota6,nota7, nota8,nota9, nota10, nota11, nota12,nota13, nota14, nota15,nota16,nota17,nota18,nota19, nota20,nombre,id_grupo,orden from grupo_nota_porcentaje where id_ano=$ano $en $nv $su $pe order by orden";
$result=pg_exec($conn,$sql);
 		return $result;
	...
	public function countGrupoforRamoPer($conn,$ramo,$per){
		 $sql="select * from grupo_nota_ramo_porcentaje where id_ramo=$ramo and id_periodo=$per order by id_grupo";
		$result=pg_exec($conn,$sql);
		return $result;
	}


public function tablaGrupoFrom($conn,$from){
		 $sql="select * from grupo_nota_ramo_porcentaje where copied_from =$from order by id_curso,id_periodo";
		$result=pg_exec($conn,$sql);
		return $result;
	}
