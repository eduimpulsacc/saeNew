...
public function __construct($con){ 
   ...
   $regis = @pg_Exec($this->conect,$sql) or die( "Error bd select 1".$sql);		
	...
	public function guardaConCal($rdb,$nombre,$sigla){
		 $sql="insert into velocidad_lectora_calectora(rdb,nombre,sigla) values($rdb,'$nombre','$sigla')";
		$regis = pg_Exec($this->conect,$sql) or die( "Error bd Update vel".$sql);	
		if ($regis){
				return true;
			}else{
	...
	public function listaConCal($rdb){
		 $sql="select * from velocidad_lectora_calectora where rdb=$rdb order by id_concepto";
		 $regis = @pg_Exec($this->conect,$sql) or die( "Error bd select 1".$sql);		
		 if($regis){
				   return $regis;
			}else{
	...
	public function quitaConCal($concepto){
		 $sql="delete from velocidad_lectora_calectora where id_concepto=$concepto";
		 $regis = @pg_Exec($this->conect,$sql) or die( "Error bd select 1".$sql);		
		 if($regis){
				   return $regis;
			}else{
	...
	public function ConCalUno($concepto){
		 $sql="select * from velocidad_lectora_calectora where id_concepto=$concepto";
		 $regis = @pg_Exec($this->conect,$sql) or die( "Error bd select 1".$sql);		
		 if($regis){
				   return $regis;
			}else{
	...
	public function modCalUno($concepto,$nombre,$sigla){
		 $sql="update velocidad_lectora_calectora set nombre='$nombre',sigla='$sigla' where id_concepto=$concepto";
		 $regis = @pg_Exec($this->conect,$sql) or die( "Error bd select 1".$sql);		
		 if($regis){
				   return $regis;
			}else{
