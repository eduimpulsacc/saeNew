...
INNER JOIN tipo_ensenanza te ON te.cod_tipo=c.ensenanza
 where id_nacional=1 and cod_tipo > 10";
	  ...
	  $arr_mat_mes1= array();
	   ...
	   foreach ($instituciones as $institucion): 
		  ...
		  <tr>
			...
			$matricula = alumnos_promovidos($id_ano,$conn);
			$reprobado = alumnos_reprobados($id_ano,$conn);
			...
			<td align="right" class="text2"><?=$reprobado;?></td>
			<? 
			$tot_instit = $tot_instit + $matricula; 
			$tot_repro	= $tot_repro + $reprobado;
			...
			for($j=0;$j<@pg_numrows($rs_ensenanza);$j++){
				$fila_e = pg_fetch_array($rs_ensenanza,$j);
				$ensenanza = alumnos_reprobados_ensenanza($id_ano,$conn,$fila_e['ensenanza']);
	...
	<? endforeach ?>	  
	  ...
	  <tr>
		...
		<td align="right" class="celdas2"><?=$tot_repro;?></td>
        <? 
        for($j=0;$j<@pg_numrows($rs_ensenanza);$j++){
			$fila_e = pg_fetch_array($rs_ensenanza,$j);				
		?>
