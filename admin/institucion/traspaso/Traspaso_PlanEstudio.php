<?	require('../../../util/header.inc');
	...
	$sql = "SELECT cod_decreto,cursos,nombre_decreto, cod_plan, rdb, tipo_ense, fecha_decreto FROM plan_estudio WHERE rdb=".$_INSTIT." ";
	...
	if($num < $total_filas){
		...
		$sql = " INSERT INTO plan_estudio (cod_decreto,cursos,nombre_decreto, cod_plan, rdb, tipo_ense, fecha_decreto) VALUES ";
		$sql.= " ('".$fila['cod_decreto']."','".$fila['cursos']."','".$fila['nombre_decreto']."','".$fila['cod_plan']."','".$fila['rdb']."', ";
		$sql.= " '".$fila['tipo_ense']."','".$fila['fecha_decreto']."')";
		$rs_destino = @pg_exec($_DESTINO,$sql);		
	}else{
		echo "<script>window.location = 'Traspaso_CursoPlan.php' </script>";	
	}
...
<table width="700" border="0" class="celdas3" align="center" >
  ...
  <tr>
	...
	<script> setTimeout("window.location='Traspaso_PlanEstudio.php?num=<? echo $num; ?>'");</script>
</body>
</html>
