<?	require('../../../util/header.inc');
	...
	$sql = "SELECT cod_decreto,cod_subsector FROM incluye_propio WHERE rdb in (SELECT rdb FROM plan_estudio WHERE rdb=".$_INSTIT.")";
	...
	if($num < $total_filas){
		...
		$sql = "INSERT INTO incluye_propio (cod_decreto,cod_subsector) VALUES('".$fila['cod_decreto']."','".$fila['cod_subsector']."')";
		$rs_destino = @pg_exec($_DESTINO,$sql);
		
	}else{
		echo "<script>window.location = 'Traspaso_Curso.php' </script>";
	}
?>
...
<table width="700" border="0" class="celdas3" align="center" >
  ...
  <tr>
	...
	<script> setTimeout("window.location='Traspaso_IncluyePropio.php?num=<? echo $num; ?>'");</script>
</body>
</html>