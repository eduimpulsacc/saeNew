		...
		<table WIDTH="600" BORDER="0" CELLSPACING="1" CELLPADDING="3">
			<tr>
				<td colspan=5 align=right>
					<?php if(($_PERFIL!=4)&&($_PERFIL!=19)&&($_PERFIL!=20)&&($_PERFIL!=6)&&($_PERFIL!=21)&&($_PERFIL!=22)&&($_PERFIL!=25)&&($_PERFIL!=26)){ //ACADEMICO Y LEGAL?>
					...
					if ($_PERFIL == 2){
					   ?>					
					   <!--INPUT class="botonXX"  TYPE="button" value="VOLVER" onClick=document.location="../seteaInstitucion.php3?caso=4&mdinamico=0"-->
					   <?
					}else{
					   ?>					
					   <!--INPUT class="botonXX"  TYPE="button" value="VOLVER" onClick=document.location="../seteaInstitucion.php3?caso=4&mdinamico=<?=$menu ?>"-->
					   <?
					}
					?>				</td>
			</tr>
			...
			?>
				...
				<?php if($fila['situacion']==0){//CERRADO?>
						<tr bgcolor=#ffffff onmouseover=this.style.background='yellow';this.style.cursor='hand' onmouseout=this.style.background='#ffffff' onClick=go('seteaAno.php3?ano=<?php echo $fila["id_ano"];?>&caso=1&mdi=<?=$menu ?>')>
				<?php }?>
				<?php if($fila['situacion']==1){//ABIERTO
						if (($_PERFIL!=19)&&($_PERFIL!=20)&&($_PERFIL!=21)&&($_PERFIL!=22)&&($_PERFIL!=6)&&($_PERFIL!=2)&&($_PERFIL!=23)&&($_PERFIL!=24)&&($_PERFIL!=25)){?>
