...
<table width="650" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><div align="right">
	  <div id="capa0">
	         <input name="button3" TYPE="button" class="botonXX" onClick="MM_openBrWindow('printNotasParciales_3y4_sc.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>','','scrollbars=yes,resizable=yes,width=770,height=500')" value="IMPRIMIR">
...
<?
	if (empty($alumno))
		$sql_alu = "select * from matricula, alumno where id_curso =" . $curso . " and matricula.rut_alumno = alumno.rut_alumno and bool_ar=0 order by alumno.ape_pat, alumno.ape_mat ";
	else
		$sql_alu = "select * from matricula where rut_alumno ='" . $alumno ."' and id_ano = " . $ano." and bool_ar=0";
...
*/	
				...
				$sql_peri = $sql_peri ." INNER JOIN ramo r on r.id_ramo=n.id_ramo WHERE n.rut_alumno='".trim($alumno)."' ";
...
*/					 $promedio_periodo_aux = $prome_abajo;
	   ...
	   <? if ($_INSTIT==12829){ ?>	<td width="153"><font size="1" face="Arial, Helvetica, sans-serif"> <strong>TOTAL HORAS A TRABAJAR </strong></font></td>
				...
				$gradino = ($fila_gradio['ensenanza']);
				 ...
				 ?>
                  ...
                  <tr>
                    ...
                    <td><div align="center"><font face="Arial, Helvetica, sans-serif" size="1"><? if($_INSTIT==1962){ echo "JEFE UTP"; }else{ echo $cargo_dir2." Establecimiento"; }?><? if ($_INSTIT==9239){ ?> de Ciclo Establecimiento<? }else ?></font></div></td>
                  </tr>
                  <tr>
		...
		echo "Paz Alejandra González Vargas ";
			...
			if ($_INSTIT!=9239 && $_INSTIT!=1962){	echo ucwords(strtoupper(trim($fila['ape_pat']) . " " . trim($fila['ape_mat']) . " " . trim($fila['nombre_emp']))); }
			    $cargo_dir    = $fila['cargo'];	
...
<input name="" type="button" value="Nueva B&uacute;squeda"  class="botonXX" onClick="window.location='NotasParciales_Taller_sc.php?flag=0'">
