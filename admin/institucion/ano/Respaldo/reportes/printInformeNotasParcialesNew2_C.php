  ...
  <?
   ...
   <? if($tipo_eval==1){?>
				...
				for($per=0 ; $per < $tot_periodo ; $per++){				
					...
					if (pg_numrows($result_peri)>0){
						...
						if($tipo_eval==2){
							if($fila_peri['notaap']=="0" or trim($fila_peri['notaap'])==""){
								...
								if (chop($fila_peri['promedio'])=="0" or empty($fila_peri['promedio']) or trim($fila_peri['promedio'])=="" or $fila['bool_ip']==0){
									$prome_1 = "&nbsp;";
								} else {																																																										
									if ($fila_peri['promedio']>0){
										$prome_1 = round($fila_peri['promedio'],0);					
					...
					if($institucion==1914){
						if($periodos==$periodo){ ?>
						<td align="center" class="subitem"><? if($prome_1<40 && $prome_1>0){ ?><strong><font color="#FF0000"><? echo $prome_1 ?></font><? } else { echo $prome_1; } ?></strong></td>
					<? 	}
					...
					<? } 
					 if($tipo_rep==2){
					  	/*	$ob_reporte ->periodo=$periodos;
							...
							$sql ="SELECT sum(cast (promedio as INTEGER)) as suma, count(*) as contador FROM notas$nro_ano WHERE id_ramo=".$id_ramo." AND promedio NOT IN('0','MB','B','S','I',' ')  ";
							if($periodos!=""){
							...
							if ($prom_curso>0){
							    echo "<br>".$suma_prom_curso[$per] = $suma_prom_curso[$per] + $prom_curso;
								$cont_prom_curso++;
							}	
							if($institucion==1914){
								if($periodos==$periodo){ ?>
  ...
  <tr>
   ...
   <? if($tipo_eval==1){?>
				...
				for($per=0 ; $per < $tot_periodo ; $per++){				
					...
					<td align="center" bgcolor="#CCCCCC"><font  size="1" face="Arial, Helvetica, sans-serif"><strong><i><?=$prome_ap;?></i></strong></font></td>
					<? } ?>
					  <? if($tipo_rep==2){echo "dentro if";
					  		$ob_reporte ->periodo=$periodos;
							$ob_reporte	->ramo=$id_ramo;
							$ob_reporte ->PromedioRamoCurso($conn);
							echo "<br>".$prom_curso = intval($ob_reporte->suma / $ob_reporte->contador);
							
							if($prom_curso==0){
								$prom_curso="&nbsp;";
							}
							if ($prom_curso>0){
							    $suma_prom_curso[$per]= $suma_prom_curso[$per] + $prom_curso;
								$cont_prom_curso++;
							}						
							if($institucion==1914){
								if($periodos==$periodo){ ?>
				...
				for($per=0 ; $per < $tot_periodo ; $per++){				
					...
					<? } ?>
					  <? if($tipo_rep==2){
							...
							$ob_reporte ->PromedioRamoCurso($conn);
							if($_PERFIL==0){
								echo "suma-->".$ob_reporte->suma;
								echo "<br> contador-->".$ob_reporte->contador;
							}
...
<TR height="25">
	...
	for($per=0 ; $per < $tot_periodo ; $per++){	
		...
		echo "<br>".$ob_reporte->suma=$ob_reporte->suma;
		echo "<br>".$ob_reporte->contador=$ob_reporte->contador;
...
?>
	...
	if($tipo_rep==2){
		...
		$sql = "SELECT round(avg(cast (promedio as INTEGER))) as suma FROM notas$nro_ano WHERE id_ramo in (select id_ramo from ramo where id_curso=".$curso.") AND promedio NOT IN('0','MB','B','S','I',' ') AND id_periodo=".$periodos."";
		$rs_prom_curso =@pg_exec($conn,$sql);
		$prom_curso = @pg_result($rs_prom_curso,0);
		if($institucion==1914){
			if($periodos==$periodo){?>
