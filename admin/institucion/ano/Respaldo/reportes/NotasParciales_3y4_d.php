...
<SCRIPT language="JavaScript">
			...
			if (form.cmb_curso.value!=0){
				...
				form.action = 'NotasParciales_3y4_d.php?institucion=$institucion';
				form.submit(true);
...
<?php 
	...
	else
	{
		if (pg_numrows($result1)!=0)
	  {
		  ...
		  $fecha_termino = $fila1['fecha_termino'];
		  if (!$fila1)
		  {
			  error('<B> ERROR :</b>Error al acceder a la BD. (PERIODOS)</B>');
			  exit();
...
// AQUI CALCULO CUANTAS HORAS DE CLASE TIENE EL PERÍODO	
$horas_semanales = 0;
for ($dia=0; $dia < 7; $dia++){	
...
<table width="650" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><div align="right">
	  <div id="capa0">
	    <input name="button3" TYPE="button" class="botonXX" onClick="MM_openBrWindow('printNotasParciales_3y4_d.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>','','scrollbars=yes,resizable=yes,width=770,height=500')" value="IMPRIMIR">
	  </div>
    </div></td>
  </tr>
</table> 
...
for($cont_paginas=0 ; $cont_paginas < $cont_alumnos ; $cont_paginas++)
{
	...
	//---------------------------
	$sql = "SELECT ano_escolar.nro_ano, curso.cod_eval, curso.grado_curso, curso.truncado_per, curso.letra_curso, curso.cod_decreto ,tipo_ensenanza.nombre_tipo, curso.cod_decreto, alumno.ape_pat, alumno.ape_mat, alumno.nombre_alu,institucion.nombre_instit, curso.ensenanza  ";
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
	  ...
	  if($institucion!=""){
		   ...
		   echo "<img src='".$d."menu/imag/logo.gif' >";
			...
			<? 	if($institucion=="770"){	
				$sql_pstudio = "select cursos, nombre_decreto from plan_estudio where cod_decreto = '$cod_decreto'";
				$res_pstudio = pg_Exec($sql_pstudio);
				$fila_pstudio = pg_fetch_array($res_pstudio);
				$plan_estudio = $fila_pstudio['nombre_decreto']."(".trim($fila_pstudio['cursos']).")";?>
              <tr>
                <td><div align="left"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>PLAN DE ESTUDIO</strong></font></div></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>:</strong></font></td>
                <td><div align="left"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><?= $plan_estudio?></font></div></td>
              </tr>
              <tr>
                <td><div align="left"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>DECRETO DE EVALUACION</strong></font></div></td>
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>:</strong></font></td>
                <td><div align="left"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><?=$cod_eval?></font></div></td>
              </tr>				  				
			<?	}else{	?>
              <tr>
                <td>&nbsp;</td>
      ...
      <tr>
        <td height="20" class="tableindex"><div align="center">INFORME  DE NOTAS PARCIALES</div></td>
      </tr>
      <tr>
      ...
      <tr>
		...
		<?
			...
			if ($_INSTIT==516){
				...
				<?  } ?>
				  ...
				  <? } 
					...
					if(((trim($fila1['nombre_periodo'])=="SEGUNDO TRIMESTRE") AND  $_INSTIT==2278) OR (trim($fila1['nombre_periodo'])=="SEGUNDO SEMESTRE")  and  ($_INSTIT==2278)){  ?>
						 <td align="center"><font size="1" face="Arial, Helvetica, sans-serif"><strong>Prom. Curso</strong></font></td>			         
				  <? }   
		  ...
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene$nro_ano.rut_alumno)='".$alumno."')) and ramo.cod_subsector < 50000 order by ramo.id_orden; ";
...
//		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.")); ";
		  ...
		  for ($e=0 ; $e < @pg_numrows($result) ; $e++){
	        ...
	        for($per=0 ; $per < $tot_periodo ; $per++){				
			   ...
			   if ($_INSTIT==516 or $_INSTIT==2278 and $per==1){  
					/// consulta para determinar el promedio del curso por subsector
					$sql_peri = "SELECT notas$nro_ano.promedio ";
					$sql_peri = $sql_peri . "FROM notas$nro_ano WHERE (id_ramo = ".$id_ramo." and id_periodo = ".$periodos.") order by id_periodo ";
					$result_peri = @pg_Exec($conn,$sql_peri);
...
*/	
				...
				if ($_INSTIT==516 or $_INSTIT==2278){
				    ...
				    /// no calculo el promedio
				}		
				
				?>
				
				<?
				if (($_INSTIT==516 or $_INSTIT==2278) and ($per==0)){ ?>						
				    <td align="center"><font size="2" face="Arial, Helvetica, sans-serif"><b><? if($prome_abajo>0) echo $prome_abajo; else  echo "&nbsp;"; ?></b></font></td>								
			 <?	}     
				
				if (($_INSTIT==516 or $_INSTIT==2278) and ($per==1) and ($promdeio_curso_subsector3!=0)){ ?>						
		...
		<table width="650" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<td width="35%"><font size="1" face="Arial, Helvetica, sans-serif"><? if ($_INSTIT==12829) { ?><strong>TOTAL DIAS SEMESTRE </strong><? } else  ?> <? if ($_INSTIT==770){ ?>&nbsp; <? }else{ ?> <strong>TOTAL DIAS PERIODO </strong></font></td><? } ?>
			<td width="15%"><font size="1" face="Arial, Helvetica, sans-serif"><? if ($_INSTIT==770){ ?>&nbsp; <? }else{ ?> <? echo $dias_habiles ?> <? } ?></font></td>
			<td width="35%"><font size="1" face="Arial, Helvetica, sans-serif"><strong><? if ($_INSTIT==770){ ?><!--TOTAL DIAS INASISTENTES --><? } else{ ?> TOTAL DIAS INASISTENTES <? } ?></strong></font></td>
			<td width="15%"><font size="1" face="Arial, Helvetica, sans-serif"><? if ($_INSTIT==770){ ?> &nbsp; <? } else{ ?><? echo $inasistencia ?><? } ?></font></td>
		  </tr>
		  <tr>
		  ...
		  <?
		  if ($_INSTIT==770){
			  $horas_periodo = round(($dias_habiles / 5) * $horas_semanales);
			  /// aqui determino cuantas inacistencia ha tenido en el mes
			  $sql_inasis = "select * from inasistencia_asignatura where rut_alumno = '".trim($alumno)."' and ano = '".$ano."' and fecha > '$fecha_inicio' and fecha < '$fecha_termino'";
			  $res_inasis = @pg_Exec($conn, $sql_inasis);
			  $num_inasis = @pg_numrows($res_inasis);
			 
			  $porcentajehoras = @round($num_inasis*100 / $horas_periodo);
			  $porcentajehoras = 100 - $porcentajehoras;
			  ?>  
			  
			  <tr>
				<td><font size="1" face="Arial, Helvetica, sans-serif">ASISTENCIA HORAS DE CLASE (%)</font></td>
				<td><font size="1" face="Arial, Helvetica, sans-serif"><?=$porcentajehoras ?> %</font></td>
				<td><font size="1" face="Arial, Helvetica, sans-serif">HORAS DE CLASE</font></td>
				<td><font size="1" face="Arial, Helvetica, sans-serif"><?=$horas_periodo ?></font></td>
			  </tr>
		<? } ?>	  
		</table>
		    ...
		    <td height="23"><div align="left"><font size="2"><strong><font face="Verdana, Arial, Helvetica, sans-seri">________________________________________________________________________</font></strong></font></div></td>
			   ...
			   ?>		
		        ...
		        <table width="650" border="0" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td><div align="center"><font face="Arial, Helvetica, sans-serif" size="1"><? if ($institucion==2278){ echo $nombre_profe; }else{ ?>  Profesor(a) Jefe <? } ?></font></div></td>
                    <td><div align="center"><font face="Arial, Helvetica, sans-serif" size="1"><? if ($institucion==2278){  echo ucwords(strtoupper(trim($fila['ape_pat']) . " " . trim($fila['ape_mat']) . " " . trim($fila['nombre_emp']))); }else{ ?>  <?= $cargo_dir2 ?> Establecimiento  <? } ?></font></div></td>
                  </tr>
                  <tr>
                    <td><div align="center"><strong><font face="Arial, Helvetica, sans-serif" size="1">
					      <? if($institucion==8930){
						           Nombre($materno,$nombre,$paterno);
							 }else{
							       if ($institucion==2278){
								        echo "Profesor(a) Jefe";
  ...
  <td><div align="justify"><font face="Arial, Helvetica, sans-serif"><strong><img src="tijera.gif" width="32" height="16">- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - </strong></font></div></table>
      ...
      <tr>
		...
		<?
          ...
          <? } 
			...
			if(((trim($fila1['nombre_periodo'])=="SEGUNDO TRIMESTRE") AND $_INSTIT==516 AND $_INSTIT==2278) OR (trim($fila1['nombre_periodo'])=="SEGUNDO SEMESTRE") and ($_INSTIT==516 or $_INSTIT==2278)){  ?>
			     <td align="center"><font size="1" face="Arial, Helvetica, sans-serif"><strong>Prom. Curso</strong></font></td>			         
          <? } 
...
*/					 $promedio_periodo_aux = $prome_abajo;
				...
				<?
				 ...
				 if (($_INSTIT!=516 and $_INSTIT!=2278) and ($promedio_periodo_aux!=NULL)){ ?>
			        <td align="center"><font size="1" face="Arial, Helvetica, sans-serif"><? if($prome_abajo>0) echo $prome_abajo; else  echo "&nbsp;"; ?></font></td>	
			 <? }		
...
<HR width="100%" color=#003b85>
		<table width="650" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td width="153"><font size="1" face="Arial, Helvetica, sans-serif"> <? if ($_INSTIT==12829) { ?><strong>TOTAL DIAS SEMESTRE </strong><? } else  ?> <? if ($_INSTIT==770){ ?>&nbsp; <? }else{ ?><strong>TOTAL DIAS PERIODO </strong></font></td> <? } ?> 
...
<input name="" type="button" value="Nueva B&uacute;squeda"  class="botonXX" onClick="window.location='NotasParciales_Taller.php?flag=0'">
