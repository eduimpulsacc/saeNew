...
<SCRIPT language="JavaScript">
			...
			if (form.cmb_curso.value!=0){
				...
				form.action = 'NotasParciales_3y4.php?institucion=$institucion';
				form.submit(true);
...
<table width="650" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><div align="right">
	  <div id="capa0">
	         <input name="button3" TYPE="button" class="botonXX" onClick="MM_openBrWindow('printNotasParciales_3y4.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>','','scrollbars=yes,resizable=yes,width=770,height=500')" value="IMPRIMIR">
	   
	  
		   <!--  <input name="button4" TYPE="button" class="botonXX" id="button4" onClick="MM_goToURL('parent','printNotasParciales_3y4.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>');return document.MM_returnValue" value="IMPRIMIR"> -->
	  </div>
    </div></td>
  </tr>
</table> 
...
<?
	if (empty($alumno))
		$sql_alu = "select * from matricula$nro_ano, alumno where id_curso =" . $curso . " and matricula$nro_ano.rut_alumno = alumno.rut_alumno and bool_ar=0  order by alumno.ape_pat, alumno.ape_mat ";
	else
		$sql_alu = "select * from matricula$nro_ano where rut_alumno ='" . $alumno ."' and id_ano = " . $ano." and bool_ar=0";
      ...
      <tr>
		...
		<?
		  ...
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector and ramo.cod_subsector < 50000) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".trim($curso).") AND ((tiene$nro_ano.rut_alumno)='".trim($alumno)."')) order by ramo.id_orden; ";
		  //echo $sql2;
...
//		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.")); ";
		  ...
		  <?
			...
			?>
            <td height="25"><div align="left"><font size="1" face="Arial, Helvetica, sans-serif"><? if((trim($fila['nombre'])=="RELIGION") && ($institucion==9239)){ echo $fila['nombre']."(optativo)"; }else{ echo $fila['nombre'];  }?></font></div></td>
...
*/	
				...
				$sql_peri = $sql_peri ." INNER JOIN ramo r on r.id_ramo=n.id_ramo WHERE n.rut_alumno='".trim($alumno)."' and r.cod_subsector < 50000 ";
				$sql_peri = $sql_peri ." AND n.id_periodo=".$periodos." and r.bool_ip=1 ";
				*/
				$sql_peri = "select promedio from notas$nro_ano where id_ramo in (select id_ramo from ramo where id_ramo in (select id_ramo from notas$nro_ano where id_periodo = '".trim($periodos)."' and rut_alumno = '".$alumno."') and cod_subsector < 50000 and bool_ip=1 and id_curso = '".$curso."') and rut_alumno = '".trim($alumno)."' and id_periodo = '".trim($periodos)."'";
				...
				if ($prome_abajo>0){
					if(($institucion==11209)OR($truncado_per==0)){
					    ...
					    //echo "pa: $prome_abajo <br>";
						//echo "ca: $cont_abajo <br>";
						//echo "pa: $prome_abajo <br>";
						//echo "ca: $cont_abajo <br>";
...
*/					 $promedio_periodo_aux = $prome_abajo;
				...
				$gradino = ($fila_gradio['ensenanza']);
				 ...
				 ?>
                  ...
                  <tr>
                    ...
                    <td><div align="center"><font face="Arial, Helvetica, sans-serif" size="1"><?=$cargo_dir2 ?></font></div></td>
                  </tr>
                  <tr>
  ...
  <tr>
   <td>
   <? $fecha = date("d-m-Y");?>
      <font face="Verdana, Arial, Helvetica, sans-seri" size="-1"><? echo ucwords(strtolower($comuna)).", ". fecha_espanol($fecha) ?></font>	</td>
  </tr>
  </table>
  <? if ($_INSTIT!=11209){?>
  <table>
  ...
  <td><div align="justify"><font face="Arial, Helvetica, sans-serif"><strong><img src="tijera.gif" width="32" height="16">- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - </strong></font></div></td>
  </tr>
 </table>
   ...
   <? } ?>
      ...
      <tr>
		...
		<?
		  ...
		  $sql2 = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval, ramo.bool_ip ";
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector and ramo.cod_subsector < 50000) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene$nro_ano.rut_alumno)='".$alumno."') and (ramo.cod_subsector<1000 and ramo.cod_subsector <> 250 and  ramo.cod_subsector !=42 and  ramo.cod_subsector !=742 and  ramo.cod_subsector !=43 and  ramo.cod_subsector !=155 and  ramo.cod_subsector !=395 and  ramo.cod_subsector !=136 or ramo.cod_subsector =3574)) order by ramo.id_orden";
          $result =@pg_Exec($conn,$sql2);
		  if (!$result) 
		  {
			  error('<B> ERROR :</b>Error al acceder a la BD. (SUBSECTOR)</B>');
...
?>
		  ...
		  $sql2 = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval, ramo.bool_ip ";
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector and ramo.cod_subsector < 50000) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene$nro_ano.rut_alumno)='".$alumno."') and ((ramo.cod_subsector>999 AND ramo.cod_subsector!=3574) or ramo.cod_subsector = 250 or ramo.cod_subsector = 42 or ramo.cod_subsector = 742  or ramo.cod_subsector = 43  or ramo.cod_subsector = 136  or ramo.cod_subsector = 395  or ramo.cod_subsector = 155)) order by ramo.id_orden";
          $result =@pg_Exec($conn,$sql2);
		  if (!$result) 
		  {
			  error('<B> ERROR :</b>Error al acceder a la BD. (SUBSECTOR)</B>');
...
<!--            <td height="25" colspan="21" align="right"><font size="1"><strong><font face="Arial, Helvetica, sans-serif">Promedio de <? //echo ucwords(strtolower($fila1['nombre_periodo']))?></font></strong></font></td> -->
	        ...
	        for($per=0 ; $per < $tot_periodo ; $per++){
				...
				*/
				...
				/*
				$sql_peri = "SELECT notas$nro_ano.promedio ";
				$sql_peri = $sql_peri . "FROM notas$nro_ano WHERE (((notas$nro_ano.rut_alumno)='".$alumno."')  and id_periodo = ".$periodos." ) and promedio > 1 order by id_periodo; ";
				*/	
				/*
				$sql_peri = "SELECT n.promedio FROM notas$nro_ano n ";
				$sql_peri = $sql_peri ." INNER JOIN tiene$nro_ano t on (t.id_ramo=n.id_ramo AND t.rut_alumno=n.rut_alumno) ";
				...
				*/				
				...
				{
					$filapm = @pg_fetch_array($result_peri,$pm);							
					if ($filapm['promedio']>1){
						$prome_abajo = $prome_abajo + $filapm['promedio'];
						$pppp = $filapm['promedio'];
