...
<table width="650" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><div align="right">
	  <div id="capa0">
	         <input name="button3" TYPE="button" class="botonXX" onClick="MM_openBrWindow('printNotasParciales_examen.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>','','scrollbars=yes,resizable=yes,width=770,height=500')" value="IMPRIMIR">
...
//		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.")); ";
		  ...
		  <?
			...
			?>
            <td height="25"><div align="left"><font style="font-size:9px;font-family:Arial, Helvetica, sans-serif"><? echo $fila['nombre']; ?></font></div></td>
...
/*				if (pg_numrows($result_peri)>0){
	            ...
	            <td align="center"><font size="1" face="Arial, Helvetica, sans-serif"><?
				   ...
				   /// para ver en que período estamos
				   if ((trim($fila1['nombre_periodo'])=="SEGUNDO SEMESTRE") and ($per==0) ){
				        /// proceso para calcular el promedio final del subsector
				       ...
				       for($jj=0;$jj<$cuenta_examen;$jj++){
					      ...
					      $sql = "SELECT porc,bool_ap FROM examen_semestral WHERE id_examen=".$fils_nota['id_examen']." ";
						  ...
						  $prome_abajo1 = $prome_abajo1 + $prome_1;
				...
				for($jj=0;$jj<$cuenta_examen;$jj++){
					...
					$aprox_ex = @pg_result($rs_porc,1);
					$nota_porc = $nota_porc + (($nota_ex * $porc)/100);
					if (chop($nota_ex)=="0" or chop($nota_ex)=="")  $nota_ex = "&nbsp;"; else $nota_ex = $nota_ex;
...
*/	
				...
				if ($prome_abajo1>0){
					$prome_abajo1 = intval($prome_abajo1/$cont_abajo1,0);
						
				
					$prome_general_pro = $prome_general_pro + $prome_abajo1;
...
*/					 $promedio_periodo_aux = $prome_abajo;
				...
				if ($per==0){
				        ...
				        /// proceso para calcular el promedio final del subsector
						if($prome_abajo1<40 && $prome_abajo1>0){ ?><strong><font color="#FF0000"><? 
						     echo $prome_abajo1; ?></font><? 
					    }else{ 
						    if($prome_abajo1==0){
							    echo "&nbsp;";
						    }else{
  ...
  <? if ($_INSTIT!=11209 and $_INSTIT!=25269){?>
  <table>
  <tr>
   ...
   <? } ?>
      ...
      <tr>
		...
		<?
		  ...
		  $sql2 = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval, ramo.bool_ip, porc_examen ";
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector and ramo.cod_subsector < 50000) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene$nro_ano.rut_alumno)='".$alumno."') and (ramo.cod_subsector<1000 and ramo.cod_subsector <> 250 and  ramo.cod_subsector !=42 and  ramo.cod_subsector !=742 and  ramo.cod_subsector !=43 and  ramo.cod_subsector !=155 and  ramo.cod_subsector !=395 and  ramo.cod_subsector !=136)) order by ramo.id_orden";
          $result =@pg_Exec($conn,$sql2);
		  if (!$result) 
		  {
			  error('<B> ERROR :</b>Error al acceder a la BD. (SUBSECTOR)</B>');
...
<HR width="100%" color=#003b85>
  ...
  <? if ($institucion!=11209 && $institucion!=25269){ ?>
<table>
  <tr> 
    <td><div align="justify"><font face="Arial, Helvetica, sans-serif"><strong><img src="tijera.gif" width="32" height="16">- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - </strong></font></div></td>
 </tr>
