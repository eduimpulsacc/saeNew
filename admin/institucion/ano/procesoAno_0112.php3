...
*/
	...
	if ($frmModo=="modificar"){
		if (pg_dbname($conn)=="coi_antofagasta"){
			$fechainicio=trim($txtFECHAINI);
			$fechatermino=trim($txtFECHATER);
		} elseif(pg_dbname($conn)=="coi_final")	{
			$fechainicio=fEs2En($txtFECHAINI);
			$fechatermino=fEs2En($txtFECHATER);
		}elseif(pg_dbname($conn)=="coi_corporaciones" || pg_dbname($conn)=="coi_final_vina"){
			$fechainicio=fEs2En($txtFECHAINI);
			$fechatermino=fEs2En($txtFECHATER);
		}	
		...
		if($cantAbiertos==0){//NO EXISTEN AÑOS ACADEMICOS REGISTRADOS ABIERTOS 
			...
			$qry="UPDATE ano_escolar SET nro_ano = ".$txtANO.", fecha_inicio = '".$fechainicio."', situacion = ".$rdSIT.", fecha_termino = '".$fechatermino."' WHERE (((id_ano)=".$_ANO."))";
			$result =pg_Exec($conn,$qry) or die("UPDATE FALLO11 : ".$qry);
			/*if (!$result) {
