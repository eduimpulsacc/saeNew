...
//SELECCIONA A TODOS LOS ALUMNOS NO REPITENTES DEL AÑO ANTERIOR
	...
	/*$sqlProm = "select distinct(matricula.rut_alumno), matricula.id_curso
 FROM matricula WHERE matricula.id_ano=".$anterior.".  and bool_ar<>1";*/
 
 	$sqlProm="SELECT  DISTINCT(matricula.rut_alumno), matricula.id_curso FROM matricula INNER JOIN curso ON matricula.id_curso=curso.id_curso WHERE ensenanza>10 AND matricula.id_ano=".$anterior." AND bool_ar<>1  ORDER BY id_curso";
...
//Entra solo si el alumno esta en 4to medio y su situacion final es Aprobado
					if ((($filaCurso['ensenanza']==310)or($filaCurso['ensenanza']==410)or($filaCurso['ensenanza']==510)or($filaCurso['ensenanza']==610)or($filaCurso['ensenanza']==710)or($filaCurso['ensenanza']==810)) and ($filaCurso['grado_curso']==4)) 		
					{
						$var=1;
					}
...
//Entra solo si es de basica, y el alumno esta en 8vo basico, y se encuentra Aprobado
					...
					if (($filaCurso['ensenanza']==110) and ($filaCurso['grado_curso']==8) ) {
					if($tipo_ense==2){	//vel	
...
//ENTRA SI EL ALUMNO ESTA APROBADO Y EL CURSO ES MENOR K 4 MEDIO 	
					if ((($filaCurso['ensenanza']==310)or($filaCurso['ensenanza']==360)or($filaCurso['ensenanza']==410)or($filaCurso['ensenanza']==510)or($filaCurso['ensenanza']==610)or($filaCurso['ensenanza']==710)or($filaCurso['ensenanza']==810)) and ($filaCurso['grado_curso']<4) ) {
							$letra= $filaCurso['letra_curso'];
							$ense= $filaCurso['ensenanza'];
							$grado= $filaCurso['grado_curso']+1;
...
//SI NO EXISTE EL NIVEL DE CURSO SIGUIENTE, NO SE MATRICULA.
					...
					if(@pg_numrows($resultMat)==0){
							...
							$num = $numero + 1;
							if($var==0){
							echo "<br>ensenanza-->".$ense;
echo "<br> INSERT TODOS-->".$qryI="INSERT INTO MATRICULA(RUT_ALUMNO,RDB,ID_ANO,ID_CURSO,FECHA,NUM_MAT,BOOL_BAJ,BOOL_BCHS,BOOL_AOI,BOOL_RG,BOOL_AE,BOOL_I,BOOL_GD,BOOL_AR,NRO_LISTA) VALUES (".$filaProm['rut_alumno'].",".trim($_INSTIT).",".$ano.",'$curso','$fecha',0,0,0,0,0,0,0,0,0,$num)";
...
//TERMINO NORMAL DE TRASPASO MATRICULA.
		...
		echo "<br>TODOS LOS ALUMNOS--> ".$sqlAlum="SELECT  DISTINCT(matricula.rut_alumno), matricula.id_curso FROM matricula INNER JOIN curso ON matricula.id_curso=curso.id_curso WHERE ensenanza=10 AND matricula.id_ano=".$anterior."  ORDER BY id_curso";
		...
		for ($j=0 ; $j<pg_numrows($Rs_Alum) ; $j++){
				...
				if (($filaCurso['ensenanza']==10) and ($filaCurso['grado_curso']<5)) {
					...
					echo "<br>CURSO NUEVO--> ".$sqlCursoNuevo= "select id_curso from curso where grado_curso=".$grado." and letra_curso='".$letra."' and ensenanza=".$ense." and id_ano=".$ano;
					$resultCursoNuevo= @pg_Exec($conn,$sqlCursoNuevo);
			    ...
			    if (($filaCurso['ensenanza']==10) and ($filaCurso['grado_curso']==5)) {
					...
					echo "<br>PASA A PRIMERO--> ".$sqlCursoNuevo= "select id_curso from curso where grado_curso=".$grado." and letra_curso='".$letra."' and ensenanza=110 and id_ano=".$ano;
					$resultCursoNuevo= @pg_Exec($conn,$sqlCursoNuevo);
				...
				echo "<br>EXISTE--> ".$sqlMat = "select rut_alumno from matricula where rut_alumno=".$fila_Alum['rut_alumno']." and id_ano=".$ano;
				$resultMat = @pg_Exec($conn,$sqlMat);
				
				if(@pg_numrows($resultMat)==0){
					if($var==0){
					echo "1".	$qryI="INSERT INTO MATRICULA (RUT_ALUMNO,RDB,ID_ANO,ID_CURSO,FECHA,NUM_MAT,BOOL_BAJ,BOOL_BCHS,BOOL_AOI,BOOL_RG,BOOL_AE,BOOL_I,BOOL_GD,BOOL_AR) VALUES (".$fila_Alum['rut_alumno'].",".trim($_INSTIT).",".$ano.",'$curso','$fecha',0,0,0,0,0,0,0,0,0)";
						$resultI =@pg_Exec($conn,$qryI);
...
//		$ano = $filaNuevo['anonuevo'];
	...
	if (!$resultSql){
		error('<B> ERROR :</b>Error al acceder a la BD. (102)</B>'.$sql);
			...
			if($max > 1){
	 		...
	 		$sqlProm = "select distinct(matricula.rut_alumno), matricula.id_curso FROM matricula WHERE matricula.id_ano=".$filaSql1['id_ano']."  and bool_ar<>1";
			$resultProm = pg_Exec($conn,$sqlProm);
	...
	/* hasta aki */
		...
		for ($j=0 ; $j<pg_numrows($Rs_Alum) ; $j++){
			...
			if(pg_numrows($resultMat)==0){
					if($var==0){
						echo "2".$qryI="INSERT INTO MATRICULA (RUT_ALUMNO,RDB,ID_ANO,ID_CURSO,FECHA,NUM_MAT,BOOL_BAJ,BOOL_BCHS,BOOL_AOI,BOOL_RG,BOOL_AE,BOOL_I,BOOL_GD,BOOL_AR) VALUES (".$fila_Alum['rut_alumno'].",".trim($_INSTIT).",".$ano.",'$curso','$fecha',0,0,0,0,0,0,0,0,0)";
						$resultI =pg_Exec($conn,$qryI);
