...
//}
	...
	if($caso==1){//INGRESAR ASISTENCIA
		...
		if($_INSTIT==10774){
		$pag="asistenciap.php3?cmbMes=".$mes;
		}else{
$pag="asistencia.php3?cmbMes=".$mes;
}
	};
	...
	if($caso==12){//INGRESAR ASISTENCIA  DE APODERADOS
		...
		$pag="asistencia_apo.php?cmbMes=".$mes."&tpv=".$_GET['tpv'];
	};
	...
	if($caso==13){//MOSTRAR ASISTENCIA
		...
		$pag="asistencia_apo.php?tpv=".$_GET['tpv'];
	};
	...
	/************ PERMISOS DEL PERFIL *************************/
		...
		$pag="Asistencia_Sige/asistencia_sige.php";
	};
