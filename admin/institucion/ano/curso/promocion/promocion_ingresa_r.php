...
<table width="100%" border="0" cellspacing="1" cellpadding="1">
 ...
 <? 
	...
	$sql_ramos = $sql_ramos . "and tiene$nro_ano.id_ramo = ramo.id_ramo and ramo.bool_ip = 1  AND ramo.cod_subsector=subsector.cod_subsector ORDER BY id_orden ASC";
...
<?   
    ...
    for($cont_sub=0 ; $cont_sub < $cont_ramos ; $cont_sub++){
		...
		for($b=0; $b<$cuenta_periodo; $b++){
			...
			$sql = "SELECT promedio FROM notas$nro_ano WHERE id_ramo=".$ramo." AND rut_alumno=".$alumno." AND id_periodo=".$fila_per['id_periodo'];
