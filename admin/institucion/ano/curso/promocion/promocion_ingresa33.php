...
?>
	...
	$sql_promsub = "SELECT promedio FROM promedio_sub_alumno WHERE id_ramo=".$ramo." AND rut_alumno=".$alumno." AND id_ano=".$ano." AND rdb=".$institucion;
	$rs_promsub = @pg_exec($conn,$sql_promsub);
	$prom_sub_fin = @pg_fetch_array($rs_promsub,0);
	
	?>
	<input name="prom_sub<?=$contador?>" id="prom_sub<?=$contador?>" type="text" size="3" maxlength="2" value="<? if($rs_promsub!=0){echo $prom_sub_fin['promedio'];}else{ echo $prom_final;}?>" onClick="funcion_dav('<?=$var_ini?>','<?=$var_fin?>','<?=trim($alumno)?>','<?=$truncado_final?>');" onBlur="funcion_dav('<?=$var_ini?>','<?=$var_fin?>','<?=trim($alumno)?>','<?=$truncado_final?>');"></div></td>
	</tr>
...
<? 
  ...
  <tr class="cuadro01" >
	...
	<?
	$sql_promfin = "SELECT promedio FROM promocion WHERE id_curso=".$curso." AND rut_alumno=".$alumno." AND id_ano=".$ano." AND rdb=".$institucion;
	$rs_promfin = @pg_exec($conn,$sql_promfin);
	$prom_fin = @pg_fetch_array($rs_promfin,0);
	?>
            <input type="text" name="campo_<?=trim($alumno)?>" id="campo_<?=trim($alumno)?>" size="3" maxlength="2" value="<? if($rs_promfin!=0){echo $prom_fin['promedio'];}else{echo ($promedio_general);}?>">    
    </div></td>
    <td align="center" ><?
