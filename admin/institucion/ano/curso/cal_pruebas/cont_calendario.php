...
if($funcion==1){
    ...
    <select name="cmb_curso" id="cmb_curso" onchange="ramo(this.value)">
    <option value="0">Seleccione curso</option>
    <?
for($c=0;$c<pg_numrows($rs_cur);$c++){
	$fila_cur = pg_fetch_array($rs_cur,$c);
	?>
     <option value="<?php echo $fila_cur['id_curso'] ?>" <?php echo ($fila_cur['id_curso']==$_CURSO)?"selected":"" ?>><?php echo $fila_cur['curso'] ?></option>
    <?
}
?>
 </select>
...
if($funcion==2){
	$rs_act = $ob_calendario->cargaAct($conn,$ramo);
...
$(document).ready(function(){
	...
	$("#add-event-form").dialog({
		...
		buttons: {
			'Add Event': function() {
				...
				var desc = jQuery.trim($("#trabajo").val());
				var mate = jQuery.trim($("#materiales").val());
			
				if(what == ""){
					alert("Ingrese nombre de la actividad.");
				}
				else if(desc == ""){
					alert("Desde describir trabajo.");
					$("#descripcion").focus();
				}
		...
		buttons: {		
			...
			'Editar': function() {
				if(clickAgendaItem != null){
						...
						modificaEvento(data['id']);
						
					}
				//alert("Make your own edit screen or dialog!");
			},
			'Eliminar': function() {
				if(confirm("Seguro que desea eliminar prueba?")){
					if(clickAgendaItem != null){
	...
	<?php for($a=0;$a<pg_numrows($rs_act);$a++){
		...
		$tip = $ob_calendario->tipoeva1($conn,$fila_act['tipo_evaluacion']);
		$tipo = pg_result($tip,1);
		?>
...
jfcalplugin.addAgendaItem(
	...
	{
		Materiales: "<?php echo $fila_act['materiales'] ?>",
	    Trabajo: "<?php echo $fila_act['trabajo'] ?>",
		"Tipo Evaluación": "<?php echo $tipo ?>",
	...
	<?php }?>
		...
		<div id="add-event-form" title="Crear prueba">
       <script type="text/javascript">
$( document ).ready(function() {
    tipoeva();
});
			...
			<fieldset>
				...
				<table style="width:100%; padding:5px;">
                    ...
                    <tr>
                      <td colspan="5">Tipo Evaluaci&oacute;n</td>
                      <td>                      
                  </tr>
                    <tr>
                      <td colspan="5"><span id="tipoe"><select name="teva" id="teva">
    <option value="0">Seleccione...</option>
    </select></span>
                          <img src="../../../../clases/img_jquery/iconos/Web-Application-Icons-Set/PNG-24/Add.png" width="24" height="24" onclick="ntipo()" /></td>
                      <td>                      
                 <!-- </tr>
				  ...
				  <tr>
					  <td colspan="5"><input name="descripcion" type="hidden" id="descripcion"  size="30" value="descripcion" /></td>
					  <td>                      
				  </tr>
					<tr>
					...
					<tr>
					  <td colspan="5"><textarea name="trabajo" cols="30" rows="5" id="trabajo"></textarea></td>
					  <td>                      
				  </tr>
					<tr>
					...
					<tr>
					  <td colspan="5"><textarea name="materiales" cols="30" rows="5" id="materiales"></textarea></td>
					  <td>                      
				  </tr>			
				</table>
			</fieldset>
		...
		<div id="display-event-form" title="Detalle prueba">	</div>
        <div id="ediact" title="Modificar datos prueba"></div>
		<div id="recu" title="Ingresar tipo evaluaci&oacute;n"></div>		
...
<?
	...
	//var_dump($_POST);
	$rs_guarda = $ob_calendario->guardaAct($conn,$curso,$nombre,CambioFE($fecha_inicio),CambioFE($fecha_termino),$hora_inicio,$hora_termino,$descripcion,$ramo,$teva,$trabajo,$materiales);
}
if($funcion==4){
//var_dump($_POST);
...
$tip = $ob_calendario->tipoeva1($conn,$fila_act['tipo_evaluacion']);
$tipo = pg_result($tip,1);
$rs_tipoeva = $ob_calendario->tipoeva($conn);
?>
<script>
$(document).ready(function(){
			...
			<fieldset>
				...
				<table style="width:100%; padding:5px;">
                    ...
                    <tr>
                      <td colspan="5"><span id="tipoe"><select name="teva" id="teva">
    <option value="0">Seleccione...</option>
    <?php for($e=0;$e<pg_numrows($rs_tipoeva);$e++){
		$fi=pg_fetch_array($rs_tipoeva,$e);
		?>
     <option value="<?php echo $fi['id_tipo'] ?>" <?php echo($fi['id_tipo']==$fila_act['tipo_evaluacion'])?"selected":"";  ?>><?php echo $fi['glosa'] ?></option>
    <?php }?>
    </select></span>
                          <img src="../../../../clases/img_jquery/iconos/Web-Application-Icons-Set/PNG-24/Add.png" width="24" height="24" onclick="ntipo()" /></td>
                      <td>                      
                  </tr>
                   <!--<tr> <td colspan="5">Descripci&oacute;n</td>
					  <td>                      
				  </tr>-->
					<tr>
					  <td colspan="5"><input name="descripcion" type="hidden" id="descripcion2" value="<?=$fila_act['descripcion']?>" size="30" /></td>
					  <td>                      
				  </tr>	
                  <tr>
					  <td colspan="5">Trabajo</td>
					...
					<tr>
					  <td colspan="5"><textarea name="trabajo" cols="30" rows="5" id="trabajo"><?=$fila_act['trabajo']?></textarea></td>
					  <td>                      
				  </tr>
					<tr>
					  <td colspan="5">Materiales</td>
					...
					<tr>
					  <td colspan="5"><textarea name="materiales" cols="30" rows="5" id="materiales"><?=$fila_act['materiales']?></textarea></td>
					  <td>                      
				  </tr>					
				</table>
<?
	...
	//var_dump($_POST);
$rs_amodi = $ob_calendario->actAct($conn,$nombre,CambioFE($fecha_inicio),CambioFE($fecha_termino),$hora_inicio,$hora_termino,$descripcion,$idact,$ramo,$teva,$trabajo,$materiales);
//echo($rs_amodi)?1:0;
}
if($funcion==7){
	$rs_ramo = $ob_calendario->traeRamo($conn,$curso);
?>
<!---->
    <select name="cmb_ramo" id="cmb_ramo" onchange="traeEvento(<?php echo $curso ?>,this.value)">
    <option value="0">Seleccione ramo</option>
    <?
for($r=0;$r<pg_numrows($rs_ramo);$r++){
	$fila_ram = pg_fetch_array($rs_ramo,$r);
	?>
     <option value="<?php echo $fila_ram['id_ramo'] ?>" <?php echo ($fila_ram['id_ramo']==$_RAMO)?"selected":"" ?>><?php echo $fila_ram['nombre'] ?></option>
    <?
}
?>
 </select>
<?
	...
	$rs_tipoeva = $ob_calendario->tipoeva($conn);
?>
<select name="teva" id="teva">
    <option value="0">Seleccione...</option>
    <?php for($e=0;$e<pg_numrows($rs_tipoeva);$e++){
		$fi=pg_fetch_array($rs_tipoeva,$e);
		?>
     <option value="<?php echo $fi['id_tipo'] ?>"><?php echo $fi['glosa'] ?></option>
    <?php }?>
    </select>
<?
}if($funcion==9){
$rs_ing= $ob_calendario->ctipoeva($conn,$nombre);
}
?>