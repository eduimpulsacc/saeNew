   ...
   <form action="proceso_informe_curso.php" method="post">
   	<input type="hidden" name="plantilla" value="<? echo $filaPlantilla['id_plantilla'];?>">
			...
			<?	$sqlPeriodo="select * from periodo where id_ano=".$ano." order by nombre_periodo";
				...
				<select name="cmb_periodo" onChange="enviapag(this.form);" id="cmb_periodo">
...
<?	}		// fin if(modificar==1)	?>			  
    ...
    <table width="900" border="1" align="center" class="tabla02" style="border-collapse:collapse">
<tr>
    ...
    <td width="150" align="left">
		<input type="hidden" name="rut_alum_<? echo $j;?>" value="<? echo $filaAlum['rut_alumno'];?>">
	<? echo $filaAlum['ape_pat']."\n".$filaAlum['ape_mat']."\n".substr($filaAlum['nombre_alu'],0,1).".";?></td>
			      	<input type="hidden" name="rut_alum_<? echo $j;?>" value="<? echo $filaAlum['rut_alumno'];?>">
    <?php }?>
  </tr>
  <?	
...
?>
  <tr>
    ...
    <td width="10" rowspan="<?php echo $Total ?>" ><div style="width:10px; word-wrap:break-word; text-align:center"><? echo $filaTraeArea['glosa'];?>&nbsp;</div></td>
    <?
               for($i=0;$i<@pg_numrows($resultTotSubArea);++$i){
						//$Tot_SubArea = @pg_fetch_array($resultTotSubArea, $i);
						...
						?>
      <?php  echo ($i>0)?"<tr>":""; ?>
    <td width="50" rowspan="<?php echo @pg_numrows($resultTraeAreaItem) ?>"><div style="width:10px; word-wrap:break-word; text-align:center"><? echo $filaTraeSubArea['glosa'];?>&nbsp;</div></td>
    <?
             for($countAreaItem=0 ; $countAreaItem<@pg_numrows($resultTraeAreaItem) ; ++$countAreaItem){	
					$filaTraeAreaItem=@pg_fetch_array($resultTraeAreaItem, $countAreaItem);
						?>
	...
	?>  <input name="cmb[<? echo $alumno;?>][<? echo $filaTraeAreaItem['id'];?>][0]" type="text" id="cmb_<? echo $j;?>_<? echo $countAreaItem;?>" value="<? echo $valor;?>" size="10">
