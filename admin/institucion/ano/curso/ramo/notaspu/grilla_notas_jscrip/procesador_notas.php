...
for( $X=1; $X <=$Xreg; $X++ )   {  // FOR 1 recorre cantidad alumnos 
	...
	$sql="SELECT * FROM pu_notas notas$ano_act WHERE RUT_ALUMNO=".trim($_POST[rut_alumno.$X])." AND ID_RAMO=".$_POST[id_ramo.$X]." AND ID_PERIODO=".$_POST[id_periodo.$X];
		$result = @pg_Exec($conn,$sql);
		...
		$nn = 0;
			  ...
			  if (@pg_numrows($result)!=0){    // _#########    ACTUALIZAR REGISTROS    //##########
					   ...
					   $sql="UPDATE pu_notas SET ".$notas." PROMEDIO='".$_POST[$p_promedio]."' WHERE RUT_ALUMNO=".$_POST[rut_alumno.$X]."  AND ID_RAMO=".$_POST[id_ramo.$X]." AND ID_PERIODO=".$_POST[id_periodo.$X]."";
						
			}else{   // _ ###########    INSERTAR REGISTROS    //##########
							
							$sql="INSERT INTO pu_notas (RUT_ALUMNO, ID_RAMO, ID_PERIODO, NOTA1, NOTA2, 
