...
function aprox_entero( $nota ){
	...
	return $prom_nuevo;
        ...
        if (pg_numrows($result)!=0){
           ...
           $truncado = trim($fila10['truncado_pu']);	
		   ...
		   $bool_bloq = trim($fila10['bool_bloq']);/*notas$nro_ano*/
		   $sql_busca = "SELECT * FROM pu_notas n 
...
$sql ="SELECT * FROM pu_notas NOTAS".$nro_ano." WHERE ID_PERIODO=".$id_periodo." AND id_ramo=".$id_ramo.";  ";
	...
	$promedio = pg_result($rs_result,0);
	 /* if( $suma > 0 ) $promedio =  intval( $suma / $contador );
	   ...
	   /*FIN Prueba Nivel*/ 
		 ...
		 /*notas".$nro_ano."*/
		$sql_Update = "UPDATE pu_notas SET promedio='".$promedio."' WHERE rut_alumno=".$fila['rut_alumno']." AND id_ramo=".$fila['id_ramo']." AND id_periodo=".$fila['id_periodo']."";
...
FROM alumno 
LEFT JOIN pu_notas notas".$nro_ano." ON notas".$nro_ano.".rut_alumno= alumno.rut_alumno 
