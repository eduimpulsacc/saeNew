...
//if($_PERFIL==0) var_dump($_REQUEST);
...
location.href = '../../../../periodo/listarPeriodo.php3';
	...
	echo $_PERIODOREAL	=	$periodo_real;
  ...
  echo  $_PERIODORAMO	=	$periodo;
	if(!session_is_registered('_PERIODORAMO')){
		session_register('_PERIODORAMO');
	};
	
	// VERIFICO SI EL PERIODO quiz ESTA CERRADO
	$q1 = "select * from quiz_periodos where id_periodo = ".$periodo."";
	$r1 = pg_Exec($conn,$q1);
...
function cargatable(){
	...
	var id_ano=<?=$ano?>; 
	var nro_ano = <?=$nro_ano?>;
	var periodo_real = <?=$periodo_real?>;
	//var ont = 0; 
	var parametros = "id_periodo="+id_periodo+"&id_ramo="+id_ramo+"&id_ano="+id_ano+"&nro_ano="+nro_ano+"&periodo_real="+periodo_real;
	//alert(parametros);
		...
		<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 align=center>
			...
			$qry="SELECT * FROM ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector WHERE (((ramo.id_ramo)=".$ramo."))";
               ...
               /*	$sql_busca = "SELECT * FROM notas$ano_act n 
                ...
                INNER JOIN  ramo r ON r.id_ramo=n.id_ramo 
                WHERE n.id_ramo=".$ramo." AND n.id_periodo=".$periodo." AND n.periodo_real= $periodo_real  ";
