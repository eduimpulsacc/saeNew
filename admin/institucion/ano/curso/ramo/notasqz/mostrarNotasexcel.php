...
//    header("Content-disposition:inline;filename=Respaldo_notas_$fechahoy$hh$mm$ss.xls");
   ...
   // header("Content-Disposition: attachment; filename=notas_$fechahoy.xls");
	...
	if($aux!=1)	{//HACER LA CONSULTA Y DESPLEGAR EL PRIMER PERIODO
		$qry="SELECT periodo.id_periodo, periodo.nombre_periodo FROM quiz_periodos periodo WHERE (((periodo.id_ano)=".$ano.")) ORDER BY NOMBRE_PERIODO";
		          ...
		          <TR height=20 bgcolor=white>
					  <TD  align=middle colspan=2 align=center>
									...
									<?php
										...
										$qry="SELECT periodo.id_periodo, periodo.nombre_periodo FROM quiz_notas periodo WHERE (((periodo.id_ano)=".$ano.")) order by id_periodo";
										$result =@pg_Exec($conn,$qry);
										if (!$result) 
				                         ...
				                         <?
											...
											for($i=0 ; $i < @pg_numrows($result) ; $i++){
												...
												//NOTAS ALUMNO POR RAMO Y PERIODO
  											    $qry2="SELECT * FROM  quiz_notas notas$nro_ano WHERE (((notas$nro_ano.rut_alumno)=".$fila1['rut_alumno'].") AND ((notas$nro_ano.id_periodo)=".$periodo.") AND ((notas$nro_ano.id_ramo)=".$ramo."))"; 
