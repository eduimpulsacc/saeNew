...
if ($id_taller != NULL or $id_taller != 0){
		$_TALLER=$id_taller;
		session_register('_TALLER');
	}

$viene_de="../../taller.php3";	
if ($viene_de){
		$_VIENEPAG= "../../taller.php3";	
	}
...
echo 'function promedioconceptual(prom_conc){ 
var letra = "x";';
...
var parametros =  "num_nota="+num_nota+"&id_curso="+<?=$curso?>+"&id_taller="+<?=$taller?>+"&id_ano="+<?=$ano?>+"&id_periodo="+<?=$periodo?>;
	 $.ajax({
...
function cargatable(){
	...
	var nro_ano = <?=$nro_ano?>;
	//var ont = 0; 
	var parametros = "id_periodo="+id_periodo+"&id_taller="+id_taller+"&id_ano="+id_ano+"&nro_ano="+nro_ano;
	//alert(parametros);
			...
			if( $("#modo_eval").val() == 2 || $("#modo_eval").val() == 4 || $("#modo_eval").val() == 5){ 
				 if(dato!=""){
				  if( $("#modo_eval").val() == 2 || $("#modo_eval").val() == 4){
				   var patron1 = /(^[iIsSbBaAlL]{1}$)/;
				   var patron2 = /(^[mMaAnNeE][bBlLpP]{1}$)/;
					}
				else if($("#modo_eval").val() == 5){
				   var patron1 = /(^[sSgGnN]{1}$)/;
				   var patron2 = /(^[rRnN][vVoO]{1}$)/;
				 }
				   //alert(patron1);
				   //alert(patron2);
					  if ( patron1.test(dato) == false ){
					...
					n=n.trim();
					  if( n.length!=0 ){
							...
							if ( patron1.test(n) == true ){
								...
								var patron1 = /(^[iIsSbBnNaAlLgGrRmM]{1}$)/;
    ...
    <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0 align=center>
			...
			$qry="SELECT * FROM taller  WHERE (((taller.id_taller)=".$taller."))";
                ...
                $pct_nivel = trim($fila10['pct_examen']);
                $prueba_nivel = trim($fila10['nota_exim']);
              ...
              $sql_busca = "SELECT * FROM notas_taller n 
                ...
                WHERE n.id_taller=".$taller." AND n.id_periodo=".$periodo." AND n.nota20 is not null AND n.nota20<>'0' ";
