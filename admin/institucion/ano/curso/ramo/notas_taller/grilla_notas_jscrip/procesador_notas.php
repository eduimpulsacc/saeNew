...
for( $X=1; $X <=$Xreg; $X++ )   {  // FOR 1 recorre cantidad alumnos 
	...
	$sql="SELECT * FROM notas_taller WHERE RUT_ALUMNO=".trim($_POST[rut_alumno.$X])." AND ID_TALLER=".$_POST[id_taller.$X]." AND ID_PERIODO=".$_POST[id_periodo.$X];
		$result = @pg_Exec($conn,$sql);
		...
		$nn = 0;
			  ...
			  if (@pg_numrows($result)!=0){    // _#########    ACTUALIZAR REGISTROS    //##########
					   ...
					   $sql="UPDATE notas_taller SET ".$notas." PROMEDIO='".$_POST[$p_promedio]."' WHERE RUT_ALUMNO=".$_POST[rut_alumno.$X]."  AND ID_TALLER=".$_POST[id_taller.$X]." AND ID_PERIODO=".$_POST[id_periodo.$X]."";
						
			}else{   // _ ###########    INSERTAR REGISTROS    //##########
							
							$sql="INSERT INTO notas_taller (RUT_ALUMNO, ID_TALLER, ID_PERIODO, NOTA1, NOTA2, 
								  ...
								  VALUES (".trim($_POST[rut_alumno.$X]).",".$_POST[id_taller.$X].",".$_POST[id_periodo.$X].",";
