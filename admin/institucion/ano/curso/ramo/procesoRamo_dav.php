						  ...
						  if ($chktodos==1){
							 ...
							 if (!$result7){ 
								...
								for($i=0 ; $i < @pg_numrows($result7) ; $i++){
									...
									$result2 =@pg_Exec($conn,$qry2) or die(pg_last_error($conn));
										if (!$result2){ 
							 ...
							 echo "<script>window.location = 'listarRamos.php3?plan=".$_PLAN."'</script>"; 
							  ...
							  $qry="INSERT INTO RAMO (ID_CURSO, TIPO_RAMO, COD_SUBSECTOR, MODO_EVAL, SUB_OBLI, BOOL_IP, BOOL_SAR, CONEX, PCT_EXAMEN, NOTA_EXIM, TRUNCADO, ID_ORDEN, PRUEBA_NIVEL, PCT_NIVEL, MODO_EVAL_PNIVEL, PCT_EX_ESCRITO, PCT_EX_ORAL, FORMACION,bool_bloq) VALUES (".$curso.",1,".$codSub.",".$cmbMODO.",".$sub_ob.",".$ip.",".$sar.",".$conEX.",".$txtPCT.",".$txtNEXIM.",".$truncado.",".$newOr .",".$prueba_niv.",".$txtPCTNIV.",".$cmbMODOpruebaNivel.",".$pct_ex_escrito.",".$pct_ex_oral.",1,".$bloq.")";
							  $result =pg_Exec($conn,$qry) or die(pg_last_error($conn)); 
							  if (!$result) {
								  error('<b> ERROR :</b>Error al acceder a la BD. (3)'.$qry);
								  exit;
...
//		  $qry="INSERT INTO RAMO (ID_CURSO, TIPO_RAMO, COD_SUBSECTOR, MODO_EVAL, SUB_OBLI, BOOL_IP, BOOL_SAR, CONEX, PCT_EXAMEN, NOTA_EXIM, TRUNCADO, ID_ORDEN, PRUEBA_NIVEL, PCT_NIVEL, MODO_EVAL_PNIVEL, PCT_EX_ESCRITO, PCT_EX_ORAL) VALUES (".$curso.",1,".$subsector.",".$cmbMODO.",".$sub_ob.",".$ip.",".$sar.",'".$conEX."','".$txtPCT."','".$txtNEXIM."','".$truncado."',".$newOr .",'".$prueba_niv."','".$txtPCTNIV."', '".$cmbMODOpruebaNivel."','".$pct_ex_escrito."','".$pct_ex_oral."')";
    	  $qry="INSERT INTO RAMO (ID_CURSO, TIPO_RAMO, COD_SUBSECTOR, MODO_EVAL, SUB_OBLI, BOOL_IP, BOOL_SAR, CONEX, PCT_EXAMEN, NOTA_EXIM, TRUNCADO, ID_ORDEN, PRUEBA_NIVEL, PCT_NIVEL, MODO_EVAL_PNIVEL, PCT_EX_ESCRITO, PCT_EX_ORAL, FORMACION,bool_bloq,prueba_especial) VALUES (".$curso.",1,".$subsector.",".$cmbMODO.",".$sub_ob.",".$ip.",".$sar.",".$conEX.",".$txtPCT.",".$txtNEXIM.",".$truncado.",".$newOr .",".$prueba_niv.",".$txtPCTNIV.",".$cmbMODOpruebaNivel.",".$pct_ex_escrito.",".$pct_ex_oral.",1,".$bloq.",".$prueba_especial.")";

		  $result =pg_Exec($conn,$qry) or die("fff");
		  if (!$result) {
			 error('<b> ERROR :</b>Error al acceder a la BD. (3)'.$qry);
			 exit;
		  }
    ...
    if ($frmModo=="modificar") {
	   ...
	   if(!isset($pct_ex_semestral)){$pct_ex_semestral=0;}	  
	    ...
	    if(!isset($pct_examen)){$pct_examen=0;}  
		if(!isset($nota_exim)){$nota_exim=0;} 
		if(!isset($prueba_especial)){$prueba_especial=0;}
...
echo $qry="UPDATE ramo SET aprox_entero= ".$aprox_entero.", pct_examen = ".$txtPCT.", nota_exim = ".$txtNEXIM.", conex = ".$conEX.", modo_eval = ".$cmbMODO.", sub_obli =".$sub_ob." , bool_ip = ".$ip.", bool_sar = ".$sar.", truncado = ".$truncado.", prueba_nivel = ".$prueba_niv.", pct_nivel = ".$txtPCTNIV.", modo_eval_pnivel = ".$cmbMODOpruebaNivel.", pct_ex_escrito=".$pct_escrito.", pct_ex_oral=".$pct_oral.", truncado_pnivel = ".$truncado_pnivel." , bool_artis = ".$artis.", porc_examen=".$txtPCTEX." , bool_bloq = ".$bloq.", truncado_ex_final=".$truncado_ex_final." 
, truncado_ex_semestral=".$truncado_ex_semestral.", pct_ex_semestral=".$pct_ex_semestral.", nota_ex_semestral=".$nota_ex_semestral.", conexper=".$conexper.",prueba_especial=".$prueba_especial."
WHERE (((id_ramo)=".$_RAMO."))";
		   ...
		   $result =@pg_Exec($conn,$qry) or die("Error222:".$qry);
	   ...
	   if((!$result) and ($_PERFIL!=17)){
		   ...
		   if(pg_numrows($rs_examen)==0){
			   $largo=count($nombre2);
				for ($i=0;$i<$largo;$i++){ 
				echo "<br>".$sql = "INSERT INTO examen_semestral (id_curso,id_ramo,nombre,porc,bool_ap) VALUES (".$Curso.",".$_RAMO.",'".$nombre2[$i]."',".$sigla[$i].",".$aprox_prom.")";
			   ...
			   $largo=count($nombre2);
				...
				"<br>". $sql = "UPDATE ramo SET porc_examen=".$txtPCTEX." WHERE id_ramo=".$fils_Curso['id_ramo']."";
