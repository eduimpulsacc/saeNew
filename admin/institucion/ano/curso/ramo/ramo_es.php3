...
<SCRIPT language="JavaScript">
			...
			if (form.cmbSECDIF.value!=0){
				...
				form.action = 'ramo_es.php3?institucion=$institucion&$frmModo=ingresar';
				form.submit(true);
				}	
			}
						...
						<TR height="50" >
							<TD align=right colspan=2>
								...
								<?php }?>
									<?php if($_PERFIL!=17){?> 
										<?php if(($_PERFIL!=2)&&($_PERFIL!=4)&&($_PERFIL!=6)&&($_PERFIL!=23)&&($_PERFIL!=24)&&($_PERFIL!=25)&&($_PERFIL!=26)){ //ACADEMICO Y LEGAL?>
												<?php if(($_PERFIL!=3)&&($_PERFIL!=5)&&($_PERFIL!=19)){ //FINANCIERO Y  CONTADOR?>
								...
								<?php if($frmModo=="modificar"){ ?>
									<INPUT class="botonXX"  TYPE="submit" value="GUARDAR" name=btnGuardar onClick="return valida(this.form);" >&nbsp;
									<INPUT class="botonXX"  TYPE="button" value="CANCELAR" name=btnCancelar onclick=document.location="setearamo_es.php3?ramo=<?php echo $ramo?>&caso=1&plan=<?php echo $plan ?>">&nbsp;
								<?php };?>
