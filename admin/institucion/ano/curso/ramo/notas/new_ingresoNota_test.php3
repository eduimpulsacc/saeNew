		...
		<?php if($_MODOEVAL==2){ //EVALUACION CONCEPTUAL ?>
			<SCRIPT language="JavaScript">
				function validaNota(box){
					if(box.value.length==0)	
						return true; // acepta longitud 0
					if(!notaConOnly(box,'Nota inválida. Usar letras MAYUSCULAS 2'))
						return false;
					return true;
				}
...
<!--									<INPUT class="botonX" onmouseover=this.style.background='FFFFD7';this.style.color='003b85' onmouseout=this.style.background='#5c6fa9';this.style.color='ffffff' TYPE="submit" value="GUARDAR" name=btnGuardar onclick="return valida();">&nbsp;	-->
									...
									<?php if($_MODOEVAL==1){ //EVALUACION NUMERICA?>
									<INPUT class="botonXX"  TYPE="submit" value="GUARDAR5" name=btnGuardar onClick="return <?php if ($aprox_entero == 1){ echo "valida1"; }else{echo "valida";} ?>(<? echo $institucion;?>, <? echo $grado;?>, <? echo $ensenanza;?>);">
									<? }else{ ?>
										<INPUT class="botonXX"  TYPE="submit" value="GUARDAR6" name=btnGuardar onClick="return <?php if ($aprox_entero == 1){ echo "valida1"; }else{echo "valida";} ?>(<? echo $institucion;?>, <? echo $grado;?>, <? echo $ensenanza;?>);">
									<? } ?>
									&nbsp;
<?	}	?>
