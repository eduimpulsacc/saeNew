...
<td height="390">
    ...
    <!-- inicio codigo antiguo -->
	    ...
	    <FORM method="post" name="frm_dos"  id="frm_dos" class="" action="proceso_asistencia.php">
...
<div id="botoncito" >
<INPUT class="botonXX"  TYPE="submit"  id="botoncito1"   name="botoncito1"  value="..::GUARDAR::.." >
</div>
...
<p>Date: <input id="datepicker" name="datepicker" type="text" value="<?=$_FechaDeSeleccion?>">

<INPUT class="botonXX"  type="submit"  id="botoncito3" name="botoncito3"  value="..::Consultar Asistencia::..">
</p>
...
<br/>
            ...
            <TABLE WIDTH="100%" BORDER=0 CELLSPACING=0 CELLPADDING=0>
			 ...
			 $qryP="SELECT tiene_taller.rut_alumno, alumno.dig_rut, alumno.nombre_alu, alumno.ape_pat, alumno.ape_mat FROM (alumno INNER JOIN tiene_taller ON alumno.rut_alumno = tiene_taller.rut_alumno)   WHERE (((tiene_taller.id_taller)=".$taller.")) order by ape_pat, ape_mat, nombre_alu asc ";
                ...
                <td> 
					...
					<input name="id_taller" type="hidden" value="<?=$taller?>">
                    <input name="id_ano" type="hidden" value="<?=$ano?>">
			...
			?>
				...
				<?                
                $sql = "SELECT  rut_alumno,ano,id_taller,fecha
                FROM  public.asistencia_taller  WHERE  rut_alumno = ".$filaP["rut_alumno"]."    AND
                ano = ".$ano."  AND  id_taller =".$taller	."  AND   fecha = '".$_FechaDeSeleccion." ';"; 
                
                $result_alumno_asistencia =@pg_Exec($conn,$sql);	
                        
                if (pg_num_rows($result_alumno_asistencia)!=0){
                ?>
                 <input type="checkbox" name="alum[<?=$i?>]" value=<?php echo $filaP["rut_alumno"];?>  checked >
                  <?  
                }else{
                    ?>
                    <input type="checkbox" name="alum[<?=$i?>]" value=<?php echo $filaP["rut_alumno"];?>> 
                    <?
                    }
                  ?>
