...
?>
							...
							<?		if($modo_eval_p_nivel==2){  	 //prueba de nivel del tipo conceptual ?>
										<INPUT class="botonXX"  TYPE="submit" value="GUARDAR1" name=btnGuardar onClick="return valida_prueba_nivel_conceptual(<? echo $fila10['pct_nivel'];?>);">
										&nbsp;
<?		}else if($modo_eval_p_nivel==1){  /*prueba de nivel del tipo numerico*/	 ?>
<input class="botonXX"  type="submit" value="GUARDAR2" name=btnGuardar4 onClick="return valida_prueba_nivel(<? echo $fila10['pct_nivel'];?>,<? echo $truncado_pnivel; ?>);">
&nbsp;
<?		}	?>
							<?	}
// modificar para el tipo conceptual
							...
							<?		if($modo_eval_p_nivel==2){	//prueba de nivel del tipo conceptual ?>
							<input class="botonXX"  type="submit" value="GUARDAR3" name=btnGuardar2 onClick="return valida_prueba_nivel_conceptual(<? echo $fila10['pct_nivel'];?>);">
							&nbsp;
<?		}else{ //prueba de nivel del tipo numerico	?>
<input class="botonXX"  type="submit" value="GUARDAR4" name=btnGuardar3 onClick="return valida_prueba_nivel(<? echo $fila10['pct_nivel'];?>);">
&nbsp;
<?		}	?>
							<?	}
...
<!--									<INPUT class="botonX" onmouseover=this.style.background='FFFFD7';this.style.color='003b85' onmouseout=this.style.background='#5c6fa9';this.style.color='ffffff' TYPE="submit" value="GUARDAR" name=btnGuardar onclick="return valida();">&nbsp;	-->
									...
									<?php if($_MODOEVAL==1){ //EVALUACION NUMERICA?>
									<INPUT class="botonXX"  TYPE="submit" value="GUARDAR" name=btnGuardar onClick="return <?php if ($aprox_entero == 1){ echo "valida1"; }else{echo "valida";} ?>(<? echo $institucion;?>, <? echo $grado;?>, <? echo $ensenanza;?>);">
									<? }else{ ?>
										<INPUT class="botonXX"  TYPE="submit" value="GUARDAR" name=btnGuardar onClick="return <?php if ($aprox_entero == 1){ echo "valida1"; }else{echo "valida";} ?>(<? echo $institucion;?>, <? echo $grado;?>, <? echo $ensenanza;?>);">
									<? } ?>
									&nbsp;
<?	}	?>
								<INPUT class="botonXX"  TYPE="button" value="CANCELAR" name=btnCancelar onClick=document.location="new_mostrarNotas.php3">&nbsp;							</TD>
						</TR>
						<TR height=20>
														...
														for($j=1;$j<22;$j++){
															...
															if ($Y==21){   // columna promedios
																	...
																	if ($fila10['modo_eval']==1){
																		  if ($fila2['promedio']!=0){ 											
																			echo "aa<INPUT TYPE=text id=\"a_".$X."_".$Y."\"  NAME=\"a_".$X."_".$Y."\" onkeyup=fn(this.form,this,".$n.") value=\"".trim($fila2['promedio'])."\" size=2  maxlength=2;>";   
																		  }
																		  else{
																		...
																		if(!strcmp($var,'nota20')){
																			 if ((trim($fila2["$var"])==MB) OR (trim($fila2["$var"])==B) OR (trim($fila2["$var"])==S) OR (trim($fila2["$var"])==I)){
																				echo "<INPUT TYPE=text id=\"a_".$X."_".$Y."\"  NAME=\"a_".$X."_".$Y."\" onkeyup=fn(this.form,this,".$n.") value=\"".trim($fila2["$var"])."\"  size=2  maxlength=2;>";
																				 ...
																				 if (trim($fila2["$var"])!=0){
																					echo "22<INPUT TYPE=text id=\"a_".$X."_".$Y."\"  NAME=\"a_".$X."_".$Y."\" onkeyup=fn(this.form,this,".$n.") value=\"".trim($fila2["$var"])."\"  size=2  maxlength=2;>";
																				 }
																				 else{
																					echo "33<INPUT TYPE=text id=\"a_".$X."_".$Y."\"  NAME=\"a_".$X."_".$Y."\" onkeyup=fn(this.form,this,".$n.") size=2  maxlength=2;>";
																				}
																			 }
																		}
																		else{
																			 if (trim($fila2["$var"])!=0){ ?>
																				c<div id="Arr<?=$jj?>" onClick="creaInput(this.id, 'campo<?=$j?>','<?=$alumno?>','<?=$ramo?>','1137','<?=$ii?>','<?=$i_alu?>','<?=trim($fila2["$var"])?>','a_<?=$X?>_<?=$Y?>','a_<?=$X?>_<?=$Y?>','<?=$n?>')" onkeyup="fn(this.form,this,<?=$n?>); cargaDatos(this.id,'campo<?=$j?>','<?=$alumno?>','<?=$ramo?>','1137','<?=$ii?>','<?=$i_alu?>','a_<?=$X?>_<?=$Y?>', 'a_<?=$X?>_<?=$Y?>', '<?=$n?>')"><INPUT style="color:#000000; background-color:#FFFFFF"  TYPE=text id="a_<?=$X?>_<?=$Y?>"  NAME="a_<?=$X?>_<?=$Y?>" onkeyup="fn(this.form,this,<?=$n?>); cargaDatos(this.id,'campo<?=$j?>','<?=$alumno?>','<?=$ramo?>','1137','<?=$ii?>','<?=$i_alu?>','a_<?=$X?>_<?=$Y?>', 'a_<?=$X?>_<?=$Y?>', '<?=$n?>');" value="<?=trim($fila2["$var"])?>"  size="2"  maxlength="2" ></div> <?  $jj++;
																			 }
																			 else{
																				echo "dd<INPUT TYPE=text id=\"a_".$X."_".$Y."\"  NAME=\"a_".$X."_".$Y."\" onkeyup=fn(this.form,this,".$n.") size=2  maxlength=2;>";
																			 }
																		}
																	 }
																	if ($fila10['modo_eval']==3){
...
*/
														...
														$m = $m+1;
														echo "<TD align=center>";
														echo "<INPUT TYPE=text id=\"a_".$X."_1\"  NAME=\"a_".$X."_1\" onkeyup=fn(this.form,this,".$m.") size=2  maxlength=2;>bbbb";
														echo "</TD>";
														echo "<TD align=center>";
														$m = $m+1;
