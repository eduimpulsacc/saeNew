 ...
 $sql = "SELECT id,id_ano,id_rdb,nombre_concepto,valor_numerico,rango_x,rango_y  
	...
	if(pg_num_rows($rs)!=0){
			...
			for($r=0;$r<pg_num_rows($rs);$r++){
				...
				if($fila[3]=='SI'){		
				   echo 'var patron8 = /(^[Ss][Ii]{1}$)/;
				   if ( patron8.test(dato) == true ) nueva_nota = '.$fila[4].';';
				}
				if($fila[3]=='G'){		
				   echo 'var patron8 = /(^[G][g]{1}$)/;
				   if ( patron8.test(dato) == true ) nueva_nota = '.$fila[4].';';
				}
				if($fila[3]=='RV'){		
				   echo 'var patron8 = /(^[Rr][Vv]{1}$)/;
				   if ( patron8.test(dato) == true ) nueva_nota = '.$fila[4].';';
				}
				if($fila[3]=='N'){		
				   echo 'var patron8 = /(^[N][n]{1}$)/;
				   if ( patron8.test(dato) == true ) nueva_nota = '.$fila[4].';';
				}
				if($fila[3]=='NO'){		
				   echo 'var patron8 = /(^[Nn][Oo]{1}$)/;
				   if ( patron8.test(dato) == true ) nueva_nota = '.$fila[4].';';
				}
...
if($("#modo_eval").val()==0) return true;	
		...
		if( $("#modo_eval").val() == 2 || $("#modo_eval").val() == 4 || $("#modo_eval").val() == 5 ){ 
				 if(dato!=""){
				   var patron1 = /(^[iIsSbBnNnaAlLgGrRvV]{1}$)/;
				   var patron2 = /(^[mMaAnNeEiIrRoOsS][bBlLpPiIvVoO]{1}$)/;
					  if ( patron1.test(dato) == false ){
			...
			if( datovalido( $("#nota").val() )==true ){
				...
				if( $("#modo_eval").val() == 2 || $("#modo_eval").val() == 4  || $("#modo_eval").val() == 5) {
					  if( n.length!=0 ){
							...
							var patron1 = /(^[mMaAnNeEiIrRoOsS][bBlLpPiIvVoO]{1}$)/;
