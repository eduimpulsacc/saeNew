...
/* if($id_ramo==294391){
	...
	$cod_subsector = pg_result($rs_cod_sub,0);
		...
		$f_d_ramo=@pg_fetch_array($result,0);
/*$id_periodo = 1200;
$id_ramo = 129637;
$id_ano = 576;*/
...
if($cod_subsector==50686 || $cod_subsector==50687 || $cod_subsector==50688 || $cod_subsector==50689 || $cod_subsector==50690 and $_INSTIT==18049){
	function desifranotaconseptual($dato,$modo_eval){
	$nueva_nota=0;
	
	if(trim($modo_eval)==5){
		$prom_conc = $dato;
	if( $prom_conc >=60 && $prom_conc <= 70 ) $letra = 'SI';
	elseif( $prom_conc >=50 && $prom_conc <= 59 ) $letra = 'G';
	elseif( $prom_conc >=40 && $prom_conc <= 49 ) $letra = 'RV';
	elseif( $prom_conc >0   && $prom_conc <= 39 ) $letra = 'N';
	else $letra = 'NO';
...
for($i=0;$i<pg_num_rows($rs_notas);$i++){
	...
	for($j=1;$j<20;$j++){
		...
		if( $modo_eval == 1 || $modo_eval == 3 ){
		 ...
		 if( $modo_eval == 2 || $modo_eval == 4 || $modo_eval == 5){
			      $nota=desifranotaconseptual($nota,$modo_eval);
   			      $suma = $suma + $nota;
   	    	   }
