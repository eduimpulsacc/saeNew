...
$cox="#ffffff";
	   if($notagrupo==1){
	//$cox="#ffffff";
			 $slqgr="select * from grupo_nota where id_ramo=".$id_ramo." order by id_grupo";
...
where rut_alumno=".$fila['rut_alumno']." and id_ramo=".$fila['id_ramo']." and id_periodo=".$fila['id_periodo'];
	...
	where rut_alumno=".$fila['rut_alumno']." and id_ramo=".$fila['id_ramo']." and id_periodo=".$fila['id_periodo']." ";	
				...
				//if($_PERFIL==0){echo "<br>".$caso."---".$fila['rut_alumno']."->".$ngrop;}
				//$ngrop = ($trunca_grupo == 1  && $truncado==0)?round($pprx):$pprx;
				
				//$ngrop=( $trunca_grupo == 1 )?round(pg_result($rsgrop,0)):intval(pg_result($rsgrop,0));
				//$ngrop="";
...
$ont++;
		...
		if ( $curso[ $i ][ "bool_ar" ] == 1){
		    ...
		    $table = $table."<tr class='textolink' align='center' id='alumm_$i' >";
		    }
