...
//buscar grupos porcentaje
	 $sqlgp="select count(*) from grupo_nota_ramo_porcentaje where id_ramo= and id_periodo = $periodo";
	$rsgp  = pg_exec($conn,$sqlgp);
	
	$ntgrupo =pg_result($rsgp,0);
	 
	
	if($ntgrupo>0){
		
		  $slqgr="select * from grupo_nota_ramo_porcentaje where id_ramo=".$ramo." and id_periodo = $periodo order by id_grupo";
		$rgr = pg_exec($conn,$slqgr);
		
		if(pg_numrows($rgr)>0){$ntgrupo=1;}
...
function editatable(a){
  ...
  //  alert(posicionk);
	...
	if( a != 'x' ){
		...
		$("#paises tbody tr:eq(0)").css("background","#F7F7F7");
			
			var text = $("#paises tbody td:eq(0)").text();
			
			nombrehiddennota = $("#paises tbody td:eq(0) :input").attr("name");	
			
			var inputhidden = '<input type="hidden" name="'+nombrehiddennota+'" id="'+nombrehiddennota+'" value="'+text+'" />'
		
	    var nombreclase = $("#paises tbody td:eq(0)").attr("class");
			// alert(nombreclase);
		if(nombreclase=="guardado"){
			
			$("#paises tbody td:eq(0)").empty().html('<input name="nota" type="text" id="nota" size="3" maxlength="3" value="'+text+'" style="color:#003366; background-color:#EC9282; " onKeyUp="apromediar()" onfocus="return false" /><input id="index_nota" type="hidden" value="0" />'+inputhidden+'');
			...
			$("#paises tbody td:eq(0)").empty().html('<input name="nota" type="text" id="nota" size="3" maxlength="3" value="'+text+'" onKeyUp=apromediar("'+nombrehiddennota+'") /><input id="index_nota" type="hidden" value="0" />'+inputhidden+'');
...
/***************************************************/
    ...
    function validar(e,a){
	   ...
	   if( datovalido( $("#nota").val() )==true ){
			...
			$("#paises tbody td:eq("+index_actual+")").empty().html(''+valor+inputhidden+'');
						
			var text = $("#paises tbody td:eq("+index+")").text();	
			
			nombrehiddennota = $("#paises tbody td:eq("+index+") :input").attr("name");
			...
			//if(index < (total_table) ){
				 ...
				 index_tr_a = index_tr;
                 $("#paises tbody tr:eq("+index_tr_a+")").css("background","");
				  cambioEstilo(index_tr,0);
                 index_tr++;
                 $("#paises tbody tr:eq("+index_tr+")").css("background","#F7F7F7");
				  cambioEstilo(index_tr,1);
 				 var nombreclase = $("#paises tbody td:eq("+index+")").attr("class");
					 
					 if(nombreclase=="guardado"){
							 			 
						$("#paises tbody td:eq("+index+")").empty().html('<input name="nota" readonly="readonly" type="text" id="nota" size="3" maxlength="3" value="'+text+'" style="color:#003366; background-color:#EC9282; " /><input id="index_nota" type="hidden" value="'+index+'" />'+inputhidden+'');
						 ...
						 $("#paises tbody td:eq("+index+")").empty().html('<input name="nota" type="text" id="nota" size="3" maxlength="3" value="'+text+'" onKeyUp=apromediar("'+nombrehiddennota+'") /><input id="index_nota" type="hidden" value="'+index+'" />'+inputhidden+'');
		  ...
		  if( datovalido( $("#nota").val() )==true ){
			   ...
			   var index_actual = index;
				...
				if(index  >= largo_notas){ 
							...
							if(index_tr!=1){
								 $("#paises tbody tbody tr:eq("+index_tr+")").css("background","");
								  index_tr--;
								 $("#paises tbody tbody tr:eq("+index_tr+")").css("background","#F7F7F7");
								  cambioEstilo(index_tr,1);
							 }
    ...
    if (keyCode == '37') {   // codigo ascii de la flecha hacia la izquierda <---
		  ...
		  if(index != 0){
			...
			$("#paises tbody td:eq("+index+")").empty().html(''+valor+inputhidden+'');
...
var largo = dato.length; 	
				...
				if (largo ==3 &&  patron0.test(dato) == false) {
							alert("Dato "+dato+" No es Valido");
							return false;
				}
				else if (largo ==2 &&  patron1.test(dato) == false){
					alert("Dato "+dato+" No es Valido");
					return false;
				}
				else if (largo ==1 &&  patron2.test(dato) == false){
					alert("Dato "+dato+" No es Valido");
					return false;
				}
				else{
					 ...
					 //fin validacion que no venga vacio
					 }else{
...
//                   }, 2000);
		  ...
		  else if($("#nota").val() <0 || $("#nota").val() >100){
			  
			  alert("Dato "+n+" No es Valido");
			$("#nota").val("");
			  promedio(index_tr_1,index_actual_1,cont_1);
			}
...
$tense = trim($fila1['ensenanza']);	
			...
			$qry="SELECT * FROM ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector WHERE (((ramo.id_ramo)=".$ramo."))";
               ...
               /* echo "<input id='modo_eval' type='hidden' value='".$modo_eval."' >";*/
				 ...
				 echo "<input id='modo_eval' type='hidden' value='1' >";
				...
				echo "<input id='cadgrupo' type='hidden' value='' >";
               	...
               	$sql_busca = "SELECT * FROM porc_notas$ano_act n 
                INNER JOIN  ramo r ON r.id_ramo=n.id_ramo 
                WHERE n.id_ramo=".$ramo." AND n.id_periodo=".$periodo." AND n.nota20 is not null AND n.nota20<>'101' ";
			...
			if (!$result5) {
				...
				if (!$result1106){
					...
					if (pg_numrows($result1106)!=0){
						...
						<TR>
						  <TD align=left><FONT face="arial, geneva, helvetica" size=2>
									<strong>INSTRUCCIONES</strong></FONT></TD>
						  <TD>&nbsp;</TD>
						  <TD><FONT face="arial, geneva, helvetica" size=2>
									<strong>EN ESTE M&Oacute;DULO SE INGRESAN SOLAMENTE PORCENTAJES (0 A 100)</strong></FONT></TD>
						  </TR>	
					</TABLE>
				</TD>
			</TR>
...
<table align="right" >
<tr>
  <td><input type="button" onClick="grupon(<? echo $ramo?>,<?php echo $_CURSO ?>,<?php echo $periodo ?>)" value="CONFIGURACI&oacute;N PROGRESO PORCENTUAL" class="botonXX" style="text-transform:uppercase"></td>
<td>
...
Vertical</td>
<td>
<input type="radio" name="desp_auto" id="desp_auto_Vertical"  value="40" checked onClick="document.getElementById('nota').focus()"></td>
</tr>
</table>
</div>
...
//configuracion de grupo de notas
function grupon(rm,cur,per){
	var parametros="funcion=1&curso="+cur+"&ramo="+rm+"&per="+per;
	//alert(parametros);
	

	$.ajax({
	  url:'../../gruponPorc/cont_grupon.php',
	  data:parametros,
	  type:'POST',
	  success:function(data){
	    $("#ramogrupo").html(data);
    ...
    buttons: {
		 "Cerrar": function(){
			 ...
			 if(acum ==0 || acum==100){
				...
				alert("Debe completar la configuracion de grupos hasta acumular el 100%");
			}
	  }
	}   
  }
...
function ngr(ramo){
	var parametros="funcion=2&ramo="+ramo+"&per="+<?php echo $periodo ?>;
	//alert(parametros);
$.ajax({
				url:"../../gruponPorc/cont_grupon.php",
				data:parametros,
...
function edifila(grupo){
	var parametros = "funcion=4+&grupo="+grupo+"&per=<?php echo $periodo ?>";
