...
/* if($id_ramo==294391){
	...
	//buscar grupos porcentaje
	 $sqlgp="select count(*) from grupo_nota_ramo_porcentaje where id_ramo=$id_ramo and id_periodo = $id_periodo";
	$rsgp  = pg_exec($conn,$sqlgp);
	
	$notagrupo =pg_result($rsgp,0);
	 
	
	if($notagrupo>0){
		
		  $slqgr="select * from grupo_nota_ramo_porcentaje where id_ramo=".$id_ramo." and id_periodo = $id_periodo order by id_grupo";
		$rgr = pg_exec($conn,$slqgr);
		
		if(pg_numrows($rgr)>0){$notagrupo=1;}
...
$cox="#ffffff";
	   if($notagrupo==1){
	//$cox="#ffffff";
			 $slqgr="select * from grupo_nota_ramo_porcentaje where id_ramo=".$id_ramo." and id_periodo = $id_periodo order by orden";
	...
	return $prom_nuevo;
        ...
        if (pg_numrows($result)!=0){
		   ...
		   $trunca_grupo = trim($fila10['bool_aprgrp']);
		    $sql_busca = "SELECT * FROM porc_notas$nro_ano n 
		   INNER JOIN  ramo r ON r.id_ramo=n.id_ramo 
		   WHERE n.id_ramo=".$id_ramo." AND n.id_periodo=".$id_periodo." AND n.nota20 is not null AND n.nota20<>'101' ";
...
$sql ="SELECT * FROM PORC_NOTAS".$nro_ano." WHERE ID_PERIODO=".$id_periodo." AND id_ramo=".$id_ramo.";  ";
...
where rut_alumno=".$fila['rut_alumno']." and id_ramo=".$fila['id_ramo']." and id_periodo=".$fila['id_periodo'];
			...
			if($cuengrp>0){
				 $sql_nogr="select (($cad_grp)) from porc_notas$nro_ano 
	where rut_alumno=".$fila['rut_alumno']." and id_ramo=".$fila['id_ramo']." and id_periodo=".$fila['id_periodo']." ";	
		...
		// echo $fila['rut_alumno']."-".$sumagrop."<br>";
		 	if( $sumagrop !="0"){
		   ...
		   if($mod_eval==3){$promedio =  promedioconceptual( $promedio );}
				
				$sql_Update = "UPDATE porc_notas".$nro_ano." SET promedio='".$promedio."' WHERE rut_alumno=".$fila['rut_alumno']." AND id_ramo=".$fila['id_ramo']." AND id_periodo=".$fila['id_periodo']."";
...
periodo.id_periodo, 
ramo.id_ramo,
porc_notas".$nro_ano.".nota1,
porc_notas".$nro_ano.".nota2,
porc_notas".$nro_ano.".nota3,
porc_notas".$nro_ano.".nota4,
porc_notas".$nro_ano.".nota5,
porc_notas".$nro_ano.".nota6,
porc_notas".$nro_ano.".nota7,
porc_notas".$nro_ano.".nota8,
porc_notas".$nro_ano.".nota9,
porc_notas".$nro_ano.".nota10,
porc_notas".$nro_ano.".nota11,
porc_notas".$nro_ano.".nota12,
porc_notas".$nro_ano.".nota13,
porc_notas".$nro_ano.".nota14,
porc_notas".$nro_ano.".nota15,
porc_notas".$nro_ano.".nota16,
porc_notas".$nro_ano.".nota17,
porc_notas".$nro_ano.".nota18,
porc_notas".$nro_ano.".nota19,
porc_notas".$nro_ano.".nota20,
porc_notas".$nro_ano.".promedio 
FROM alumno 
LEFT JOIN porc_notas".$nro_ano." ON porc_notas".$nro_ano.".rut_alumno= alumno.rut_alumno 
AND porc_notas".$nro_ano.".id_periodo = ".$id_periodo." AND porc_notas".$nro_ano.".id_ramo = ".$id_ramo." 
...
$ont++;
		...
		$table = $table."<th align='left' class='formatoth alumn_".($i+1)."'' >&nbsp;".strtoupper($curso[ $i ][ "ape_pat" ])." ".strtoupper($curso[ $i ][ "ape_mat" ])." ".strtoupper($curso[ $i ][ "nombres" ])."&nbsp;</th>";
			...
			for ( $e = 0 ; $e < 20 ; $e++ ){ 
				...
				if( $curso[ $i ][ "notas" ][ $e ]!=101 or $curso[ $i ][ "notas" ][ $e ]=='B' or $curso[ $i ][ "notas" ][ $e ]=='S' or $curso[ $i ][ "notas" ][ $e ]=='I' or $curso[ $i ][ "notas" ][ $e ]=='MB' or $curso[ $i ][ "notas" ][ $e ]=='AL' or $curso[ $i ][ "notas" ][ $e ]=='L' or $curso[ $i ][ "notas" ][ $e ]=='NL' or $curso[ $i ][ "notas" ][ $e ]=='EP' or $curso[ $i ][ "notas" ][ $e ]=='SI' or $curso[ $i ][ "notas" ][ $e ]=='G' or $curso[ $i ][ "notas" ][ $e ]=='RV' or $curso[ $i ][ "notas" ][ $e ]=='N' or $curso[ $i ][ "notas" ][ $e ]=='NO' or $curso[ $i ][ "notas" ][ $e ]=='D' or $curso[ $i ][ "notas" ][ $e ]=='ED'or $curso[ $i ][ "notas" ][ $e ]=='NM' ){
			...
			$table = $table.'<input type="hidden" name="a_'.$i_i.'_'.$e_e.'" id="a_'.$i_i.'_'.$e_e.'" value="101" />';
			//$table = $table.'<div style="display:none;" >a_'.$i_i.'_'.$e_e.'</div>';
							$table = $table."</td>";
							$inex_td++; 
	...
	$np  = ($curso[ $i ][ "notas" ][ 20]==101 )?0:$curso[ $i ][ "notas" ][ 20];
	$table = $table."<th class='formatoth' ><div id='prom".$ont."' >".$np."</div>";
