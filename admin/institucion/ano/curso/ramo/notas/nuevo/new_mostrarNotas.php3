...
var parametros = "id_periodo="+id_periodo+"&id_ramo="+id_ramo+"&id_ano="+id_ano;
...
for (e=1;e<21;e++){  
table =  table+"<th onMouseOver='mostrarventana("+e+")' ><a href='#' ><div  class='formatothmsj' id='ventanita"+e+"'>&nbsp;"+e+"º&nbsp;</div></a></th>";
}
...
ont++;
		...
		table = table+"<th class='formatoth' >"+data[i]["nro_lista"]+" ";
		table = table+"<div id='rut_alumno"+ont+"' style='display:none;' >"+data[i]["rut"]+"</div>"; 
		table = table+"<div id='id_periodo"+ont+"' style='display:none;' >"+data[i]["id_periodo"]+"</div>"; 
		table = table+"<div id='id_ramo"+ont+"' style='display:none;' >"+data[i]["id_ramo"]+"</div>"; 
		table = table+"</th>";
		table = table+"<th align='left' class='formatoth' >&nbsp;"+data[i]["nombres"]+" "+data[i]["ape_pat"]+" "+data[i]["ape_mat"]+"&nbsp;</th>";
		
		if($("#bool_bloq").val()==1) clasico = "guardado'";
			...
			for (e=0;e<20;e++){ 
				...
				if( data[i]["notas"][e]!=0 ){
						if( e != 19 ){
						      if(data[i]["bool_ar"] == 1){
							      table = table+"<td class='guardado' >"+data[i]["notas"][e]+"</td>";
							  }else{ table = table+"<td class='"+clasico+"' >"+data[i]["notas"][e]+"</td>"; } 
						   inex_td++;
						 }else{
							  table = table+"<th class='guardadopruebadenivel' >";
							  table = table+"<div id='pruebadenivel"+ont+"' >"+data[i]["notas"][e]+"</div></th>";
						 }
				}else{
				      if(data[i]["bool_ar"] == 1){
					      if( e != 19 ){	
					        ...
					        if( e != 19 ){		
							  table = table+"<td onDblClick='editatable("+inex_td+")' ></td>";
						      inex_td++; 
							}else{
			...
			table = table+"<th class='formatoth' ><div id='prom"+ont+"' >"+data[i]["notas"][20]+"</div></th>";
			table = table+"</tr>";
        } // FIN CICLO 
table =  table+"<tr align=center class='formatoth' >";
table =  table+"<th>NºLista</th>";
table =  table+"<th>Alumno</th>";
	for (e=1;e<21;e++){  
	table =  table+"<th><div class='formatothmsj' >&nbsp;"+e+"º&nbsp;</div></th>";
	}
table =  table+"<th>&nbsp;Prom&nbsp;</th>";
table =  table+"</tr>";
...
/*	if( y != "" ){
		...
		if(nombreclase=="guardado"){
			...
			$("#paises td:eq(0)").empty().html('<input name="nota" type="text" id="nota" size="2" maxlength="2" value="'+text+'" style="color:#003366; background-color:#EC9282; " /><input id="index_nota" type="hidden" value="0" />');
			$("#nota").focus(); 
			$("#nota").select();
		}else{
			var text = $("#paises td:eq(0)").text();
			$("#paises td:eq(0)").empty().html('<input name="nota" type="text"  id="nota" size="2"  maxlength="2" value="'+text+'" /><input id="index_nota" type="hidden" value="0" />');
			$("#nota").focus(); 
			$("#nota").select();
		}
 /*   }	*/
...
/***************************************************/
    ...
    if (keyCode == '37') {   // codigo ascii de la flecha hacia la izquierda <---
		  ...
		  if(index != 0){
		        teclafecha = '37';
                if($("#nota").val()!=""){  promedio(index_tr,index,cont);	 }
				 if(cont==0){ cont=18; 
				 }else{ cont-=1; } 
				if(index%largo_notas==0){
                      var index_tr_a = index_tr;
				 ...
				 var nombreclase = $("#paises td:eq("+index+")").attr("class");
						 if(nombreclase=="guardado"){
								 ...
								 $("#paises td:eq("+index+")").empty().html('<input name="nota" readonly="readonly" type="text" id="nota" size="2" maxlength="2" value="'+text+'" style="color:#003366; background-color:#EC9282; " /><input id="index_nota" type="hidden" value="'+index+'" />');
								  $("#nota").select();
								  $("#nota").focus();
						 }else{   var text = $("#paises td:eq("+index+")").text();
								 $("#paises td:eq("+index+")").empty().html('<input name="nota" type="text" id="nota" size="2" maxlength="2" value="'+text+'" /><input id="index_nota" type="hidden" value="'+index+'" />');
								  $("#nota").select();
								  $("#nota").focus();  }
			         $("#paises tr:eq("+index_tr+")").css("background","#FFFF99");
...
function promedio(xindextr,xintextd,conttd){   
	...
	var indextd = 0;
		...
		//alert(xindextr+"+"+xintextd+"+"+conttd);
		if( xintextd < conttd ){
			indextd = 0;
		}else{
			if(xintextd!=0){
