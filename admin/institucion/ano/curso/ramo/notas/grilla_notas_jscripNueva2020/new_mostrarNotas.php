...
if ($viene_de){
 ...
 $rs_escala = getEscalaNotas($institucion,$conn);
 $fila_escala = pg_fetch_array($rs_escala,0);
 $notaMinima = $fila_escala['nmin']*10;
	...
	//if($_PERFIL==0){
		...
		?>
        <script>
		alert('Edici\xf3n de notas bloqueada.\nPara debloquear, debe eliminar promoci\xf3n');
		</script>	
		<?
		}
 ...
 //ver si tengo que agrupar
	...
	if($ntgrupo==1){
		 ...
		 $slqgr="select * from grupo_nota where id_ramo=".$ramo." and id_periodo=$_PERIODORAMO order by orden";
		$rgr = pg_exec($conn,$slqgr);
		
			if(pg_numrows($rgr)==0 && $cnt_prom>0){
			 $slqgr="select * from grupo_nota where id_ramo=".$ramo." order by orden";
 ...
 //}
	...
	$result =@pg_Exec($conn,$qry);
		...
		if (!$result) 
			...
			if (pg_numrows($result)!=0){
				...
				echo "NO EXISTEN PERIODOS INGRESADOS PARA ESTE A&Ntilde;O";
	...
	if (!$r1){
	   echo "Error: No encontr&oacute; el periodo";
	   exit();
	}else{
  ...
  //  alert(posicionk);
	...
	if( a != 'x' ){
		...
		if(nombreclase=="guardado"){
			...
			$("#paises tbody td:eq(0)").empty().html('<input name="nota" type="text" id="nota" size="2" maxlength="2" value="'+text+'" style="color:#003366; background-color:#EC9282; " onKeyUp="apromediar()" onfocus="return false" /><input id="index_nota" type="hidden" value="0" />'+inputhidden+'');
			...
			$("#paises tbody td:eq(0)").empty().html('<input name="nota" type="text" id="nota" size="2" maxlength="2" value="'+text+'" onKeyUp=apromediar("'+nombrehiddennota+'") /><input id="index_nota" type="hidden" value="0" />'+inputhidden+'');
                 ...
                 $("#paises tbody tr:eq("+index_tr+")").css("background","#F7F7F7");
 				 ...
 				 var nombreclase = $("#paises tbody td:eq("+index+")").attr("class");
					 ...
					 if(nombreclase=="guardado"){
						...
						$("#paises tbody td:eq("+index+")").empty().html('<input name="nota" readonly="readonly" type="text" id="nota" size="2" maxlength="2" value="'+text+'" style="color:#003366; background-color:#EC9282; " /><input id="index_nota" type="hidden" value="'+index+'" />'+inputhidden+'');
						 ...
						 $("#paises tbody td:eq("+index+")").empty().html('<input name="nota" type="text" id="nota" size="2" maxlength="2" value="'+text+'" onKeyUp=apromediar("'+nombrehiddennota+'") /><input id="index_nota" type="hidden" value="'+index+'" />'+inputhidden+'');
...
var minima = parseInt(<?php echo $notaMinima; ?>);
...
if(typeof dato == "undefined"){return false};  	
		...
		if( $("#modo_eval").val() == 1 || $("#modo_eval").val() == 3 ){ 
				...
				if(dato>=minima){ // que nota sea mayor o igual a nota mínima	
				...
				alert("Dato "+dato+" es menor a nota m\xedmima");
							//msjerror("Dato "+dato+" No es Valido");
							return false; 
				}
...
<tr>
<td colspan="2">							
Desplazamiento Autom&aacute;tico</td>
<tr align="center">
<td width="160">
Horizontal</td>
<td width="98">
...
<tr align="center" >
  <td>Activado</td>
  <td><input name="desact" type="checkbox" id="desact" checked="CHECKED"></td>
</tr>
<?php //}?>
</table>
...
if(files !== undefined){
    ...
    var fileName = files.name; //obtenemos la extensiï¿½n del archivo
    var fileExtension = fileName.substring(fileName.lastIndexOf('.') + 1);//obtenemos el tamaï¿½o del archivo
