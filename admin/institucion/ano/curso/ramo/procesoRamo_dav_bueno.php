										...
										if (!$result2){ 
											...
											if (pg_numrows($result2)==0){
												...
												if ($chk > 0){ // ingresar
													...
													echo "<br><br>";	
													echo   $qry="INSERT INTO RAMO (ID_CURSO, TIPO_RAMO, COD_SUBSECTOR, MODO_EVAL, SUB_OBLI, BOOL_IP, BOOL_SAR, CONEX, PCT_EXAMEN, NOTA_EXIM, TRUNCADO, ID_ORDEN, PRUEBA_NIVEL, PCT_NIVEL, MODO_EVAL_PNIVEL, PCT_EX_ESCRITO, PCT_EX_ORAL) VALUES (".$curso.",1,".$chk.",".$cmbMODO.",".$sub_ob.",".$ip.",".$sar.",".$conEX.",".$txtPCT.",".$txtNEXIM.",".$truncado.",".$newOr .",".$prueba_niv.",".$txtPCTNIV.",".$cmbMODOpruebaNivel.",".$pct_ex_escrito.",".$pct_ex_oral.")";
    ...
    if ($frmModo=="modificar") {
	   ...
	   if((!$result) and ($_PERFIL!=17)){
		   ...
		   if(@pg_numrows ($rs_examen)==0){
		   		$largo=count($nombre2);
				for ($i=0;$i<$largo;$i++){ 
					$sql = "INSERT INTO examen_semestral (id_curso,id_ramo,nombre,porc,bool_ap) VALUES (".$Curso.",".$_RAMO.",'".$nombre[$i]."',".$sigla[$i].",".aprox_prom.")";
