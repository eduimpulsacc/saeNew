...
if($funcion==1){
	...
	$rs_listado=$ob_grupo->tablaCurso($conn,$curso,$ramo,$per);
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" id="conte" style="border-collapse:collapse">
 ...
 <?php  for($l=0;$l<pg_numrows($rs_listado);$l++){
	 ...
	 $pac= $pac+pg_result($rs_listado,5) ;
	 ?>
  <tr id="fila<?php echo $fila['id_grupo'] ?>" class="textosimple">
    <td height="33"><?php echo pg_result($rs_ramo,0) ?></td>
    <td height="33"><?php echo pg_result($rs_listado,26) ?></td>
    <td align="center"><?php echo pg_result($rs_listado,5) ?> <input type="hidden" class="prco" value="<?php echo pg_result($rs_listado,5) ?>" /></td>
...
<input type="hidden" name="acum" id="acum" value="<?php echo $pac ?>" />
 <br />
  <?php if($pac<100){?>
...
<tr class="textosimple">
    ...
    <td><?php for($n=1;$n<=19;$n++){
		$rs_marca = $ob_grupo->veMarca($conn,$per,$ramo,$n);
...
if($funcion==4){
  ...
  <td class="textosimple"><?php echo pg_result($rs_ramo,0) ?></td>
  <td height="33" class="textosimple"><input name="txtLECCIONARIO" id="txtLECCIONARIO" type="text"  value="<?php echo $fila['nombre']; ?>"/></td>
  <td height="3" align="center" class="textosimple"><input name="prc" type="text" id="prc" class="prco" size="2" value="<?php echo $fila['porcentaje'] ?>" /></td>
  <td class="textosimple"><?php for($n=1;$n<=19;$n++){
		$rs_marca = $ob_grupo->veMarca($conn,$per,$fila['id_ramo'],$n,$grupo);
		?>
  ...
  <?php }?></td>
  <td align="center" class="textosimple"><input name="orden" type="text" id="orden" value="<?php echo $fila['orden'] ?>" size="2" class="solo-numero" /></td>
  <td class="textosimple"><input name="idg" type="hidden" id="idg" value="<?php echo $fila['id_grupo'] ?>" /><input name="" type="button" value="Guardar" class="botonXX" onclick="guardaGrupoEdi();" />&nbsp;<input type="button" name="button" id="button" value="Cancela Edici&oacute;n" class="botonXX" onclick="grupon(<?php echo $fila['id_ramo'] ?>,<?php echo $fila['id_curso'] ?>,<?php echo $per ?>)" /></td>
