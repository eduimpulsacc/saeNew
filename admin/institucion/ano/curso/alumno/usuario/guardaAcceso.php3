...
<?php
	...
	if (!$result) {
		...
		$newID =  $fila['cant'];
		$newID = $newID + 1;
		$qry="INSERT INTO USUARIO (ID_USUARIO, NOMBRE_USUARIO, PW) VALUES (".$newID.",'".trim($_ALUMNO)."','".$txtPW1."')";
		$result =@pg_Exec($conn,$qry);
		if (!$result) {
			...
			if (!$result) {
				...
				$qry="UPDATE ALUMNO SET ID_USUARIO=".$newID." WHERE RUT_ALUMNO='".trim($_ALUMNO)."'";
				$result =@pg_Exec($conn,$qry);
				if (!$result) {
