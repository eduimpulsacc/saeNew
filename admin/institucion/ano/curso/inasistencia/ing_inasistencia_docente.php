...
//echo $contenido = $datos.$i;	
$dia = substr($fechac,0,2);
$mes = substr($fechac,3,2);
$fechaa = $nroAno."-".$mes."-".$dia;


$sql_inasis = "delete from inasistencia_docente where ano = '".trim($ano)."' and fecha = '".trim($fechaa)."'";
...
//		$sql_inasis = "select * from inasistencia_docente where rut_emp = '$rut_emp' and id_ramo = '$id_ramo' and ano = '$ano' and fecha = '$fechaa' and hora = '$horaini'";
//		$res_inasis = pg_Exec($conn, $sql_inasis);
...
//		if(pg_numrows($res_inasis)=="")		
	//	{
			...
			$q8 = "insert into inasistencia_docente (rut_emp, ano, id_curso, fecha, id_ramo, hora, tipo)
			values ('$rut_emp','$ano','$curso','$fechaa','$id_ramo','$horaini','$tipo')";
			$r8 = @pg_Exec($conn,$q8);
	//	}else{
	//		echo "se encunetra";
	//	}
...
<script language="javascript">
	...
	window.location="inasistencia_docente.php";	
</script>	
