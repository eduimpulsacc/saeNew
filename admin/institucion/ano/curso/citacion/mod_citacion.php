...
class Citacion{
	...
	public function ano2($conn,$anio){
		 $sql="SELECT id_ano, nro_ano,situacion FROM ano_escolar WHERE id_ano=".$anio." ORDER BY nro_ano ASC";
		$result = pg_exec($conn,$sql) or die("ERROR");
	...
	public function Apoderado2($conn,$curso){
		$sql="select ap.rut_apo,ap.nombre_apo,ap.ape_pat,ap.ape_mat,al.rut_alumno,al.ape_pat patalu,al.ape_mat matalu,al.nombre_alu from tiene2 t2
inner join apoderado ap on ap.rut_apo = t2.rut_apo
inner join alumno al on al.rut_alumno = t2.rut_alumno
inner join matricula ma on ma.rut_alumno = t2.rut_alumno
where id_curso = $curso and bool_ar=0 and t2.responsable=1
order by ap.nombre_apo,ap.ape_pat,ap.ape_mat ";
$result = pg_exec($conn,$sql);
	...
	public function alumno($conn,$rut_alumno){
		$sql="select * from alumno where rut_alumno = $rut_alumno";
		$result = pg_exec($conn,$sql) or die($sql);
		return $result;
	}
	
	public function buscarAlumnoscombo($conn,$ano,$curso,$apo){
	if($apo==0){
		$sql="select rut_alumno from matricula where id_ano=$ano and bool_ar=0 ";
		if($curso!=0 ){
			$sql.=" and id_curso=$curso ";
		}
	}else{
		
		$sql=" select m.rut_alumno
 from matricula m
 inner join tiene2 on tiene2.rut_alumno = m.rut_alumno
 where id_ano = $ano and id_curso = $curso
 and tiene2.rut_apo = $apo ";
		}
	//echo $sql;
...
public function guardaMensajeCom($conn,$token,$rbd,$curso,$destinatario,$user,$fecha,$hora,$modo,$user_type,$texto,$tipomensaje,$motivo,$ano){
   $sql="insert into sms_comu(token,rdb,id_curso,rut_destino,rut_origen,fecha,hora,modo,tipo_usuario,mensaje,tipo_mensaje,estado,id_motivo,id_ano) values ('$token',$rbd,$curso,$destinatario,$user,'$fecha','$hora','$modo',$user_type,'$texto',$tipomensaje,0,0,$ano)";
