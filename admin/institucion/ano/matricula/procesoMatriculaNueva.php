...
if(pg_numrows($rs_existe)==0){
	...
	// REGISTRA ALUMNO
//	$sql ="INSERT INTO alumno (rut_alumno, dig_rut, nombre_alu, ape_pat, ape_mat, fecha_nac, calle, telefono, region, ciudad, comuna, nacionalidad,sexo, telefono_recado, peso_nace, talla_nace,tipo_parto,edad_madre_nace,s_salud,cant_hermanos,num_hermano,estado_padres,pais_origen,pasaporte,txt_etnia) VALUES (".$txtRUT.",'".$txtDIGRUT."','".$txtNOMBRE."','".$txtAPEPAT."','".$txtAPEMAT."','".$txtFECHA_nac."','".$txtDIRECCION."','".$txtFONO."',".$region.",".$ciudad.",".$comuna.",".$cmbNACIONALIDAD.",".$cmbGENERO.",'".$txtFONORECADOS."','".$txtPESONACE."','".$txtTALLANACE."',".$cmbTIPOPARTO.",".$txtEDADPARTOLAUMNO.",$cmbSALUDP2,$cant_hermanos,$num_hermano,$cmbESTADO,$pais_origen,'$txtPASAPORTE','$txt_etnia')";
$sql ="INSERT INTO alumno (rut_alumno, dig_rut, nombre_alu, ape_pat, ape_mat, fecha_nac, calle, telefono, region, ciudad, comuna, nacionalidad,sexo, telefono_recado, peso_nace, talla_nace,tipo_parto,edad_madre_nace,s_salud,cant_hermanos,num_hermano,estado_padres,sector,pais_origen,pasaporte,txt_etnia,bool_bautismo,bool_pcomunion,bool_confirmacion,religion,celular) 
VALUES (".$txtRUT.",
'".$txtDIGRUT."',
'".str_replace("'","`",$txtNOMBRE)."',
'".str_replace("'","`",$txtAPEPAT)."',
'".str_replace("'","`",$txtAPEMAT)."',
'".$txtFECHA_nac."'
,'".str_replace("'","`",$txtDIRECCION)."',
'".$txtFONO."',
".$region.",
".$ciudad.",
".$comuna.",
...
'".$txtTALLANACE."',
".$cmbTIPOPARTO.",
".$txtEDADPARTOLAUMNO.",$cmbSALUDP2,$cant_hermanos,$num_hermano,$cmbESTADO,'$txtSECTOR',$pais_origen,'$txtPASAPORTE','$txt_etnia',$bool_bautismo,$bool_pcomunion,$bool_confirmacion,'$religion','$txtCELULAR')";
	$rs_alumno = pg_exec($conn,$sql)or die("Fallo al ".$sql);
}else{
	$sql="UPDATE alumno SET 
	nombre_alu='".str_replace("'","`",$txtNOMBRE)."',
	ape_pat='".str_replace("'","`",$txtAPEPAT)."',
	ape_mat='".str_replace("'","`",$txtAPEMAT)."',
	fecha_nac='".$txtFECHA_nac."',
	telefono='".$txtFONO."',
	region=".$region.",ciudad=".$ciudad.",comuna=".$comuna.",sexo=".$cmbGENERO.",
	 telefono_recado='".$txtFONORECADOS."',peso_nace='".$txtPESONACE."',talla_nace='".$txtTALLANACE."',tipo_parto=".$cmbTIPOPARTO.",edad_madre_nace=".$txtEDADPARTOLAUMNO.",s_salud=$cmbSALUDP2,cant_hermanos=$cant_hermanos,num_hermano=$num_hermano,estado_padres=$cmbESTADO,pais_origen=$pais_origen,pasaporte='$txtPASAPORTE',txt_etnia='$txt_etnia',bool_bautismo=$bool_bautismo,bool_pcomunion=$bool_pcomunion,bool_confirmacion=$bool_confirmacion,religion='$religion',celular='$txtCELULAR' WHERE rut_alumno=".$txtRUT;
	$rs_alumno = pg_exec($conn,$sql)or die("Fallo al ".$sql);	
}
	...
	if($textSEGURO=="") $SEGURO='';
	$AUTORIZA=$autoriza_emergencia;
	
	if($txtELECCION=="") $txtELECCION="";
	
	//personajes nuevos que autorizaron a retirar al alumno
	if($txtRUTRETIRA2=="")$txtRUTRETIRA2="";
	if($txtNOMBRERETIRA2=="")$txtNOMBRERETIRA2="";
	if($txtPARENTESCORETIRA2=="")$txtPARENTESCORETIRA2="";
	if($txtFONORETIRA2=="")$txtFONORETIRA2="";
	if($txtCELULARRETIRA2=="")$txtCELULARRETIRA2="";
	
	if($txtRUTRETIRA3=="")$txtRUTRETIRA3="";
	if($txtNOMBRERETIRA3=="")$txtNOMBRERETIRA3="";
	if($txtPARENTESCORETIRA3=="")$txtPARENTESCORETIRA3="";
	if($txtFONORETIRA3=="")$txtFONORETIRA3="";
	if($txtCELULARRETIRA3=="")$txtCELULARRETIRA3="";
	
	//matricula p2
	
	$sql="INSERT INTO matricula (rut_alumno,
	id_ano,
	id_curso,
	rdb,
	fecha,
...
/*	
	...
	if($txtRUTM!=""){
		...
		if(pg_numrows($rs_madre)==0){
			...
			$sql ="INSERT INTO apoderado (rut_apo, dig_rut, nombre_apo, ape_pat, ape_mat, fecha_nac, calle, region, ciudad, comuna, telefono, email, celular, ocupacion, religion, nivel_edu,sexo,sistema_salud,ultimo_ano_aprobado,edad_primer_parto,parentezco,lugar_trabajo,estado_civil,tipo_trabajo,nacionalidad,pais_origen) VALUES(".$txtRUTM.",'".$txtDIGRUTM."','".$txtNOMBREM."','".$txtAPEPATM."', '".$txtAPEMATM."', '".CambioFecha($txtFECHAMADRE)."','".$txtDIRECCIONM."', ".$regionM.", ".$ciudadM.", ".$comunaM.", '".$txtFONOM."', '".$txtMAILM."', '".$txtCELULARM."', '".$txtOCUPACIONM."','".$txtRELIGIONM."','".$txtESTUDIOSM."',".$cmbSEXOMADRE.",".$cmbSALUDM.",'".$cmbULTIMOANOMADRE."',".$txtEDADPRIMERPARTO.",'".$parentesco_madre."','".$txtLUGARTRABAJOM."',".$cmbESTADOCIVILM.",".$cmbTIPOTRABAJOM.",".$cmbNACIONALIDADMADRE.",".$cmbPAISORIGENMADRE.")";
		...
		$sql ="INSERT INTO tiene2 (rut_apo, rut_alumno, responsable) VALUES (".$txtRUTM.",".$txtRUT.",".$sostenedor.")";*/
		$sostenedor=($rdAPODERADO==1)?1:0;
		$responsable=($rdAPODERADO==1)?1:1;
	...
	$sqlsuplente ="SELECT * FROM apoderado WHERE rut_apo=".$txtRUTSUPLENTE;
		...
		$comunaS=$rutaS[2];
			...
			$sql="insert into apoderado (rut_apo,dig_rut,nombre_apo,ape_pat,ape_mat,calle,telefono,region,ciudad,comuna,parentezco,ocupacion,lugar_trabajo,email,ultimo_ano_aprobado,nacionalidad,pais_origen,sexo) values($txtRUTSUPLENTE,'$txtDIGRUTSUPLENTE','$txtNOMBRESU','$txtAPEPATSUP','$txtAPEMATSUP','$txtDIRECCIONSUP','$txtFONOSUP',$regionS,$ciudadS,$comunaS,'$txtPARENTEZCOSUP','$txtOCUPACIONSUP','$txtLUGARTRABAJOSUP','$txtMAILSUP','$cmbULTIMOANOSUP',$cmbNACIONALIDADOTRO,$cmbPAISORIGENOTRO,$cmbSEXOSUP)"	;
