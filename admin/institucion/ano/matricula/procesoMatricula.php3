<?php require('../../../../util/header.inc');
	...
	if(pg_dbname()=="coi_antofagasta"){
		...
		$FechaMatric;
		$fecha_1=explode('-',$FechaMatric);
		$dia1=$fecha_1[0];
		$mes1=$fecha_1[1];
		$ano1=$fecha_1[2];
		
		$FechaMatric=$dia1.'-'.$mes1.'-'.$ano1;
	//exit;
	}
	...
	if(trim($txtCOM)=="" OR trim($txtCOM)==0){
		 $txtCOM='1_1_1';
	}else{
		$dato = split("_",$txtCOM);
...
/////////////////					
					...
					$qry_2="INSERT INTO MATRICULA$ano_act  (RUT_ALUMNO,RDB,ID_ANO,ID_CURSO,FECHA,NUM_MAT,BOOL_BAJ,BOOL_BCHS,BOOL_AOI,BOOL_RG,BOOL_AE,BOOL_I,BOOL_GD,BOOL_AR) VALUES (".trim($txtRUT).",".trim($_INSTIT).",".$_ANO.",".$cmbCURSO.",to_date('" . $FechaMatric . "','DD MM YYYY'),".$NumerMatric.",0,0,0,0,0,0,0,0)";
	...
	//////////////////////				
					...
					if (!$resultC) {
						error('<b> ERROR :</b>Error al acceder a la BD. (31)'.$qryC);
					} 
					...
					$qry = "UPDATE MATRICULA$ano_act SET ID_CURSO=" . $cmbCURSO . ", FECHA=to_date('" . $FechaMatric . "','DD MM YYYY') WHERE (RDB=".$_INSTIT.") AND (RUT_ALUMNO=".$txtRUT.") AND (ID_ANO=".$_ANO.")";
					$result =pg_Exec($conn,$qry);
					...
					$resultC =pg_Exec($conn,$qryC);
					if (!$resultC) {
						error('<b> ERROR :</b>Error al acceder a la BD. (32)'.$qryC);
					}
					if (pg_numrows($resultC)!=0){ 
						for($i=0 ; $i < pg_numrows($resultC) ; $i++){
							$filaC = pg_fetch_array($resultC,$i);
				...
				if((intval(trim($cmbNac)))==1){
					...
					$separa = explode("_",$txtCOM);
					echo $txtCOM."<br>";
					echo $separa[0]."--".$separa[1]."---".$separa[2];
	...
	//////////////////
				$qry_2="INSERT INTO MATRICULA$ano_act  (RUT_ALUMNO,RDB,ID_ANO,ID_CURSO,FECHA,NUM_MAT,BOOL_BAJ,BOOL_BCHS,BOOL_AOI,BOOL_RG,BOOL_AE,BOOL_I,BOOL_GD,BOOL_AR) VALUES ('".trim($txtRUT)."',".$rdb.",".$ano.",".$cmbCURSO.",to_date('" . $FechaMatric . "','DD MM YYYY'),".$NumerMatric.",0,0,0,0,0,0,0,0)";
				$result =@pg_Exec($conn,$qry_2);
	...
	///////////////////////			
				...
				$resultC =@pg_Exec($conn,$qryC);
				if (!$resultC) {
					error('<b> ERROR :</b>Error al acceder a la BD. (33)'.$qryC);
				}
				if (pg_numrows($resultC)!=0){ 
					for($i=0 ; $i < pg_numrows($resultC) ; $i++){
						$filaC = pg_fetch_array($resultC,$i);
...
$separa = explode("_",$txtCOM);
	...
	$result =pg_Exec($conn,$qry);
		...
		if (!$result)
		echo('<b> ERROR :</b>Error al acceder a la BD. (update__)'.$qry);
			...
			if ($filaA['id_curso']!=$cmbCURSO){
						...
						$resultE =pg_Exec($conn,$qryE);
							 ...
							 $qryC="Select * from ramo where id_curso=".$cmbCURSO;
								$resultC =pg_Exec($conn,$qryC);
								  if (!$resultC) {
									error('<b> ERROR :</b>Error al acceder a la BD. (34)'.$qryC);
								   }
									if (pg_numrows($resultC)!=0){ 
										for($i=0 ; $i < pg_numrows($resultC) ; $i++){
											$filaC = pg_fetch_array($resultC,$i);
...
//											$qry ="UPDATE notas$ano_act SET id_ramo=".$ramo." WHERE id_ramo=".$filsNot['id_ramo'];
	...
	$qry="UPDATE MATRICULA SET ID_CURSO=".$cmbCURSO.", FECHA=to_date('" . $FechaMatric . "','DD MM YYYY'), num_mat=".$NumerMatric.", ben_sep=".$sep." WHERE RUT_ALUMNO=".$alumno." AND RDB=".$rdb." AND ID_ANO =".$ano;
	$result =pg_Exec($conn,$qry);
	
	
	$qry="UPDATE MATRICULA$ano_act SET ID_CURSO=".$cmbCURSO.", FECHA='".$FechaMatric."', num_mat=".$NumerMatric.", ben_sep=".$sep." WHERE RUT_ALUMNO=".$alumno." AND RDB=".$rdb." AND ID_ANO =".$ano;
	$result =pg_Exec($conn,$qry);
	
	//if($_PERFIL==0) echo $qry; exit;
	
	
	/*if (!$result)
		error('<b> ERROR :</b>Error al acceder a la BD. (35)'.$qry);
		else
...
if ($frmModo=="eliminar") {
	...
	$qry="DELETE FROM MATRICULA$ano_act WHERE RUT_ALUMNO=".$_ALUMNO." AND RDB=".$_INSTIT." AND ID_ANO =".$_ANO;
	$result =@pg_Exec($conn,$qry);
