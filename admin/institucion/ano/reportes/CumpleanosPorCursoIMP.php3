...
<body onload="javascript:window.print();history.back();">
	<center>
		<table WIDTH="600" BORDER="0" CELLSPACING="1" CELLPADDING="3">
			...
			<?php
				$qry="SELECT alumno.rut_alumno, alumno.dig_rut, alumno.nombre_alu, alumno.ape_pat, alumno.ape_mat, alumno.fecha_nac,curso.id_curso FROM ((alumno INNER JOIN matricula ON alumno.rut_alumno = matricula.rut_alumno) INNER JOIN curso ON matricula.id_curso = curso.id_curso) INNER JOIN ano_escolar ON matricula.id_ano = ano_escolar.id_ano WHERE (((curso.id_curso)=".$curso.") AND ((ano_escolar.id_ano)=".$ano.")) order by alumno.ape_pat, alumno.ape_mat,alumno.nombre_alu ASC";
				$result =@pg_Exec($conn,$qry);
				if (!$result) {
