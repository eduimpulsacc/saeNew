...
echo 	$alumno		    =$c_alumno;
	...
	if ($periodo==0) $sw = 1;
	if ($sw == 1) exit;
/*$query_total_alumnos= "select * from matricula, alumno where id_curso =" . $curso . " and matricula.rut_alumno = alumno.rut_alumno";
$result_total_alumnos=pg_exec($conn,$query_total_alumnos);
$num_total_alumnos=pg_numrows($result_total_alumnos);
	for ($i=0;$i<$num_total_alumnos;$i++){
		$row_total_alumnos=pg_fetch_array($result_total_alumnos);
		$query="select * from ramo where id_curso=curso
		
 		$query="select * from  periodo as perio where perio.id_curso=$row_total_alumnos[id_curso];";
		echo $query."<BR>";
}*/
	...
	//	$muestra_examen=1;
	 ...
	 $query_ramo="select * from ramo where id_curso=$curso and modo_eval=1";
	$result_ramo=pg_exec($conn,$query_ramo);
	$num_ramo=pg_numrows($result_ramo);
	for ($i=0;$i<$num_ramo;$i++){
		$row_ramo=pg_fetch_array($result_ramo);
//		$query_notas="select rut_alumno,id_periodo,promedio from notas2005 as notas where id_ramo='$row_ramo[id_ramo]' order by rut_alumno,id_periodo";
	//	$query_prom="select rut_alumno,avg(to_number(promedio,'000000000000000')) as promedio from notas2005 as notas where id_ramo='$row_ramo[id_ramo]' and promedio<>'0' group by rut_alumno";
		$query_prom="select rut_alumno,sum(to_number(promedio,'000000000000000')) as suma,count(rut_alumno) as cantidad from notas2005 as notas where id_ramo='$row_ramo[id_ramo]' and promedio<>'0' group by rut_alumno";
		...
		for ($j=0;$j<$num_prom;$j++){
			...
			$temporal=$row_prom[suma]/$row_prom[cantidad];
			$xxx=$temporal;
			$temporal=aproxima_prom($temporal,$aproxima);
			$arreglo_prom[$j]=$arreglo_prom[$j]+$temporal;
			//$arreglo_prom[$j]=$row_prom[promedio];
			$arreglo_rut[$j]=$row_prom[rut_alumno];
			if ($j==24){
			echo "<BR>".$j.")".$xxx."--".$row_prom[rut_alumno];
			}
		}
	echo "<BR>".$query_prom."<br>";
		
		$arreglo_antes=$arreglo_prom;
		
	}
		for ($x=0;$x<count($arreglo_prom);$x++){
		$arreglo_prom[$x]=aproxima_prom(number_format($arreglo_prom[$x]/$num_ramo,2),$aproxima);
...
//	echo "---$num_ramo.----";
echo "<table><tr>"."<td>";imprime_array($arreglo_antes); echo "</td><td>";imprime_array($arreglo_prom); echo "</td><td>";imprime_array($arreglo_rut);echo "</td></tr></table>";

	//para sacer los mayores y menores
	$promedio_menor=$arreglo_prom[0];
	$promedio_mayor=$arreglo_prom[0];
	for ($x=0;$x<count($arreglo_prom);$x++){
		if ($arreglo_prom[$x]<$promedio_menor){$promedio_menor=$arreglo_prom[$x];}
		if ($arreglo_prom[$x]>$promedio_mayor){$promedio_mayor=$arreglo_prom[$x];}
		$suma_promedios=$suma_promedios+$arreglo_prom[$x];
		
	}
	$promedio_final_curso=$suma_promedios/count($arreglo_prom);
	
	$promedio_final_curso=aproxima_prom($promedio_final_curso,$aproxima);
	$promedio_menor=aproxima_prom($promedio_menor,$aproxima);
	$promedio_mayor=aproxima_prom($promedio_mayor,$aproxima);
...
//"				and fecha >= to_date('" . $fecha_ini ."','YYYY MM DD') and fecha <= to_date('" . $fecha_fin . "','YYYY MM DD')";
		  ...
		  <tr>
			...
			<? }?>	
			<td class="letra_chica">Final</td>	
			<? if ($muestra_examen){?>
			<td class="letra_chica">Examen<br></td>
			<td class="letra_chica">Final</td>	         
			<? }?>
		          </tr>
		  ...
		  $cont_prom = 0;
		  $promedio = 0;
		  $sql2 = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval,ramo.conex,ramo.pct_examen,ramo.nota_exim ";
...
<? 		$contador_xxx=0;
		...
		?>				
			<td align="center" >&nbsp;<? echo $fila[nota_exim];
				...
				?></td>
			<td  align="center" ><font size="1" face="Arial, Helvetica, sans-serif">&nbsp;<? echo $row_situacion_final[nota_final];?></font></td>	         
		<? }?>
...
*/					 $promedio_periodo_aux = $prome_abajo;
				...
				?>
	            <td align="center"><font size="1" face="Arial, Helvetica, sans-serif">
				<? if($prome_abajo>0) {echo $prome_abajo; }else { echo "&nbsp;"; }?></font></td>								
				<?
				$prome_abajo=0; 
			}
			...
			echo $prom_general_vhs=aproxima_prom($prom_general_vhs,$aproxima);
			$suma_general=0;
			$contado_xxx2=0;
			}?>
			</td>
		...
		<table width="650" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td width="153"><font size="1" face="Arial, Helvetica, sans-serif"><strong>TOTAL DIAS A&Ntilde;O </strong></font></td>
		  ...
		  <tr>
  		  ...
  		  <tr>
			  <td><font size="1" face="Arial, Helvetica, sans-serif">PROMEDIO MENOR</font></td>
			  <td><font size="1" face="Arial, Helvetica, sans-serif"><? echo $promedio_menor;?></font></td>
			  <td><font size="1" face="Arial, Helvetica, sans-serif">PROMEDIO MAYOR</font></td>
			  <td><font size="1" face="Arial, Helvetica, sans-serif"><? echo $promedio_mayor;?></font></td>
		  </tr>

		  <tr>
			  <td><font size="1" face="Arial, Helvetica, sans-serif">PROMEDIO GENERAL DEL CURSO</font></td>
