...
<table width="650" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td><div align="right">
	  <div id="capa0">
	    <?
		if ($_INSTIT==14912 or $_INSTIT==516){  ?>		
	           <input name="button3" TYPE="button" class="botonXX" onClick="MM_openBrWindow('printNotasParciales_3y4_anotaciones27000.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>','','scrollbars=yes,resizable=yes,width=770,height=500')" value="IMPRIMIR">
	  <? }else{ ?>
	           <input name="button3" TYPE="button" class="botonXX" onClick="MM_openBrWindow('printNotasParciales_3y4_anotaciones.php?c_curso=<?=$c_curso ?>&c_alumno=<?=$c_alumno ?>&c_periodos=<?=$c_periodos ?>','','scrollbars=yes,resizable=yes,width=770,height=500')" value="IMPRIMIR">
	  <? } ?>
	  </div>
    </div></td>
  </tr>
				...
				if (pg_numrows($result_peri)>0){
					...
					if (chop($fila_peri['promedio'])=="0" or empty($fila_peri['promedio'])){
						...
						if ($fila_peri['promedio']>0){
							...
							if ($_INSTIT==27000){							
								///// nuevo proceso para sacar el promedio en número ////
								//// calculo el promedio de religión  /////
								$contador_re=0;
								$acomulo_re = 0;
								for ($xx=1; $xx < 19; $xx++){ 								
								    $nota_re = $fila_peri['nota'.$xx];
									if ($nota_re > 0){
									   $acomulo_re = $acomulo_re + $nota_re;
									   $contador_re++;									
									}							    
								}
								/// calculo el promedio de religión
								$prome_re = @round($acomulo_re/$contador_re);
...
<?	}
 ...
 //// AQUI LA NUEVA FUNCION DE DE MOSTRAR LAS ANOTACIONES TAMBIEN
    ?>
	...
	$sql_anota = $sql_anota . "where rut_alumno = ".$alumno." and id_periodo='$c_periodos'";
...
//			$tipo = "AVANZADO";
		...
		$profesor_res = strtoupper($fil_emp['ape_pat'] . " " . $fil_emp['ape_mat'] . " " . $fil_emp['nombre_emp']);
...
?>
  ...
  if($fila_anota["codigo_anotacion"]!=""){?>
    ...
    <?
	$sql_asis = "select * from asistencia where rut_alumno = ".$alumno." and ano = ".$ano." and id_curso='$c_curso'";
...
<input name="" type="button" value="Nueva B&uacute;squeda"  class="botonXX" onClick="window.location='NotasParciales_Taller_2.php?flag=0'">
