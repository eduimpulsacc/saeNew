...
$rs_atenciones=$ob_reporte->grafVelLectora($conn);
...
$(function () {
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'GRAFICO VELOCIDAD LECTORA'
        },
        subtitle: {
            text: ''
        },
       ...
       pointFormat: '<span style="color:{series.color}"></span> {series.name}: <b>{point.y:.0f}</b><br/>'
		...
		//$ob_reporte->patologia=$fila_atencion['nombre'];
		//$rs_datopatologia = $ob_reporte->Patologia($conn);
		//$patologia = pg_result($rs_datopatologia,1);
		//echo "['".strtoupper($fila_atencion['nombre']) ."',   ".$fila_atencion['cuenta']."],";
		?>
		{
				
                name: '<?php echo $fila_atencion['nombre'] ?>',
