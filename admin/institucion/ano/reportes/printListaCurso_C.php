...
function exportar(){
			...
			window.location='printListaCurso_C.php?cmb_curso=<?=$curso?>&c_reporte=<?=$reporte?>&ck_retirado=<?php echo $ck_retirado ?>';
  ...
  <table width="550" border="0" cellspacing="1" cellpadding="3">
    ...
    <?php
		...
		for($i=0 ; $i < @pg_numrows($result) ; $i++){ 
			...
			$nombre_alu=ucwords(strtoupper($ob_reporte->ape_nombre_alu));
			?>
    ...
    <? } ?>
      <td align="left" class="subitem" ><strong>&nbsp;&nbsp;
            <?=$ob_reporte->tildeM(strtoupper($nombre_alu));?>
      </strong> </td>
    </tr>
    <?php
					}
