		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<? } ?>
				  ...
				  <tr>
					...
					if($institucion==8930 or $institucion==770){
      ...
      <tr>
		...
		<?
		  ...
		  <tr>
			...
			<td align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><b>General</b></font></td>			         
            </tr>
		  <?
		  $cont_prom = 0;
		  $promedio = 0;
		  ...
		  /// aqui para ver si el ramo es padre o hijo
				$qryaux="SELECT ramo.id_ramo, subsector.nombre, id_formula, modo FROM subsector INNER JOIN ramo ON subsector.cod_subsector = ramo.cod_subsector INNER JOIN formula ON ramo.id_ramo=formula.id_ramo WHERE (((ramo.id_curso)=".$curso.")) order by ramo.id_orden";
				$resaux = @pg_Exec($conn,$qryaux);
				$filaaux = @pg_fetch_array($resaux,0);
				$id_formula = $filaaux['id_formula'];				
		//**********************************************	
...
//		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.")); ";
		  ...
		  <?
			...
			?>
            <td height="25"><div align="left"><font size="1" face="Arial, Helvetica, sans-serif">
			...
			$num_periodos = @pg_numrows($resultPer);
			$suma_promedios_concept = 0;
			...
			{				
				...
				if (pg_numrows($result_peri)>0){
					...
					if (trim($promedio)=="MB" or trim($promedio)=="B" or trim($promedio)=="S" or trim($promedio)=="I"){
					    $suma_promedios_concept = $suma_promedios_concept + Conceptual($promedio,2);
					}
				} else {
					$prome_1 = "&nbsp;";
				}
	            ...
	            <td align="center"><font size="1" face="Arial, Helvetica, sans-serif"><? 
			     ...
			     $prom_g_sub =  $prom_g_sub +  $prome_1; 		
													
			}	
			
				
			if ($prom_g_sub > 0){
	       	     $prom_g_sub = round($prom_g_sub / $contador_periodos);
			}else{  /// de lo contrario, puede ser Religion
			
			     $prom_g_sub = Conceptual(intval($suma_promedios_concept/2),1);
			}			
			 ?>				
             <td align="center">
			  ...
			  <?
			  if (($prom_g_sub!=0) or ($prom_g_sub!=NULL)){
					?>
...
<!--            <td height="25" colspan="21" align="right"><font size="1"><strong><font face="Arial, Helvetica, sans-serif">Promedio de <? //echo ucwords(strtolower($fila1['nombre_periodo']))?></font></strong></font></td> -->
            <td height="25" colspan="21" align="right"><font size="1"><strong><font face="Arial, Helvetica, sans-serif">Promedio periodos  &nbsp;&nbsp;&nbsp;</font></strong></font></td>
			<?
			$sqlperiodos = "Select * from periodo where id_ano = $ano order by fecha_inicio";
			$resultPer =@pg_Exec($conn,$sqlperiodos);			
			$prome_abajo = 0;
...
*/	
				$sql_peri = "SELECT n.promedio, n.id_ramo FROM notas$nro_ano n ";
				...
				$sql_peri = $sql_peri ." AND n.id_periodo=".$periodos;
				$result_peri =@pg_Exec($conn,$sql_peri);
				$num_prom  = @pg_numrows($result_peri);
		        ...
		        for($pm=0 ; $pm < @pg_numrows($result_peri) ; $pm++)
				{
					...
					$id_ramo_aux = $filapm['id_ramo'];
					
					//echo "id_ramo: $id_ramo_aux <br>";
					
					$qry_formula  ="SELECT * from formula_hijo where id_hijo='$id_ramo_aux'";
					$Rs_formula   =pg_Exec($conn,$qry_formula);
					$num_formula  =pg_numrows($Rs_formula);
				...
				if ($prome_abajo>0){
					...
					$prome_abajo = round($prome_abajo/$cont_abajo);
...
*/					 $promedio_periodo_aux = $prome_abajo;
          ...
          <tr>
            <td height="25" colspan="23" align="right"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>Promedio General</strong> </font></td>
            <td align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"> &nbsp;<? echo $prome_general_pro;?>&nbsp;</font></td>
          </tr>
...
<!-- fin taller -->			
		...
		<table width="650" height="35" border="0" cellpadding="0" cellspacing="0">
		  <tr>
			<td height="16"><div align="left"><font size="1"><strong><font face="Arial, Helvetica, sans-serif" size="1">Con excepción de 1º y 2º medio, las asignaturas (entre paréntesis) no inciden en el promedio final de notas. Ambas constituyen la nota de la asignatura en negrita que les antecede.</font></strong></font></div></td>
		  </tr>		 
		</table>
...
if ($fila['ensenanza']>800 and $fila['grado_curso']>10){
?>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td>&nbsp;</td>
...
//			if((trim($fila1['nombre_periodo'])=="SEGUNDO TRIMESTRE") OR (trim($fila1['nombre_periodo'])=="TERCER TRIMESTRE")){ 
		...
		//**********************************************
		  ...
		  $sql2 = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval, ramo.bool_ip ";
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene$nro_ano.rut_alumno)='".$alumno."') and (ramo.cod_subsector<60000 and ramo.cod_subsector <> 250)) and ramo.sub_obli=1 order by ramo.id_orden";
...
?>
		...
		//**********************************************
		  ...
		  $sql2 = "SELECT ramo.id_ramo, subsector.nombre, ramo.modo_eval, ramo.bool_ip ";
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector) INNER JOIN tiene$nro_ano ON (ramo.id_curso = tiene$nro_ano.id_curso) AND (ramo.id_ramo = tiene$nro_ano.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene$nro_ano.rut_alumno)='".$alumno."') and (ramo.cod_subsector>999 or ramo.cod_subsector = 250)) and ramo.sub_obli=2 order by ramo.id_orden";
          $result =@pg_Exec($conn,$sql2);
		  if (!$result) 
		  {
			  error('<B> ERROR :</b>Error al acceder a la BD. (SUBSECTOR)</B>');
