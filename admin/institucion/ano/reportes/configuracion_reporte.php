<?php 	require('../../../../util/header.inc');
	...
	if($caso==1){
		...
		$empleado1	=$fila_rep['empleado1'];
		$empleado2	=$fila_rep['empleado2'];
		$empleado3	=$fila_rep['empleado3'];
		$empleado4	=$fila_rep['empleado4'];
		...
		$grado13	=$fila_rep['grado13'];
		$grado14	=$fila_rep['grado14'];
		$grado15	=$fila_rep['grado15'];	
		$grado16	=$fila_rep['grado16'];	
		$ensenanza	=$fila_rep['tipo_ense'];
		$timbre	=$fila_rep['poner_timbre'];	
		$firmadig1	=$fila_rep['firmadig1'];
		$firmadig2	=$fila_rep['firmadig2'];
		$firmadig3	=$fila_rep['firmadig3'];
		$firmadig4	=$fila_rep['firmadig4'];
		$grado23	=$fila_rep['grado23'];
		$grado24	=$fila_rep['grado24'];
		$grado25	=$fila_rep['grado25'];
		$grado32	=$fila_rep['grado32'];
		$grado33	=$fila_rep['grado33'];
		$grado31	=$fila_rep['grado31'];
	}
	


$sql = "select DISTINCT b.rut_emp,b.ape_pat || cast(' ' as varchar) || b.ape_mat || cast(' ' as varchar) || b.nombre_emp as nombre from trabaja a INNER JOIN empleado b ON a.rut_emp=b.rut_emp where a.rdb=".$institucion." ORDER BY nombre ASC ";
						...
						<table width="100%" border="0">
                          <tr>
                            <td colspan="2"><div align="right">
							  ...
							  <input name="eliminar" type="button" class="botonXX" id="eliminar" onClick="window.location='procesaReporte2.php?caso=1&eliminar=si&id_config=<?=$id_config;?>&id_item=<?=$cmbREPORTE;?>'" value="ELIMINAR">
							  <? } ?>
                          ...
                          <tr>
                            <td colspan="2"><table width="100%" border="0">
                              <tr>
                                <td width="17%"  class="textosimple">Cargo que firma  </td>
                                <td width="28%"  class="textosimple">
								<select name="cmbFIRMA1" onChange="recargarFirma( this );">
                                ...
                                <td width="3%">&nbsp;</td>
                                <td width="15%" class="textosimple">Cargo que firma  </td>
                                <td width="37%" class="textosimple">
								<select name="cmbFIRMA2" onChange="recargarFirma2( this );">
								<option value="0">Seleccione</option>
								<? 	$result_cargo=$ob_reporte->Cargos($conn);
									for($i=0;$i<@pg_numrows($result_cargo);$i++){
										$fila = @pg_fetch_array($result_cargo,$i);
							  ...
							  <div id="firma1" style="visibility:hidden; display:block ">
                              <tr>
                                <td  class="textosimple">Persona que firma </td>
                                <td  class="textosimple">
								<select name="cmbEMPLEADO1">
									<option value="0">seleccione</option>
									<? for($xx=0; $xx<@pg_numrows($rs_empleado);$xx++){
											$fila_emp = @pg_fetch_array($rs_empleado,$xx);
											if($fila_emp['rut_emp']==$empleado1){
									?>
									<option value="<?=$fila_emp['rut_emp'];?>" selected="selected"><?=substr($fila_emp['nombre'],0,30);?></option>
									<? 		}else{?>
									<option value="<?=$fila_emp['rut_emp'];?>"><?=substr($fila_emp['nombre'],0,30);?></option>
									<? 		}
										} ?>
								</select>
								</td>
                                ...
                                <td class="textosimple">Persona que firma </td>
                                <td class="textosimple">
								<select name="cmbEMPLEADO2">
									<option value="0">seleccione</option>
									<? for($xx=0; $xx<@pg_numrows($rs_empleado);$xx++){
											$fila_emp = @pg_fetch_array($rs_empleado,$xx);
											if($fila_emp['rut_emp']==$empleado2){
									?>
									<option value="<?=$fila_emp['rut_emp'];?>" selected="selected"><?=substr($fila_emp['nombre'],0,30);?></option>
									<? 		}else{ ?>
									<option value="<?=$fila_emp['rut_emp'];?>"><?=substr($fila_emp['nombre'],0,30);?></option>
                              ...
                              <tr>
                                <td  class="textosimple">Insertar firma Digital</td>
                                <td  class="textosimple"><input name="ckboxfir1" type="checkbox" id="ckboxfir1" value="1" <? if($firmadig1==1) echo "checked";?>></td>
                                <td>&nbsp;</td>
                                <td class="textosimple">Insertar firma Digital</td>
                                <td class="textosimple"><input name="ckboxfir2" type="checkbox" id="ckboxfir2" value="1" <? if($firmadig2==1) echo "checked";?>></td>
                              </tr>
                              <tr>
                              ...
                              <tr>
                                <td class="textosimple">Cargo que firma </td>
                                <td class="textosimple">
								<select name="cmbFIRMA3" onChange="recargarFirma3( this );">
								<option value="0">Seleccione</option>
								<? 	$result_cargo=$ob_reporte->Cargos($conn);
									for($i=0;$i<@pg_numrows($result_cargo);$i++){
										$fila = @pg_fetch_array($result_cargo,$i);
                                ...
                                <td class="textosimple">Cargo que firma </td>
                                <td class="textosimple">
								<select name="cmbFIRMA4" onChange="recargarFirma4( this );">
								<option value="0">Seleccione</option>
								<? 	$result_cargo=$ob_reporte->Cargos($conn);
									for($i=0;$i<@pg_numrows($result_cargo);$i++){
										$fila = @pg_fetch_array($result_cargo,$i);
                              ...
                              <tr>
                                <td class="textosimple">Persona que firma </td>
                                <td class="textosimple">
								<select name="cmbEMPLEADO3">
									<option value="0">seleccione</option>
									<? for($xx=0; $xx<@pg_numrows($rs_empleado);$xx++){
											$fila_emp = @pg_fetch_array($rs_empleado,$xx);
											if($fila_emp['rut_emp']==$empleado3){
									?>
									<option value="<?=$fila_emp['rut_emp'];?>" selected="selected"><?=substr($fila_emp['nombre'],0,30);?></option>
									<? 		}else{?>
									<option value="<?=$fila_emp['rut_emp'];?>"><?=substr($fila_emp['nombre'],0,30);?></option>
                                ...
                                <td class="textosimple">Persona que firma </td>
                                <td class="textosimple">
								<select name="cmbEMPLEADO4">
									<option value="0">seleccione</option>
									<? for($xx=0; $xx<@pg_numrows($rs_empleado);$xx++){
											$fila_emp = @pg_fetch_array($rs_empleado,$xx);
											if($fila_emp['rut_emp']==$empleado4){
									?>
									<option value="<?=$fila_emp['rut_emp'];?>" selected="selected"><?=substr($fila_emp['nombre'],0,30);?></option>
									<? 		}else{?>
									<option value="<?=$fila_emp['rut_emp'];?>"><?=substr($fila_emp['nombre'],0,30);?></option>
                              ...
                              <tr>
                                <td class="textosimple">Insertar firma Digital</td>
                                <td class="textosimple"><input name="ckboxfir3" type="checkbox" id="ckboxfir3" value="1" <? if($firmadig3==1) echo "checked";?>></td>
                                <td>&nbsp;</td>
                                <td class="textosimple">Insertar firma Digital</td>
                                <td class="textosimple"><input name="ckboxfir4" type="checkbox" id="ckboxfir4" value="1" <? if($firmadig4==1) echo "checked";?>></td>
                              </tr>
                              <tr>
                                <td class="textosimple">&nbsp;</td>
                                <td class="textosimple">&nbsp;</td>
                              ...
                              <tr>
                                <td class="textosimple">Agregar timbre</td>
                                <td class="textosimple"><input name="ckboxtim" type="checkbox" id="ckboxtim" value="1" <? if($timbre==1) echo "checked";?>></td>
                                <td>&nbsp;</td>
                                <td class="textosimple">&nbsp;</td>
                                <td class="textosimple">&nbsp;</td>
                              </tr>
                          ...
                          <tr>
                            <td colspan="2"><table width="100%" border="0">
                              ...
                              <tr>
                                <td colspan="2" align="right" class="textosimple">(S&oacute;lo educaci&oacute;n especial)</td>
                                <td class="textosimple">
                                13<input type="checkbox" name="ckbox13" value="1" <? if($grado13==1) echo "checked";?>>
								14<input type="checkbox" name="ckbox14" value="1" <? if($grado14==1) echo "checked";?>>
								15<input type="checkbox" name="ckbox15" value="1" <? if($grado15==1) echo "checked";?>>
                                16<input type="checkbox" name="ckbox16" value="1" <? if($grado16==1) echo "checked";?>>
                                </td>
                              </tr>
                              <tr>
                                <td colspan="2" align="right" class="textosimple">&nbsp;</td>
                                <td class="textosimple">23
                                  <input name="ckbox23" type="checkbox" id="ckbox23" value="1" <? if($grado23==1) echo "checked";?>>
24
<input name="ckbox24" type="checkbox" id="ckbox24" value="1" <? if($grado24==1) echo "checked";?>>
25
<input name="ckbox25" type="checkbox" id="ckbox25" value="1" <? if($grado25==1) echo "checked";?>>
31
<input name="ckbox31" type="checkbox" id="ckbox31" value="1" <? if($grado31==1) echo "checked";?>>
32
<input name="ckbox32" type="checkbox" id="ckbox32" value="1" <? if($grado32==1) echo "checked";?>>
33
<input name="ckbox33" type="checkbox" id="ckbox33" value="1" <? if($grado33==1) echo "checked";?>></td>
                              </tr>
                            </table></td>
                          </tr>
                        </table>
