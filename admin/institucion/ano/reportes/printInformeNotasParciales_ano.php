...
?>
<br />
<table width="850" border="0" align="center" cellpadding="0" cellspacing="0" style="font-size:10px">
  <tr>
    <? if ($institucion!="770"){ ?>
...
<table width="850" border="0" align="center" style="font-size:10px">
  <tr>
    <td class=""><div align="center"><strong>INFORME DE NOTAS <? echo ucwords(strtoupper($ano_pal))?></strong> </div></td>
  </tr>
     <?
	  if ($institucion==770){
	      ?>
   ...
   <? } ?>
      <!--<tr>
        <td><div align="center"><font size="1" face="Arial, Helvetica, sans-serif"><strong></strong></font></div></td>
      </tr>-->
...
$ob_reporte->AlumnoInscrito2($conn);
...
*/
	...
	if($fila_n["nota".$n] >0 || ( trim($fila_n["nota".$n])=="MB" || trim($fila_n["nota".$n])=="B") || trim($fila_n["nota".$n])=="S" || trim($fila_n["nota".$n])=="I" ){
		...
		if($fila_r['modo_eval']==2 || $fila_r['modo_eval']==3){
			$promedio_religion = $promedio_religion + $fila_n['nota'.$n.''];	
			$cont_religion++;
		}
	}	
...
if($fila_n['promedio']>0 && $fila_r['modo_eval']==1){
	...
	if($fila_n['promedio']!=""){
		$promedio_religion = Conceptual($fila_n['promedio'],0);
	}
	if($truncado_per==1){
	...
	/*if($_PERFIL==0){
		...
		echo $prom_religion;
		echo "<br>".$cont_religion;
	}*/
	$prom_gral_religion = $prom_gral_religion + $promedio_religion;
	
}
if($fila_r['bool_pgeneral']==1 && $fila_r['modo_eval']==1 && $fila_n['promedio']>0){
		 //$cont_pge++;
		 $npt[$alumno][$periodo][]= $promedio;
...
if($fila_r['modo_eval']==1 && $suma_promedio_total>0 && $fila_r['bool_ip']==1){
	$cuenta_promedio++;
}else{
	if($_PERFIL==0){
	...
	echo $suma_promedio_total = Conceptual(round($prom_gral_religion / $cont_gral_religion),1) ;	
}
...
<td align="center"><? 
  ...
  // $total_prom_gral = $suma_prom_g/$suma_count;
   $total_prom_gral =  array_sum($npt[$alumno][$fila_periodoo['id_periodo']])/count($npt[$alumno][$fila_periodoo['id_periodo']]);
   if($truncado_sem==1){
   ...
   <font color="red">
   <?
	echo array_sum($npt[$alumno][$fila_periodoo['id_periodo']] );	
	echo "-". count($npt[$alumno][$fila_periodoo['id_periodo']] );	
	?>
    </font>
    <?
	}?>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="textosimple" style="font-size:10px">
  <tr>
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0" align="center" style="font-size:10px">
     <tr>
	    <td height="17" class="textosimple"><div align="left">	Observaciones</div></td>
    </tr>
    <?php if(strlen($fila_alu['obs_reporte'])>0){?>
     <tr>
       <td height="17" class="textosimple" valign="top"><?php echo $fila_alu['obs_reporte'] ?></td>
     </tr>
    <?php  }else{?>
    <tr>
	    <td height="27"><div align="left">___________________________________________________________________________________________________________________________________________________</div></td>
    </tr>
     <tr>
	    <td height="27"><div align="left">___________________________________________________________________________________________________________________________________________________</div></td>
    </tr>
...
<table width="850" align="center" style="font-size:10px">
	<tr>
	<td>
    <?
    $fecha = strftime("%d %m %Y");		
    ?>
<font face="Verdana, Arial, Helvetica, sans-serif" size="-1"><? echo ucwords(($ob_membrete->comuna)).", ". fecha_espanol($fecha) ?></font>
	</td>
	</tr>
	</table><br />
<br>
