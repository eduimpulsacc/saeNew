...
?>
	<? if ($guardar){
	      $sql_borra_detalle = "delete from concentracion_detalle where rut_alumno = '$rut_alumno' and curso = '$curso'";
		  $res_borra_detalle = @pg_Exec($conn,$sql_borra_detalle);
		  
		  $sql_borra_concentracion = "delete from concentracion_notas where rut_alumno = '$rut_alumno' and curso = '$curso'";
		  $res_borra_concentracion = @pg_Exec($conn,$sql_borra_concentracion);
		  
		  
		  //// GRABAMOS LA NUEVA INFORMACION
		//if($_PERFIL==0){
			$query_insert="insert into concentracion_notas (rut_alumno,institucion,ano,curso,promedio,asistencia,ciudad,comuna,plan,decreto,letra) 
	      	values('$rut_alumno','$institucion','$ano','$curso','$promedio','$asistencia','$ciudad','$comuna','$plan','$decreto','$letra')";
		...
		/*}else{
		  ...
		  $result_insert=@pg_Exec($conn,$query_insert) or die(pg_last_error($conn));
		
		  $largo=count($notas);
		
		  for ($i=0;$i<$largo;$i++){
			 if ($notas[$i]!=""){
				if($subsector[$i]=='13' or $subsector[$i]==729 or $subsector[$i]==9863){
 ...
 /// aqui recibo el rut del alumno y según los cursos que tenga en concentracion de notas
 /// lleno el select
 ...
 if  (($curso!=0) AND ($curso!=NULL)){
	  ...
	  $comuna           = $fila_info_curso['comuna'];
	  $plan             = $fila_info_curso['plan'];
	  $decreto          = $fila_info_curso['decreto'];
	  $letra   	        = $fila_info_curso['letra'];
...
var opciones="left=100,top=100,width=500,height=600,scrollbars=yes,resizable=yes", i= 0;
...
function valida(){
	...
	if (a.ano.value==''){
		error=error+'debe ingresar el año\r';
	}else{
...
function redireccion(form){
	 ...
	 window.location="editar.php?rut_alumno=<?=$rut_alumno ?>&curso="+curso
}
</script>
</head>
<body>
...
<?  
   ...
   for ($i=1;$i<4;$i++){
		...
		where mat.rut_alumno='$rut_alumno' and mat.id_ano=ano.id_ano and curso.id_curso=mat.id_curso and curso.grado_curso='$i' and curso.ensenanza>300
	...
	*/
	...
	if ($num_concentracion>0){
	    for ($i=0; $i < $num_concentracion; $i++){
		    $row_concentracion=pg_fetch_array($result_concentracion,$i);
   ...
   <table width="1%" border="1" align="center" class="td2">
	<tr>
	  <td colspan="2" class="td2"><span class="Estilo5">EDICI&Oacute;N</span><br />
      curso:</td>
	</tr>
	<tr>
		     ...
		     <td class="td2">
			   <? if (isset($curso)){ 
				   ...
				   <select name="curso" onChange="redireccion(this.form);">
					  ...
					  <? for ($i=0; $i<$num_cursos_concentracion; $i++){?>
							<option value="<?=$arreglo_curso[$i] ?>" <? if ($curso==$arreglo_curso[$i]){ ?> selected="selected" <? } ?>><?=$arreglo_curso[$i] ?></option>			  
					  <? } ?>
				   </select>
			<? } ?>	   
			 </td>
...
<? if (($curso!=0) AND ($curso!=NULL)){  ?>
	...
	<td class="td2"><input name="comuna" class="editbox " value="<?=$comuna ?>" /></td>
	</tr>
	<tr>
	<td class="td2 Estilo3 Estilo4">LETRA</td>
	<td class="td2"><input name="letra" class="editbox " value="<? echo trim($letra); ?>" maxlength="1" /></td>
	</tr>
	<tr>
...
<tr><td colspan="2" class="td2 Estilo3 Estilo4"><table width="100%" class="td2" id="mytable" >
  ...
  while ($i < $num_conc_detalle){ 
	  ...
	  ?> 
      <tr>
      <td nowrap="nowrap" class="td2"><?=$nombre_subsector ?> 
	    <input name="subsector[]" type="hidden"  value="<?=$subsector ?>"/>
	 </td>
      <td nowrap="nowrap" class="td2"><input name="notas[]" class="editbox" size="5" maxlength="2"  value="<? if ($promedio_sub!=''){ echo $promedio_sub; }else{ echo $religion; } ?>" /></td>
      </tr>
      <?
	  $i++;
  ...
  <table width="100%" class="td2" id="mytable" >
  <tr>
    <td class="td2">SUBSECTOR</td>
    <td class="td2">PROM</td>
  </tr>
  ...
  <table width="100%">
  <tr>
    <td colspan=2 align="center"><a href="javascript:;"  onclick="window_open('buscador2.php');"><b>&lt;---Agregar---&gt;</b></a></td>
  </tr>
...
<tr>
	<td colspan="2" align="center">
		...
		<input name="guardar" type="submit" class="botonXX" id="cb_ok" value="ACTUALIZAR INFORMACION"/></td>
</tr>
