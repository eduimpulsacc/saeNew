	  ...
	  if($institucion!=""){
		   ...
		   echo "<img src='".$d."menu/imag/logo.gif' >";
              ...
              <tr>
                ...
                <td><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>:</strong></font></td>
                <td><div align="left"><font face="Verdana, Arial, Helvetica, sans-serif" size="1">
                  <? $nombre_alumno = ucwords(strtoupper(trim($fila['ape_pat']) . " " . trim($fila['ape_mat']) . " " . trim($fila['nombre_alu']))); echo $ob_reporte->tildeM($nombre_alumno);?>
                </font></div></td>
              </tr>
              <tr>
		  ...
		  $conexper=$fila_lista['conexper'];?>
                <td class="textonegrita"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><strong>CON EXAMEN</strong></font></td>
                <td class="textonegrita">:</td>
                <td class="textosimple"><? echo ($conexper==1)?"SI":"NO";?></td>
      ...
      <tr>
		...
		<?
		  ...
		  while ($arr_lista = pg_fetch_array($res_lista)) {
			  ...
			  ?>
		  		<tr>
          			<td><font size="1" face="Arial, Helvetica, sans-serif"><?=$arr_lista['nombre']?></font>
...
$conexper=$arr_lista['conexper'];

if($conexper==1){

$sql_conexamen="SELECT subsector.nombre,ramo.id_ramo,ramo.conexper, sp.id_periodo, sp.rut_alumno, sp.nota_final,  sp.nota_examen, sp.prom_gral, sp.id_ramo FROM ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector LEFT outer JOIN situacion_periodo as sp on ramo.id_ramo = sp.id_ramo WHERE ramo.id_curso=".$curso." and sp.rut_alumno=".$alumno."    and sp.id_periodo =".$id_periodo." and ramo.conexper=1 ORDER BY ramo.id_orden";	
$result_con =pg_Exec($conn,$sql_conexamen)or die("Fallo :".$sql_conexamen);
$filaconexamen = @pg_fetch_array($result_con,0);  
$prom = $filaconexamen['nota_final'];
}								
    ...
    <font size="1" face="Arial, Helvetica, sans-serif" <?=$color?>><?=$prom?></font>
