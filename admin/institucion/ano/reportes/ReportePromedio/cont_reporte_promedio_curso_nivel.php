...
require "mod_reporte_promedio_curso_nivel.php";

//$objMembrete= new Membrete($_IPDB,$_DBNAME);
$obj_PromediosCursoNivel = new PromediosCursoNivel($conn);
$funcion = $_POST['funcion'];
	...
	if($funcion == 0){
		  ...
		  $rdb=$_POST['rdb'];	
		  $result = $obj_PromediosCursoNivel->carga_anos($rdb);
		  if($result){
		$select = "<select name='select_anos' id='select_anos' onchange='carga_nivel(this.value)'>
	...
	if($funcion == 1){
		   ...
		   $id_ano=$_POST['id_ano'];	
		  $result = $obj_PromediosCursoNivel->carga_nivel($id_ano);
		  if($result){
	...
	if($funcion == 2){
		  ...
		  $result = $obj_PromediosCursoNivel->carga_cursos($id_ano,$id_nivel);
		  if($result){
	...
	if($funcion == 3){
		  ...
		  $result = $obj_PromediosCursoNivel->carga_asignatura($id_curso);
		  if($result){
	...
	if($funcion == 4){
		   ...
		   $id_nivel=$_POST['id_nivel'];	
		   $id_ano=$_POST['id_ano'];
		  $result = $obj_PromediosCursoNivel->carga_ramos_nivel($id_nivel,$id_ano);
		  if($result){
	...
	if($funcion == 2){
		   ...
		   $id_ano=$_POST['id_ano'];	
		  $result = $obj_PromediosCursoNivel->carga_cursos($id_ano);
		  if($result){
		...
		if($funcion == carga_alumnos){
			$id_curso=$_POST['id_curso'];
			 $id_ano=$_POST['id_ano'];
		  $result = $obj_PromediosCursoNivel->carga_alumnos($id_curso,$id_ano);
