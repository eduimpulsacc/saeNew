<?php 	
	...
	if($caso!=1){
		$ob_reporte->InsertReporte2($conn);
	}else{	
			if($eliminar=="si"){
			
			echo "config:".$ob_reporte->id_config=$_GET['id_config'];
			echo "item:".$ob_reporte->id_item=$_GET['id_item'];
			 
			$ob_reporte->EliminaReporte($conn);
			
			}else{
		
			$ob_reporte->ModificaReporte2($conn);
			}
	}
