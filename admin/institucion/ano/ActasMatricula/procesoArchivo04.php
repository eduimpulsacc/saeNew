...
if($_GET['grado_filtro']!=0){
	$grado_filtro = $_GET['grado_filtro'];
	$filtro = explode("_",$_GET['grado_filtro']);
    $gradocurso_filtro = $filtro[0];
	$ensenanza_filtro = $filtro[1];
}else{
	$grado_filtro = 0;
	}
   ...
   //// FUNCION QUE VALIDA EL RUT   ///////
	...
	if($_GET['grado_filtro']!=0){ 
	$sql = "select * from curso where grado_curso = ".$filtro[0]." and  id_ano = ".$ano." and ensenanza = ".$filtro[1]."; "; 
	}else{
    ...
    $total_filas= pg_num_rows($rsCurso);		
	
	if ($num < $total_filas){
		
		$fCurso = @pg_fetch_array($rsCurso,$num);	
		echo "<br/>".$curso = $fCurso['id_curso'];
	
			
$sql = "SELECT institucion.rdb, institucion.dig_rdb, institucion.region, curso.ensenanza, curso.id_curso, curso.truncado_per, curso.grado_curso, curso.letra_curso, ano_escolar.nro_ano, matricula.rut_alumno, alumno.dig_rut, curso.cod_decreto, curso.cod_eval , matricula.bool_ar ";
			...
			$sql = $sql . "WHERE (((institucion.rdb)=".$institucion.") AND ((ano_escolar.id_ano)=".$ano.") and matricula.id_ano = ".$ano.") and curso.ensenanza > 109 and curso.id_curso = $curso and ((matricula.bool_ar<>1) or ((matricula.bool_ar isnull) or (matricula.bool_ar=0)))";
			...
			for ($yy=0; $yy < pg_numrows($resultado_query); $yy++){
				...
				$bool_ar    = $fila_curso['bool_ar']	;
				//--
				
				$sql_subsector 		= "select * from ramo where id_curso = $curso and (bool_ip = 1 or cod_subsector = 13)";
				...
				for ($i=0; $i < pg_numrows($result_subsector); $i++)
				{
					...
					$sub_obli		= $fila_subsector['sub_obli'];
					...
					//--
					/*if($_PERFIL==0 and $alumno==17700750){
						echo $sql_eximido;
						exit;
					}*/
					if ($fila_eximido['cantidad']==0 )//and $sub_obli==1
					{
						$promedio1 = ""		;
						$promedio2 = ""		;
						$promedio3 = "EX"	;
						
					}/*elseif ($fila_eximido['cantidad']==0 and $sub_obli==2 and $subsector==13){
						$promedio1 = ""		;
						$promedio2 = ""		;
						$promedio3 = "N/O"	;
					}*/
					else
					{
						...
						//{
							...
							//$sql_notas = "SELECT notas$nro_ano.promedio FROM notas$nro_ano WHERE 
							//(((notas$nro_ano.rut_alumno)='".$alumno."') AND ((notas$nro_ano.id_ramo)=".$ramo."));";
								
								$sql_notas = "SELECT promedio FROM promedio_sub_alumno WHERE rdb=".$institucion." AND id_ano=".$ano." AND rut_alumno=".$alumno." AND id_ramo=".$ramo."";
								...
								if ($prom>0 and $bool_ar != "1")
									...
									if (($subsector!=13 or $subsector!=2073 or $subsector!=5002 or $subsector!=249 or $subsector!=170 or $subsector!=144  )and $prom==NULL  and $bool_ar!="1"){
									     $promedio2 = "";
								         $promedio3 = "EX";
									}
							...
							else
							{
								...
								if ($prom>0)
									...
									if ($subsector!=13 and $subsector!=2073  and  $subsector!=22 and $subsector!=5002 and $subsector!=568 and  $subsector!=170 and  $subsector!=249 and  $subsector!=144){																	
									    $promedio3 = "EX";
									}
									
									if ($promedio2==NULL and $subsector==13 and $bool_ar!="1"){
									    $promedio3 = "EX";
									}
...
<? 
	...
	$porcentaje = round(($num*100)/$total_filas,2);
	 ...
	 ?>
 <script> 
	var t=setTimeout("window.location='procesoArchivo04.php?num=<?=$num?>&grado_filtro=<?=$grado_filtro?>'",1000);
     </script>
