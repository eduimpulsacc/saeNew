   ...
   //// FUNCION QUE VALIDA EL RUT   ///////
	...
	if ($num< $total_filas){
		...
		/*
			...
			$sql = $sql . "WHERE (((institucion.rdb)=".$institucion.") AND ((ano_escolar.id_ano)=".$ano.") and matricula.id_ano = ".$ano.") and curso.ensenanza > 109 and curso.id_curso = $curso and ((matricula.bool_ar=1 and matricula.fecha_retiro > '".$fecha_mat."') or ((matricula.bool_ar isnull) or (matricula.bool_ar=0)))";
						...
						//{
							if ($modo_eval==1)
							{
								...
								if ($prom>0 and $bool_ar != "1")
									...
									if (($subsector!=13 or $subsector!=5002 or $subsector!=249 or $subsector!=170 )and $prom==NULL  and $bool_ar!="1"){
									     $promedio2 = "";
								         $promedio3 = "EX";
									}
							...
							else
							{
								...
								if ($prom>0)
									...
									if ($subsector!=13 and $subsector!=22 and $subsector!=5002 and $subsector!=568 and  $subsector!=170 and  $subsector!=249){																	
									    $promedio3 = "EX";
									}
...
<?
		...
		<script> setTimeout("window.location='procesoArchivo04.php?num=<? echo $num; ?>'");</script>
</body>
</html>
