...
<BODY>
	<?php echo tope("../../util/");?>
	<FORM method=post name="frm" action="procesoCP.php3">
			...
			<TR height=15>
				<TD>
					<TABLE WIDTH="100%" BORDER=0 CELLSPACING=5 CELLPADDING=5 height="100%">
						...
						<?php if(($_PERFIL==1)||($_PERFIL==14)||($_PERFIL==0)||($_PERFIL==10)){ //DIRECTOR GENERAL , ADM WEB COLEGIO, ADM TOTAL WEB?>
										<?php if($_FRMMODO=="mostrar"){ ?>
											<INPUT TYPE="button" value="MODIFICAR" onClick=document.location="seteaCP.php3?caso=3">
										<?php }?>
										<?php if($_FRMMODO=="modificar"){ ?>
											<INPUT TYPE="submit" value="GUARDAR" onclick="return valida(this.form);">&nbsp;
											<INPUT TYPE="button" value="CANCELAR" onClick=document.location="seteaCP.php3?caso=1">
										<?php }?>
									<?php }?>
								<?php }?>
								<?php if($_PERFIL!=10){?>
						...
						<TR height=20 bgcolor=#0099cc>
							<TD align=middle colspan=2>
								<FONT face="arial, geneva, helvetica" size=2 color=White>
									<strong>CENTRO DE PADRES</strong>
								</FONT>
							</TD>
						</TR>
						<TR>
									...
									<TR>
										<TD>
											...
											<?php if($frmModo=="modificar"){ ?>
												<INPUT type="text" name=txtREP size=83 maxlength=50 value="<?php echo $fila1['repcp']?>">
											<?php };?>
										</TD>
									</TR>
								</TABLE>
								...
								<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0>
									...
									<TR>
										<TD>
											...
											?>
											<?php if($frmModo=="modificar"){ ?>
												<INPUT type="text" name=txtEMAIL size=83 maxlength=50 value="<?php echo $fila1['emailcp']?>">
											<?php };?>
										</TD>
									</TR>
								</TABLE>
						...
						<TR>
							<TD width=30></TD>
							<TD> 
							<INPUT TYPE="button" value="DIARIO ELECTRONICO"   name=btnCancelar onClick=document.location="noticia/listarNoticia.php3?agrupacion=2">
							<INPUT TYPE="button" value="AGENDA WEB" name=btnCancelar name=btnCancelar onClick=document.location="agenda/listarAgenda.php3?agrupacion=2">
							</TD>
						</TR>
					</TABLE>
				</TD>
