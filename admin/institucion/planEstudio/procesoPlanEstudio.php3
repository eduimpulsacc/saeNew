     		...
     		$qry="Select * from plan_inst where ((rdb='".$instit."') and (cod_decreto='".$cmbPLAN."'))";
				$result =@pg_Exec($conn,$qry);
		         if (pg_numrows($result)!=0){ ?>
    			     ...
    			     <?php
			         echo ('<b> EL DECRETO YA EXISTE! 1</b>');
				     ?>
 			 	     </div></td>
				     <div align="center">
  					<script> setTimeout("window.location='listarPlanesEstudio.php3'",2000);</script>
	             	<?php
					exit; 
                }else{
					  ...
					  $result =@pg_Exec($conn,$qry);
		 		 	 ...
		 		 	 echo "<script>window.location = 'listarPlanesEstudio.php3'</script>";
				}  
 }else{  
	if ($frmModo=="ingresar") {	 
		 $qry="Select * from plan_estudio where ((rdb='".$instit."') and cod_decreto='".$txtNRESOL."')";
		 $result =@pg_Exec($conn,$qry);
		 if (@pg_numrows($result)!=0){ ?>
			   <td align="center"> <div align="center">
    			<?php
			  	echo ('<b> EL DECRETO YA EXISTE!2 </b>');
				?>
 			 		</div></td>
					<div align="center">
  					<script> setTimeout("window.location='listarPlanesEstudio.php3'",2000);</script>
			...
			$fec_dd=explode("-",trim($txtFECHARES));
		      $qry="INSERT INTO plan_estudio (cod_decreto, cursos, nombre_decreto, rdb, fecha_decreto) VALUES ('".trim($txtNRESOL)."','".$txtDRES."','".trim($txtNOMDECRE)."','".trim($instit)."','".$fec_dd[2].$fec_dd[1].$fec_dd[0]."')";
		      $result =@pg_Exec($conn,$qry);
		      if (!$result){ 
			      error('<b> ERROR :</b>Error al acceder a la BD. (3)'.$qry);
			      exit;
		...
		for($i=1 ; $i <= 20 ; $i++){
			...
			echo "<script>window.location = 'listarPlanesEstudio.php3?plan=".$txtNRESOL."&ensenanza=".$cmbENS."'</script>";
			  }
		 }
	}
