...
<BODY>
	<?php echo tope("../../../util/");?>
	<FORM method=post name="frm" action="procesoActividadesEspeciales.php">
							...
							<TD align=right colspan=2>
								...
								<?php if($frmModo=="mostrar"){ ?>
									<?php if(($_PERFIL!=2)&&($_PERFIL!=4)){ //ACADEMICO Y LEGAL?>
										...
										<INPUT TYPE="button" value="MODIFICAR" name=btnModificar  onClick=document.location="seteaActividadesEspeciales.php?caso=3">&nbsp;
											<?php }?>
									<?php } //ACADEMICO Y LEGAL?>
									<INPUT TYPE="button" value="VOLVER" onClick=document.location="../institucion.php">&nbsp;
								<?php };?>

								<?php if($frmModo=="modificar"){ ?>
									<INPUT TYPE="submit" value="GUARDAR"   name=btnGuardar onclick="return valida(this.form);">&nbsp;
									<INPUT TYPE="button" value="CANCELAR" name=btnCancelar onClick=document.location="seteaActividadesEspeciales.php?caso=1">&nbsp;
								<?php };?>
							</TD>
						</TR>
						<TR height=20 bgcolor=#0099cc>
							<TD align=middle colspan=2>
								<FONT face="arial, geneva, helvetica" size=2 color=White>
									<strong>ACTIVIDADES ESPECIALES</strong>
								</FONT>
							</TD>
						</TR>
						<TR>
								...
								<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0>
									...
									<TR>
										<TD>
											...
											<?php 
												if($frmModo=="mostrar"){ 
													imp($fila['actividades_especiales']);
												};
											?>
											<?php if($frmModo=="modificar"){ ?>
												<TEXTAREA NAME="txtCONT" ROWS="20" COLS="60">
												<?php 
													echo $fila['actividades_especiales'];
												?>
												</TEXTAREA>
											<?php };?>
										</TD>
