...
<?php
	...
	$empleado		=$_EMPLEADO;
	$institucion = $_INSTIT;
	$por=$_REQUEST["tipo_conducta"];
	$sigla2=$_POST["cmb_sigla"];
	$txtOBS=$_POST["txtOBS"];
	$txtFECHA=$_POST["txtFECHA"];
	$cmb_periodos=$_POST["cmb_periodos"];
	$rdTIPO=$_POST["rdTIPO"];
				...
				if (!$result) {
					...
					$newID = $newID + 1 ;
					  ...
					  // buscar codigos y luego grabara
							$q1 = "select * from sigla_subsectoraprendisaje where id_sigla = '".trim($sigla2)."' and rdb = '".trim($institucion)."'";
							$r1 = pg_Exec($conn,$q1);
							$n1 = pg_numrows($r1);
						if ($n1==0){	?>
							...
							$qry="INSERT INTO ANOTACION (ID_ANOTACION, TIPO,FECHA, OBSERVACION, RUT_ALUMNO, RUT_EMP, id_periodo, sigla, codigo_tipo_anotacion, codigo_anotacion, rdb) VALUES	('".$newID."','".$rdTIPO."','".fEs2En($txtFECHA)."','".trim($txtOBS)."','".trim($rut)."','".$empleado."','" .$cmb_periodos."' , '".$sigla_subsector."', '".$tipo_anotacion."', '".$detalle_anotaciones."', '".$institucion."')";
