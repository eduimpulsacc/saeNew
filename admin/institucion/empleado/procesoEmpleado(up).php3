...
if ($frmModo=="ingresar"){
	...
	if (!$result){error('<B> ERROR :</b>Error al acceder a la BD. (1)</B>');
			...
			if(pg_numrows($result)!=0){
				...
				if (!$result){
					...
					if(!$result){
						...
						if(pg_numrows($result)==0){
							...
							$result =@pg_Exec($conn,$qry);
							if (!$result){error('<b> ERROR :</b>Error al acceder a la BD.(2)');}//else{echo "<script>window.location = 'seteaEmpleado.php3?caso=1&empleado=".$txtRUT."'</script>";}
						}
					}
				}
				...
				$qry="INSERT INTO EMPLEADO (RUT_EMP, DIG_RUT, NOMBRE_EMP, APE_PAT, APE_MAT, CALLE, NRO, DEPTO, BLOCK, VILLA, REGION, CIUDAD, COMUNA, TELEFONO, SEXO, TITULO, EMAIL, ESTADO_CIVIL, ESTUDIOS) VALUES (".trim($txtRUT).",'".trim($txtDIGRUT)."','".trim($txtNOMBRE)."','".trim($txtAPEPAT)."','".trim($txtAPEMAT)."','".trim($txtCALLE)."','".trim($txtNRO)."','".trim($txtDEP)."','".trim($txtBLO)."','".trim($txtVIL)."',".trim($txtREG).",".trim($txtCIU).",".trim($txtCOM).",'".trim($txtTELEF)."',".trim($cmbSEXO).",'".trim($txtTITULO)."','".trim($txtEMAIL)."',".$cmbCIVIL.",'".$txtESTUDIOS."')";
				$result =@pg_Exec($conn,$qry);
				if (!$result){
...
if ($frmModo=="modificar") {
		$qry="UPDATE empleado SET nombre_emp = '".trim($txtNOMBRE)."', ape_pat = '".trim($txtAPEPAT)."', ape_mat = '".trim($txtAPEMAT)."', calle = '".trim($txtCALLE)."', nro = '".trim($txtNRO)."', depto = '".trim($txtDEP)."', block = '".trim($txtBLO)."', villa = '".trim($txtVIL)."', region = ".$txtREG.", ciudad = ".$txtCIU.", comuna = ".$txtCOM.", telefono = '".trim($txtTELEF)."', sexo = ".$cmbSEXO.", titulo = '".trim($txtTITULO)."', email = '".trim($txtEMAIL)."', estado_civil = ".$cmbCIVIL." , estudios ='".$txtESTUDIOS."' WHERE (((rut_emp)=".$_EMPLEADO."))";
		$result =@pg_Exec($conn,$qry);
		if (!$result) {
			error('<b> ERROR :</b>Error al acceder a la BD. (3)'.$qry);
		}else{
