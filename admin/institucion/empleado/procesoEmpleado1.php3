...
echo $habilitado_para=serialize($cod_subsector);
...
if ($frmModo=="ingresar"){
	...
	if (!$result){error('<B> ERROR :</b>Error al acceder a la BD. (1)</B>');
			...
			if (!iseet($txtNROres)){ $txtNROres=0;}
			if($txtFECHA==""){
					...
					if(!$result){
						...
						if(pg_numrows($result)==0){
						 ...
						 if (($cmbCARGO2)&&(!$arreglo_cargo[1])){ //si no exite en la db
 							$qry="INSERT INTO TRABAJA (RDB,RUT_EMP,CARGO) VALUES (".trim($rdb).",".trim($txtRUT).",".$cmbCARGO2.")";
							$result =@pg_Exec($conn,$qry);
						 }
						 if (($cmbCARGO2)&&($arreglo_cargo[1])){ //si  exite en la db
							$qry="UPDATE TRABAJA SET CARGO=".$cmbCARGO1." WHERE (RDB=".$_INSTIT.") AND (RUT_EMP=".$txtRUT.") and cargo=$arreglo_cargo[1]";
							$result =@pg_Exec($conn,$qry);
						 }
...
/// ---CURSOS RECONOCIDOS tipo=4///
			...
			$cmbTITULO=0;
				...
				if (!$result){
					...
					$qry="INSERT INTO TRABAJA (RDB,RUT_EMP,CARGO) VALUES (".trim($rdb).",".trim($txtRUT).",".$cmbCARGO1.")";
					$result =@pg_Exec($conn,$qry);
					if (!$result){error('<b> ERROR :</b>Error al acceder a la BD.(2)'.$qry);}//else{echo "<script>window.location = 'listarEmpleado.php3'<!-/script-->";}
				}
