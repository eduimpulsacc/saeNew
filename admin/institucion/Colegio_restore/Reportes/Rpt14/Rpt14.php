...
<table width="670" border="1" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr class="texto8px"> 
	...
	<?
	//number_format(($li_promedio_semestral/$li_cont_ramos),0)
	//$prom_gen = $prom_gen + pg_result($resultado_query_notas, 0, 23);
	//$cont_gen = $cont_gen + 1;
	if ($cont_gen>0)
	{
	...
	//if ($sw == 0) $promedio_gen = int(($promedio_gen*10) / $prom_cont);			  
	//echo $prom_cont;
	echo $prom_gen;
	}
	?>
  ...
  <tr class="texto8px"> 
    <td colspan="22"><div align="right">aSISTENCIA pERIODO:<? print number_format((pg_result($resultado_query_asistencia, 0, $x)/pg_result($resultado_query_fecha, $x, 6))*100,0);?>% 
 </div></td>
  </tr>
</table>
<br>
