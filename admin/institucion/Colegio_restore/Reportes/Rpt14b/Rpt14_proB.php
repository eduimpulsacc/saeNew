...
<?
	...
	//---------------------------
	$sql69 = "select * from curso where grado_curso = " . $li_curso . " and letra_curso = '" . $ls_letra . "' and ensenanza = ". $li_tipo_ense . " and id_ano = ". $ano;

	$result10 =@pg_Exec($conexion,$sql69);
	if (!$result10) 
	{
		error('<B> ERROR :</b>Error al acceder a la BD. (CURSO)</B>');
	}
	else
	{
		if (pg_numrows($result10)!=0)
	...
	//--
	$sql88 = "select * from matricula, alumno where alumno.rut_alumno = matricula.rut_alumno and id_curso = ". $curso . " and id_ano = ". $ano . " order by ape_pat, ape_mat, nombre_alu";

	$result88 =@pg_Exec($conexion,$sql88);
	if (!$result88) 
	{
		error('<B> ERROR :</b>Error al acceder a la BD. (MATRICULA0)</B>');
	}
	else
	{
		if (pg_numrows($result88)!=0)
		{
			$fila88 = @pg_fetch_array($result88,0);	
			if (!$fila88)
			{
				error('<B> ERROR :</b>Error al acceder a la BD. (MATRICULA1)</B>');
				exit();
			}
		}
	}

$cont = @pg_numrows($result88);
for($yy=0 ; $yy <  $cont ; $yy++)
{
$fila88 = @pg_fetch_array($result88,$yy);
$alumno = $fila88['rut_alumno'];
?>
<table width="650" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td></td>
    ...
    <td><table width="650" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
			...
			?>
              ...
              <tr>
				...
				if (!$result) 
				{
					error('<B> ERROR :</b>Error al acceder a la BD. (PROFESOR)</B>');
				}
				else
				{
					if (pg_numrows($result)!=0)
      ...
      <tr>
		...
		<?
		  ...
		  $sql2 = $sql2 . "FROM (ramo INNER JOIN subsector ON ramo.cod_subsector = subsector.cod_subsector) INNER JOIN tiene3 ON (ramo.id_curso = tiene3.id_curso) AND (ramo.id_ramo = tiene3.id_ramo) ";
   		  $sql2 = $sql2 . "WHERE (((ramo.id_curso)=".$curso.") AND ((tiene3.rut_alumno)=".$alumno.")); ";
