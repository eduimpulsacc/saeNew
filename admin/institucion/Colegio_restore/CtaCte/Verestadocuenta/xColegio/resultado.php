...
<?	
	...
	$total_filas_datos_cuentas     = pg_numrows($resultado_query_datos_cuentas);
			...
			// *** QUERY QUE SACA VALORES DE SALDO
...
<form name="form1" method="post" action="procesar.php?ai_colegio=<?=($li_id_colegio)?>">
  <?
		if($li_mostrar_tabla==1)
	{
			...
			<div align="center">
			  ...
			  <input type="button" name="cb_back" value="&lt;&lt; Volver a Consultar" class="cb_none_9_x_200" onClick="MM_goToURL('parent.frames[\'main\']','resultado.php?ai_mostrar=1&as_rut_apo='+hf_rut_apo.value+'&ai_colegio='+hf_id_colegio.value);return document.MM_returnValue">
			</div>
			</td>
			</tr>
			</table>
  ...
  <table width="80%" border="1" align="center" cellspacing="0" cellpadding="0">
    ...
    <tr> 
	  ...
	  ?>
        <select name="ddlb_mi" class="ddlb_9_x_100" OnChange="parent.main.location.href='resultado.php?ai_mostrar=1&ai_colegio='+hf_id_colegio.value+'&as_rut_apo='+ddlb_apoderado.options[ddlb_apoderado.selectedIndex].value+'&ai_mes='+ddlb_mi.options[ddlb_mi.selectedIndex].value">
  ...
  <table width="80%" border="1" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr> 
      ...
      <td class="membrete_datos">
        <select name="ddlb_apoderado" class="ddlb_9_x_250" OnChange="parent.main.location.href='resultado.php?ai_mostrar=1&as_rut_apo='+ddlb_apoderado.options[ddlb_apoderado.selectedIndex].value+'&ai_colegio='+hf_id_colegio.value+'&ai_mes='+ddlb_mi.options[ddlb_mi.selectedIndex].value">
          <?
	For ($j=0; $j < $total_filas; $j++)
	{
	?>
          <option value="<?=Trim(pg_result($resultado_query, $j, 2));?>" <? If(Trim(pg_result($resultado_query, $j, 2)) == $ls_rut_apoderado) {echo("selected");}?> >
		  <?=Trim(pg_result($resultado_query, $j, 3));?> <?=Trim(pg_result($resultado_query, $j, 4));?> <?=Trim(pg_result($resultado_query, $j, 5));?>
		 </option>
	<?
	}
	?>
	...
	$li_monto = Trim(pg_result($resultado_query_datos, 0, 10));
	$li_total_pagar = $li_total_pagar + $li_monto;
	

		IF($total_filas_saldo <= 0)
		{
		$li_monto_saldo = 0;
		}Else
		{
	...
	$li_monto_total = ($li_total_pagar + $li_monto_saldo);
	//ECHO("<BR> Calculo : ($li_total_pagar) + ($li_monto_saldo) ");
	?><b><?=number_format($li_monto_total,2)?></b>
	  </td>
    </tr>
  </table>
  <br>
  ...
  <table width="80%" border="1" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr> 
      <td class="membrete_datos">&nbsp; 
        <?print Trim(pg_result($resultado_query_datos_cuentas, $x, 4));?>
      </td>
      <td class="membrete_datos"> 
        <div align="right">&nbsp; 
          <?print number_format(pg_result($resultado_query_datos_cuentas, $x, 5),2);?>
        </div>
      </td>
    </tr>
    <?
  ...
  <table width="80%" border="1" align="center" cellpadding="0" cellspacing="0">
    <tr> 
      ...
      <td class="linea_datos_02"> 
        <div align="right"><?=number_format($li_total_pagar,2)?>
</div>
      </td>
    </tr>
    <tr> 
...
?>
    ...
    <tr> 
	  ...
	  $li_monto_total = ($li_monto_saldo + $li_total_pagar);
	  ?>
          <b> 
          <?=number_format($li_monto_total,2)?>
          </b> </div>
      </td>
    </tr>
  </table>
  <?
  } //Cierra el Ciclo Saldo
  ?>
  <BR>
  <table width="80%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
