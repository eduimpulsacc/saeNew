...
//--------TRAE USUARIOS QUE HACEN CAJA-------
	...
	if($_PERFIL!=5){
		$sql_users = "select empleado.nombre_emp, empleado.ape_pat, empleado.ape_mat, empleado.rut_emp, empleado.id_usuario from empleado inner join accede on empleado.id_usuario = accede.id_usuario";
		 $sql_users = $sql_users . " where accede.id_perfil=5 and empleado.rut_emp in (select empleado.rut_emp from empleado inner join trabaja on empleado.rut_emp=trabaja.rut_emp where rdb=$_INSTIT) ";
	}else{
		$sql_users = "select empleado.nombre_emp, empleado.ape_pat, empleado.ape_mat, empleado.rut_emp, empleado.id_usuario from empleado inner join accede on empleado.id_usuario = accede.id_usuario";
...
<br>
  ...
  if($_PERFIL!=5){
  $user=$cmb_usuario;
 //	$sql_subt_ctas= "select distinct sum(cdd.monto) as monto, cdd.id_cuenta from con_documenta_comprobante as cdc, con_documenta_doc as cdd where cdc.id_comprobante_doc = cdd.id_comprobante_doc and cdc.id_usuario = ".$cmb_usuario." and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') group by cdd.id_cuenta";
//  	$sql_subt_ctas="select distinct sum(cdd.monto) as monto, cdd.id_cuenta, cc.nombre from con_documenta_comprobante as cdc, con_documenta_doc as cdd, con_cuenta as cc where cdc.id_comprobante_doc = cdd.id_comprobante_doc and cc.id_cuenta=cdd.id_cuentaand cdc.id_usuario = ".$cmb_usuario." and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') group by cdd.id_cuenta, cc.nombre";
  }else{
  $user=$_USUARIO;
  	//$sql_subt_ctas= "select distinct sum(cdd.monto) as monto, cdd.id_cuenta from con_documenta_comprobante as cdc, con_documenta_doc as cdd where cdc.id_comprobante_doc = cdd.id_comprobante_doc and cdc.id_usuario = ".$_USUARIO." and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') group by cdd.id_cuenta";
	//$sql_subt_ctas="select distinct sum(cdd.monto) as monto, cdd.id_cuenta, cc.nombre from con_documenta_comprobante as cdc, con_documenta_doc as cdd, con_cuenta as cc where cdc.id_comprobante_doc = cdd.id_comprobante_doc and cc.id_cuenta=cdd.id_cuentaand cdc.id_usuario = ".$_USUARIO." and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') group by cdd.id_cuenta, cc.nombre";
  }
  $sql_subt_ctas= "select distinct sum(cdd.monto) as monto, cdd.id_cuenta from con_documenta_comprobante as cdc, con_documenta_doc as cdd where cdc.id_comprobante_doc = cdd.id_comprobante_doc and cdc.id_usuario = ".$user." and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') group by cdd.id_cuenta";
  $result_subt_ctas=@pg_exec($conexion,$sql_subt_ctas);
  for($count_s=0; $count_s<@pg_numrows($result_subt_ctas) ; $count_s++){
  $fila_subt_ctas=@pg_fetch_array($result_subt_ctas, $count_s);
  ?>
    <tr> 
      <td width="200" class="membrete_datos">subtotal&nbsp;<?	$sql_nom="select nombre from con_cuenta where id_cuenta=".$fila_subt_ctas['id_cuenta'];
												$res_nom=pg_exec($conexion, $sql_nom);
												$fila_nom=pg_fetch_array($res_nom, 0);
												echo trim($fila_nom['nombre']);
	  ?>
	  &nbsp;</td>
      <td width="105" align="right" class="membrete_datos">&nbsp;<? 	$sql_chq2 = " select sum(cdd.monto) as monto, count(id_tipo_documento) as cant from con_documenta_doc as cdd inner join con_documenta_comprobante as cdc";
															$sql_chq2 = $sql_chq2 . " on cdd.id_comprobante_doc=cdc.id_comprobante_doc where cdd.id_cuenta=".$fila_subt_ctas['id_cuenta']." and cdc.id_usuario = ".$user."";
															$sql_chq2 = $sql_chq2 . " and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') and cdd.id_tipo_documento=2 group by cdd.id_cuenta";
	  														$res_chq2 = @pg_exec($conexion,$sql_chq2);
															$fila_chq2 = @pg_fetch_array($res_chq2,0);
															$monto_chq2 = $fila_chq2['monto'];
															$cant_doc_chq2 = $fila_chq2['cant'];
															if($cant_doc_chq2==0) $cant_doc_chq2=0;
	  														echo number_format($monto_chq2, '2'), " " , "(" , trim($cant_doc_chq2), ")" ;?></td>
      <td width="108" align="right" class="membrete_datos">&nbsp;<? 	$sql_ltr2 = " select sum(cdd.monto) as monto, count(id_tipo_documento) as cant from con_documenta_doc as cdd inner join con_documenta_comprobante as cdc";
															$sql_ltr2 = $sql_ltr2 . " on cdd.id_comprobante_doc=cdc.id_comprobante_doc where cdd.id_cuenta=".$fila_subt_ctas['id_cuenta']." and cdc.id_usuario = ".$user."";
															$sql_ltr2 = $sql_ltr2 . " and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') and cdd.id_tipo_documento=3 group by cdd.id_cuenta";
	  														$res_ltr2 = @pg_exec($conexion,$sql_ltr2);
															$fila_ltr2 = @pg_fetch_array($res_ltr2,0);
															$monto_ltr2 = $fila_ltr2['monto'];
															$cant_doc_ltr2 = $fila_ltr2['cant'];
															if($cant_doc_ltr2 == 0) $cant_doc_ltr2 =0;
	  														echo number_format($monto_ltr2, '2'), " " , "(" , trim($cant_doc_ltr2), ")" ;?></td>
      <td width="109" align="right" class="membrete_datos">&nbsp;<? 	$sql_ctd2 = " select sum(cdd.monto) as monto, count(id_tipo_documento) as cant from con_documenta_doc as cdd inner join con_documenta_comprobante as cdc";
															$sql_ctd2 = $sql_ctd2 . " on cdd.id_comprobante_doc=cdc.id_comprobante_doc where cdd.id_cuenta=".$fila_subt_ctas['id_cuenta']." and cdc.id_usuario = ".$user."";
															$sql_ctd2 = $sql_ctd2 . " and cdc.fecha = to_date('".$fecha_buscar."', 'MM DD YYYY') and cdd.id_tipo_documento=1 group by cdd.id_cuenta";
	  														$res_ctd2 = @pg_exec($conexion,$sql_ctd2);
															$fila_ctd2 = @pg_fetch_array($res_ctd2,0);
															$monto_ctd2 = $fila_ctd2['monto'];
															$cant_doc_ctd2 = $fila_ctd2['cant'];
															if($cant_doc_ctd2=="") $cant_doc_ctd2=0;
	  														echo number_format($monto_ctd2, '2'), " " , "(" , trim($cant_doc_ctd2), ")";?></td>
      <td width="116" align="right" class="membrete_datos">&nbsp;</td>
    </tr>
    <tr>
	<?php }//fin for?>
      <td class="membrete_datos">TOTAL GENERAL&nbsp;</td>
      <td class="membrete_datos" align="right"><? echo number_format($cheques_total, '2'), " " , "(" , trim($total_count_chq), ")";?></td>
      <td class="membrete_datos" align="right"><? echo number_format($letras_total, '2'), " " , "(" , trim($total_count_ltr), ")"?></td>
      <td class="membrete_datos" align="right"><? echo number_format($ctdo_total, '2'), " " , "(" , trim($total_count_ctdo), ")"?></td>
      <td class="membrete_datos" align="right"><? echo number_format($total_gral, '2');?></td>
    </tr>
  </table>
