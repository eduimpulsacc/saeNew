<? include"../../Coneccion/conexion.php";
	//include ('../../util/ValidaCaracteres.inc');
...
/*	function descuento(){
	...
	if ($cb_finaliza!=""){
		...
		$sql_doc_comp= $sql_doc_comp. "fecha = to_date('" . $fecha_actual . "','DD MM YYYY'), ";
		//$sql_doc_comp= $sql_doc_comp. "fecha = fEs2En('".$fecha_actual."'), ";
...
//		$sql_doc_comp= $sql_doc_comp. "nro_comprobante = " . $newNroComprobante . ", ";
		...
		for($k=0 ; $k<$hasta ; $k++){
			...
			$fecha=$fila['fechavenc'];
			$ano = substr (($fecha),0,4);
			$mes = substr(($fecha),5,2);
			$dia = substr(($fecha),8,3);
			$fecha1 = sprintf ("%02d-%02d-%04d", $dia, $mes, $ano);
			$sql_doc = "INSERT INTO con_documenta_doc VALUES (" . trim($fila['id_temp']) . ", " . trim($fila['item']) . ", " . trim($fila['id_tipo_documento']) . ", '" . trim($fila['numero']) ."', " . trim($fila['monto']) . ", to_date('".$fecha1."', 'DD MM YYYY') , '" . trim($fila['observacion02']) . "' , ".$fila['id_cuenta'].",NULL, " . $fila['id_banco'] ." ) ";
	...
	$sql = "Select distinct a.rdb, a.id_ano, b.rut_apo, c.nombre_apo, c.ape_pat, c.ape_mat, e.nro_ano From matricula a inner join tiene2 b on a.rut_alumno = b.rut_alumno inner join apoderado c on b.rut_apo = c.rut_apo, ano_escolar e where a.rdb = $_INSTIT And a.rdb = e.id_institucion And a.id_ano = e.id_ano Order by 4 ;";
	...
	$sql = "Select distinct a.rdb, b.rut_apo, c.nombre_apo, c.ape_pat, c.ape_mat From matricula a inner join tiene2 b on a.rut_alumno = b.rut_alumno inner join apoderado c on b.rut_apo = c.rut_apo, ano_escolar e where a.rdb = $_INSTIT And a.rdb = e.id_institucion And a.id_ano = e.id_ano  And b.sostenedor=1 Order by 4,5 ;";
	//$sql = "Select distinct a.rdb, a.id_ano, b.rut_apo, c.nombre_apo, c.ape_pat, c.ape_mat, e.nro_ano From matricula a inner join tiene2 b on a.rut_alumno = b.rut_alumno inner join apoderado c on b.rut_apo = c.rut_apo, ano_escolar e where a.rdb = $_INSTIT And a.rdb = e.id_institucion And a.id_ano = e.id_ano  And b.sostenedor=1 Order by 5,6 ;";
	//$sql = "Select distinct a.rdb, a.id_ano, b.rut_apo, c.nombre_apo, c.ape_pat, c.ape_mat, e.nro_ano From matricula a inner join tiene2 b on a.rut_alumno = b.rut_alumno inner join apoderado c on b.rut_apo = c.rut_apo, ano_escolar e where a.rdb = $_INSTIT And a.rdb = e.id_institucion And a.id_ano = e.id_ano And e.nro_ano = $year_actual And b.sostenedor=1 Order by 5,6 ;";
	...
	If($total_filas > 0){
	  If(Trim($cmbapoderado) == ''){
	  	$cmbapoderado = Trim(@pg_result($resultado_query, 0, 1));
	  }Else{
	...
	$sql_documento = "INSERT INTO con_documento_temp (id_temp, item, id_tipo_documento, numero, monto, fechavenc, observacion02, id_cuenta,id_banco) VALUES (" . trim($max_comp) .", " . trim($ls_item) . ", " . trim($cmbtipo_doc) . ", '" . trim($serie) . "', ". trim($monto) . ", to_date('".trim($venc)."', 'DD MM YYYY'), '" . trim($obs) ."', ".$cmbcuenta.", " .$cmbbanco . ")";
...
//				form.action = form.cmbPERIODO.value;
	...
	function Validar(form){
		...
		if (!ValVac(document.form.cmbcuenta,document.form.cmbcuenta.options[document.form.cmbcuenta.options.selectedIndex].value,"Debe seleccionar la Cuenta a Documentar")){ return; };
...
<input name="imprime" type="hidden" value="<? echo $imprime;?>">
  <table width="660" border="1" align="center" cellspacing="0" cellpadding="0">
    <tr> 
      <td width="101" class="linea_datos_02">INSTITUCI&Oacute;N</td>
      <td width="429" class="linea_datos_02" align="center"> <? print Trim(@pg_result($resultado_query_instit, 0, 1));?></td>
      <td width="161" class="linea_datos_02">
...
<BR>
  <table width="660" border="1" align="center" cellspacing="0" cellpadding="0">
	...
	{
	?>
          <option value="<?=Trim(@pg_result($resultado_query, $j, 1));?>" <? If(Trim(@pg_result($resultado_query, $j, 1)) == $cmbapoderado) {echo("selected");}?> > 
          <?=Trim(@pg_result($resultado_query, $j, 3));?>
          <?=Trim(@pg_result($resultado_query, $j, 4));?>
          <?=Trim(@pg_result($resultado_query, $j, 2));?>
          </option>
          <?
	}
...
<div><a name="tabla"></a></div>
  <table width="680" border="1" align="center" cellspacing="0" cellpadding="0" id="capa1">
    <tr> 
    <td colspan="8" class="linea_datos_02"><div align="center">DOCUMENTO PAGO</div></td>
  </tr>
  <tr> 
    <td width="100" class="linea_datos_05">CUENTA</td>
    <td width="75" class="linea_datos_05">TIPO DOCUMENTO</td>
	<td width="100" class="linea_datos_05">banco</td>
    <td width="80" class="linea_datos_05">NUMERO-SERIE</td>
    <td width="100" class="linea_datos_05">MONTO $</td>
    <td width="100" class="linea_datos_05">FECHA VENC</td>
    <td width="100" class="linea_datos_05">OBSERVACION</td>
    <td width="24" class="linea_datos_05">&nbsp; </td>
  </tr>
  <tr>
  <td>
  <select name="cmbcuenta" class="text_9_x_100">
	...
	<?  $fecha="";
		...
		$sqlIncFec="select fechavenc from con_documento_temp where numero='".$filaIncDoc['max']."' and id_temp=".$max_comp;
		$resIncFec = @pg_exec($conexion,$sqlIncFec);
		$filaIncFec = @pg_fetch_array($resIncFec,0);
		$nserie = $filaIncDoc['max']+1;
		
			
	// AAQ2UI CAMBIO DE FECHA **************************
	
		
		$dia=substr($filaIncFec['fechavenc'], 8,2);
		$slash_uno = substr($filaIncFec['fechavenc'], 7,1);
		$mes = substr($filaIncFec['fechavenc'], 5,2);
		$slash_dos = substr($filaIncFec['fechavenc'], 4,1);
		$ano = substr($filaIncFec['fechavenc'],0,4);
		if ($mes==12){
			$nmes = 1;
			$ano = $ano + 1;
		}else{
	 ...
	 <td class="membrete_datos"><select name="cmbbanco"  class="ddlb_9_x_100">
        <OPTION value="0">Seleccione Banco</OPTION>
  <?
	For ($j=0; $j < @pg_numrows($Rs_Banco); $j++){
		$fils = @pg_fetch_array($Rs_Banco,$j);?>
		
        <option value=<? echo $fils['id_banco'];?> <? If($fils['id_banco'] == $cmbbanco) {?>selected <? }?>><? echo trim($fils['nombre']);?></option>
 <? }?>
        </select></td>
    ...
    <td class="membrete_datos"><div align="center"><input type="text" name="venc" class="text_9_x_100" value="<? echo $nfecha;?>" ></div></td>
	...
	<input  type="button" name="cb_save" value="+" class="cb_submit_9_20x17" onmouseover=this.style.background='FFFFD7';this.style.color='003b85' onmouseout=this.style.background='#5c6fa9';this.style.color='ffffff' onClick="javascript:Validar(this.form);"></td>
  </tr>
</table>
<br>
<? if($paso==1){?>
  ...
  ?>
    <tr> 
	 ...
	 <td class="membrete_datos"><? $sql_bco ="select nombre from banco where id_banco=". $fila_doc['id_banco'];
	 							   $Rs_bco = @pg_exec($conexion, $sql_bco);
								   $fila_bco = @pg_fetch_array($Rs_bco,0);
								   echo $fila_bco['nombre'];?> &nbsp;</td>
      <td class="membrete_datos"><? echo $fila['numero'];?>&nbsp;</td>
      <td class="membrete_datos" align="right"><? echo number_format($fila['monto'], '2');?>&nbsp;</td>
      <td class="membrete_datos" align="center"><? impF($fila['fechavenc']);?>&nbsp;</td>
