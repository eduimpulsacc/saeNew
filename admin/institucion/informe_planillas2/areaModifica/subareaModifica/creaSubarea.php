...
<body>
<form action="../../../planilla_planillas2/areaModifica/subareaModifica/procesoSubarea.php" method="post">
<!-- <input name="plantilla" type="text" value="<?php //echo $plantilla?>"> -->
<?php 
if($creada!=1){
	$sqlTraeAreas="SELECT * FROM informe_area where id_plantilla=".$plantilla." AND con_subarea=1 order by id_area";
	...
	for ($cant=0 ; $cant<$txtCant ; $cant++){
			...
			$obj = $obj."<input name=\"txtNombreSubar[".$cant."]\" type=text size=30 maxlength=200>";
			$obj = $obj."</font></td>";
	...
	if($cant!=0){
		...
		echo "<script>window.location='area/subarea/subarea.php'</script>";
	}
}
	...
	if($creada==1){
		...
		$sqlTraeSubareas="select informe_subarea.nombre, informe_subarea.id_subarea from informe_subarea inner join informe_area on informe_subarea.id_area=informe_area.id_area where informe_area.id_plantilla=".$plantilla."order by id_subarea" ;
		...
		if (!$resultTraeSubareas)
					...
					for($count=0 ; $count<pg_numrows($resultTraeSubareas) ; $count++){
						...
						echo "&nbsp;&nbsp;";
						echo "<a href='#' onclick=\"parent.frames['iframeItem'].location.href='itemModifica/item.php?subarea=".$filaSubareas['id_subarea']."';\">";
